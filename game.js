...
(() => {
  ...
  const incomingDamageScale = () => (hardMode?1.25:1)*(impossibleMode?1.28:1)*(endlessMode
    ? Math.min(2.35,(endlessFromHall?endlessBuildTier===2?1.25:endlessBuildTier===1?1.12:1:1) * (1 + wave*.012))
  ...
  let endlessMode=false,endlessSelectedBuild=null,endlessFromHall=false,endlessBuildTier=0,endlessBossesDefeated=0,stageVisual=0,stagePrepared=false;
  const endlessCarryFactor=()=>endlessFromHall?(endlessBuildTier===2?2.9:endlessBuildTier===1?1.8:1.22):1;
  ...
  try{
    ...
    hallOfFame=hallOfFame.filter(r=>r&&typeof r==='object'&&(!Number.isInteger(r.gamePlusLevel)||[0,1,2].includes(r.gamePlusLevel)));
  }catch{}
  try{const records=JSON.parse(localStorage.getItem('snickers3-endless-hall-v1')||'[]');if(Array.isArray(records))endlessHallOfFame=records.filter(r=>r&&r.endlessMode===true&&r.build&&typeof r.build==='object'&&Number.isFinite(r.endlessWave));}catch{}
  ...
  const achievements = [
    ...
    {id:'power_encyclopedia',title:'POWER-LEXIKON',desc:'Sammle alle 16 verschiedenen Power-ups mindestens einmal.',goal:16,kind:'progress'},
  ...
  function hallBuild(entry){
    const b=entry.build||{};
    return {...b,hardMode:Boolean(entry.hardMode??b.hardMode),impossibleMode:Boolean(entry.impossibleMode??b.impossibleMode),sourceTier:hallTier(entry),maxHp:b.maxHp??entry.maxHp,skills:levelMap(b.skills??entry.skills),specials:levelMap(b.specials??entry.specials),weapons:[...new Set(b.weapons??entry.weapons??['nutBomb'])],scorePerks:{...(entry.scorePerks||{}),...(b.scorePerks||{})}};
  }
  function restoreBuildValues(build){
    ...
    if(characterRoster[p.characterId]&&p.characterId!=='snickers'){
      const starter=characterRoster[p.characterId].starter;
      p.weapons=[starter,...p.weapons.filter(id=>!starterWeaponIds.has(id))];
      p.weaponUses[starter]=weaponBook[starter].max+(p.ammoBonus||0)+(p.bombBonus||0);
    }
  ...
  function applyNGPlusBuild(p,build){
    ...
    p.weaponUses={};for(const id of p.weapons)p.weaponUses[id]=(weaponBook[id]?.max||0)+(p.ammoBonus||0)+(starterWeaponIds.has(id)?(p.bombBonus||0):0);
  ...
  const hallTier=r=>[0,1,2].includes(r?.gamePlusLevel)?r.gamePlusLevel:2;
  const hallDifficulty=r=>r?.impossibleMode||r?.build?.impossibleMode?'impossible':r?.hardMode||r?.build?.hardMode?'hard':'normal';
  const tierName=level=>['NORMAL','NEW GAME+','NEW GAME+2'][level]||'NORMAL';
  function completedEntry(build){return {hardMode:Boolean(hardMode),impossibleMode:Boolean(impossibleMode),gamePlusLevel,completedAt:new Date().toISOString(),score,kills,time:Math.floor(runTime),maxHp:player.maxHp,retriesLeft,skills:Object.keys(player.skills||{}),specials:Object.keys(player.specials||{}),weapons:[...(player.weapons||[])],scorePerks:{...(player.scorePerks||{})},build};}
  function archiveCampaignWin(build){
    const entry=completedEntry(build);hallOfFame.unshift(entry);
    try{localStorage.setItem('snickers3-hall-of-fame-v1',JSON.stringify(hallOfFame));}catch{toast('Dieser Abschluss konnte nicht in der Hall of Fame gespeichert werden.');}
  }
  function archiveEndlessRun(voluntary){
    if(!endlessMode||!player||player.endlessHallArchived)return;
    const survived=Math.max(0,Math.floor(player.endlessCleared||0));
    if(!voluntary&&wave+1<50)return;
    const entry={...completedEntry(captureNGPlusBuild(player)),endlessMode:true,endlessWave:voluntary?survived:Math.max(wave+1,survived),endlessCleared:survived,voluntary:Boolean(voluntary),gamePlusLevel:3};
    endlessHallOfFame.unshift(entry);player.endlessHallArchived=true;
    try{localStorage.setItem('snickers3-endless-hall-v1',JSON.stringify(endlessHallOfFame));}catch{toast('Endlos-Build konnte nicht in der Hall of Fame gespeichert werden.');}
  }
  function migrateLegacyHallRecords(){
    let changed=false;
    for(const [difficulty,b0,b1] of [['normal',ngPlusBuild,ngPlus2Build],['hard',hardProgress.ngBuild,hardProgress.ng2Build],['impossible',impossibleProgress.ngBuild,impossibleProgress.ng2Build]]){
      for(const [level,build] of [[0,b0],[1,b1]]){
        if(!build||typeof build!=='object'||hallOfFame.some(r=>hallDifficulty(r)===difficulty&&hallTier(r)===level))continue;
        if(Boolean(build.hardMode)!==(difficulty!=='normal')||Boolean(build.impossibleMode)!==(difficulty==='impossible'))continue;
        hallOfFame.push({legacy:true,gamePlusLevel:level,hardMode:difficulty!=='normal',impossibleMode:difficulty==='impossible',completedAt:null,score:0,kills:0,time:0,maxHp:build.maxHp,retriesLeft:0,skills:Object.keys(build.skills||{}),specials:Object.keys(build.specials||{}),weapons:build.weapons||['nutBomb'],scorePerks:build.scorePerks||{},build});changed=true;
      }
    }
    if(changed)try{localStorage.setItem('snickers3-hall-of-fame-v1',JSON.stringify(hallOfFame));}catch{}
  }
  ...
  function persistProgressionWin(){
    ...
    if(gamePlusLevel===0){
      ...
      try{localStorage.setItem('snickers3-normal-ng2-clear-v1','1');}catch{}
    }
    archiveCampaignWin(build);
    updateNGPlusMenu();
  }
  function updateNGPlusMenu(){
    const b1=$('ngPlusButton'),b2=$('ngPlus2Button'),hall=$('hallOfFameButton'),endless=$('endlessButton');
    if(b1)b1.classList.toggle('hidden',!canStartMode(1,false));
    if(b2)b2.classList.toggle('hidden',!canStartMode(2,false));
  ...
  function runDate(r){if(!r.completedAt)return 'Datum unbekannt';const date=new Date(r.completedAt);return Number.isNaN(date.getTime())?'Datum unbekannt':date.toLocaleString('de-DE',{dateStyle:'medium',timeStyle:'short'});}
  function runSummary(r,i,source){
    ...
    return `<button class="run-card" id="${source}Run${i}"><span class="run-card-medal" aria-hidden="true">♛</span><span class="run-card-main"><span class="run-card-heading"><strong>RUN ${hallOfFame.length-i}</strong><small>${tierName(hallTier(r))} ABGESCHLOSSEN</small></span><span class="run-card-date">${r.legacy?'Alter Spielstand · Datum unbekannt':escapeHTML(runDate(r))}</span><span class="run-card-score">${r.legacy?'BUILD ÜBERNOMMEN':`${displayNumber(r.score)} <small>PUNKTE</small>`}</span><span class="run-card-tags"><span>${characterRoster[b.characterId]?.name||'SNICKERS'}</span><span>${skillCount} Skills</span><span>${b.weapons.length} Waffen</span><span>${Object.keys(b.specials).length} Boss-Upgrades</span><span>${scorePerkLevels(b)} Punkte-Boni</span></span></span><span class="run-card-arrow" aria-hidden="true">↗</span></button>`;
  }
  let hallCategory='normal:0';
  function showHallOfFame(category=hallCategory){
    if(!['menu','hallDetail','hallOfFame'].includes(mode)||!hallOfFame.length&&!endlessHallOfFame.length)return;
    if(mode==='menu'&&category!=='endless'&&!hallOfFame.some(r=>`${hallDifficulty(r)}:${hallTier(r)}`===category)){const first=hallOfFame[0];category=first?`${hallDifficulty(first)}:${hallTier(first)}`:'endless';}
    hallCategory=category;mode='hallOfFame';
    const groups=[...['normal','hard','impossible'].flatMap(d=>[0,1,2].map(l=>({d,l,key:`${d}:${l}`}))),{d:'endless',l:3,key:'endless'}];
    const entries=hallOfFame.map((r,i)=>({r,i})).filter(({r})=>`${hallDifficulty(r)}:${hallTier(r)}`===category);
    const endlessCards=endlessHallOfFame.map((r,i)=>`<button class="run-card" id="endlessHallRun${i}"><span class="run-card-medal" aria-hidden="true">∞</span><span class="run-card-main"><span class="run-card-heading"><strong>ENDLOSRUN ${endlessHallOfFame.length-i}</strong><small>${hallDifficulty(r).toUpperCase()} · WELLE ${r.endlessWave}</small></span><span class="run-card-date">${escapeHTML(runDate(r))} · ${r.voluntary?'freiwillig beendet':'im Kampf gefallen'}</span><span class="run-card-score">${displayNumber(r.score)} <small>PUNKTE</small></span><span class="run-card-tags"><span>${characterRoster[hallBuild(r).characterId]?.name||'SNICKERS'}</span><span>${Object.keys(hallBuild(r).skills).length} Skills</span><span>${hallBuild(r).weapons.length} Waffen</span><span>${r.endlessCleared} Wellen überstanden</span></span></span><span class="run-card-arrow" aria-hidden="true">↗</span></button>`);
    const list=category==='endless'?endlessCards:entries.map(({r,i})=>runSummary(r,i,'hall'));
    showOverlay(`<span class="eyebrow">DIE UNSTERBLICHEN KNABBERLÄUFE</span><h2 id="overlayTitle">HALL OF FAME</h2><p>${hallOfFame.length} Kampagnen-Runs und ${endlessHallOfFame.length} Endlosläufe in zehn Kategorien. Endlos-Builds sind nur als Gegner im Hasenbein-Modus spielbar.</p><div class="hall-category-grid">${groups.map((g,i)=>`<button class="hall-category ${g.key===category?'selected':''}" id="hallCategory${i}" aria-pressed="${g.key===category}"><strong>${g.d.toUpperCase()}</strong><small>${g.key==='endless'?'ALLE SCHWIERIGKEITEN':tierName(g.l)}</small><b>${g.key==='endless'?endlessHallOfFame.length:hallOfFame.filter(r=>hallDifficulty(r)===g.d&&hallTier(r)===g.l).length}</b></button>`).join('')}</div><div class="hall-run-list">${list.length?list.join(''):'<p class="build-empty">In dieser Kategorie liegt noch kein abgeschlossener Run.</p>'}</div><button class="secondary-button" id="hallClose">ZURÜCK ZUM MENÜ</button>`);
    groups.forEach((g,i)=>{$('hallCategory'+i).onclick=()=>showHallOfFame(g.key);});
    entries.forEach(({i})=>{$('hallRun'+i).onclick=()=>showRunDetail(i,'hall');});
    if(category==='endless')endlessHallOfFame.forEach((_,i)=>{$('endlessHallRun'+i).onclick=()=>showEndlessHallDetail(i);});
  ...
  function buildCards(ids,lookup,kind){
    ...
    return `<div class="build-card-grid">${ids.map(id=>{const c=lookup(id);return `<article class="build-item ${kind}"><span class="build-item-icon" aria-hidden="true">${escapeHTML(c?.icon||'•')}</span><div><strong>${escapeHTML(c?.title||c?.name||id)}</strong>${upgradeTargetMarkup(c)}<small>${escapeHTML(c?.desc||'Aus einem früheren Run übernommen.')}</small></div></article>`;}).join('')}</div>`;
  }
  function upgradeTargetMarkup(c){
    if(!c?.requires)return '';
    const prerequisite=allSkillInfo(c.requires);
    return `<span class="upgrade-target">VERBESSERT: ${escapeHTML(prerequisite?.title||c.requires)}</span>`;
  }
  ...
  function startGame(level=0,hard=false,impossible=false,selectedCampaignBuild=null) {
    if(!canStartMode(Number(level)||0,hard,Number(level)===3?endlessSelectedBuild:selectedCampaignBuild,impossible))return;
    ...
    const carryBuild=endlessMode?endlessSelectedBuild:gamePlusLevel>0?(selectedCampaignBuild||campaignSources(gamePlusLevel,hardMode,impossibleMode).map(({r})=>hallBuild(r))[0]||legacyCampaignBuild(gamePlusLevel,hardMode,impossibleMode)):null;
    if((gamePlusLevel===1||gamePlusLevel===2)&&!carryBuild)return;
    endlessBuildTier=endlessMode&&carryBuild?clamp(carryBuild.sourceTier??2,0,2):0;
    player = createPlayer();
    ...
    preparePlayableCharacter(player,selectedCharacter);
    ...
    const intro=endlessMode?`<span class="eyebrow ngplus-kicker">ENDLOSMODUS · KEIN FEIERABEND</span><h2 id="overlayTitle">WELLEN BIS DER GARTEN AUFGIBT.</h2><p>${endlessFromHall?'Du startest mit einem übernommenen '+tierName(endlessBuildTier)+'-Build. Die Gegner passen sich seiner Ausgangsstufe an.':'Du startest mit einem frischen Build auf Normal-Niveau.'} Neue Gegnertypen werden stufenweise freigeschaltet; jede Welle wird härter und jede fünfte Welle endet mit einem Boss.</p><blockquote class="snickers-quote"><b>SNICKERS</b>„Unendlich ist auch nur sehr oft neun.“</blockquote><button class="primary-button ngplus-button" id="introButton">ENDLOSMODUS STARTEN <span>↗</span></button>`:gamePlusLevel===2?'<span class="eyebrow ngplus-kicker">NEW GAME+2 · JETZT WIRD ES UNVERNÜNFTIG</span><h2 id="overlayTitle">RUNDE DREI. ENDE DER NAHRUNGSKETTE.</h2><p>Dein gewählter NG+-Build bleibt. Die Gegner sind gegenüber NG+ nochmals härter, aber die Skalierung bleibt abgestuft: mehr Leben, höherer Druck, stärkere Treffer und mehr Elite-Gegner.</p><blockquote class="snickers-quote"><b>SNICKERS</b>„Danach ist Schluss. Entweder mit den Hasen oder mit dem Garten.“</blockquote><button class="primary-button ngplus-button" id="introButton">NEW GAME+2 STARTEN <span>↗</span></button>':gamePlusLevel===1?'<span class="eyebrow ngplus-kicker">NEW GAME+ · DIE HASEN HABEN NICHT GELERNT</span><h2 id="overlayTitle">RUNDE ZWEI. KEINE AUSREDEN.</h2><p>Snickers behält den gewählten Normal-Build. Dafür steigen Gegnerdichte, Leben, Tempo und Angriffsdruck kontrolliert an; neue Elite-Verstärkung kommt dazu.</p><blockquote class="snickers-quote"><b>SNICKERS</b>„Ihr wolltet mehr? Ich habe meine Upgrades mitgebracht.“</blockquote><button class="primary-button ngplus-button" id="introButton">NEW GAME+ STARTEN <span>↗</span></button>':'<span class="eyebrow">VORSPANN · DIE ERSTE NUSS</span><h2 id="overlayTitle">DER FELLIGE FRIEDEN IST VORBEI.</h2><p>Snickers putzt gerade gemütlich sein Fell. Da schnappt sich ein frecher Hase seine Nuss und hoppelt davon. Snickers wird wütend, schnappt sich den Blaster und geht auf Hasenjagd.</p><blockquote class="snickers-quote"><b>SNICKERS</b>„Meine Nuss. Meine Regeln.“</blockquote><button class="primary-button" id="introButton">HASENJAGD STARTEN <span>↗</span></button>';
    showOverlay(impossibleMode?`<span class="eyebrow impossible-kicker">IMPOSSIBLE · ${endlessMode?'ENDLOSMODUS':gamePlusLevel===2?'NEW GAME+2':gamePlusLevel===1?'NEW GAME+':'NORMAL'}</span><h2 id="overlayTitle">${endlessMode?'TOLLWÜTIGE HASENJAGD DES GRAUENS.':'DIE FÄULNIS HAT ZÄHNE.'}</h2><p>${endlessMode?'Jede fünfte Welle ein Boss, mehr verdorbene Gegner und Angriffsmuster. Welle 200 bringt Herbert Grönemeyer.':'Neue verdorbene Gegner, zwei zusätzliche Phasen je Hauptboss und mehr Schaden.'} ${gamePlusLevel===2?'Ottah und Pafti kämpfen am Ende gemeinsam.':''}</p><button class="primary-button" id="introButton">IMPOSSIBLE STARTEN ↗</button>`:intro);$('introButton').onclick=beginFirstWave;beginRunSession(carryBuild);
  }
  ...
  const powerBook={
    ...
    splinterStorm:{name:'SPLITTERSTURM',icon:'✴',duration:8,desc:'Für 8 s springt jeder normale Nusstreffer einmal auf ein anderes Ziel in 180 Reichweite über und verursacht 35 % seines Projektilschadens. Ohne zweites Ziel kein Zusatzschaden.',apply:p=>p.powerups.splinterStorm=8},
    carrotFlak:{name:'MÖHRENFLAK',icon:'✺',duration:8,desc:'Für 8 s zerstört die Flak höchstens alle 1,6 s ein feindliches Geschoss innerhalb von 95 Einheiten. Sie schützt nicht vor Kontakt und Flächenangriffen.',apply:p=>{p.powerups.carrotFlak=8;p.carrotFlakCd=0;}}
  };
  ...
  function spawnEnemy(type) {
    ...
    const enemyHpMult=endlessMode?endlessCarryFactor()*endlessHpScale(wave):campaignHpScale();
    const enemySpeedMult=endlessMode?(endlessFromHall&&endlessBuildTier===2?1.08:1)*endlessSpeedScale(wave):campaignSpeedScale();
    const actionRate=endlessMode?(endlessFromHall&&endlessBuildTier===2?1.08:1)*endlessActionScale(wave):campaignActionScale();
  ...
  function spawnOttah(){
    ...
    addAchievementProgress('ng2_ottah');announce('NEUE STAGE · '+getStageName(stageVisual),'OTTAH');tone(58,.9,'sawtooth',.1,30);updateHud();
  }
  ...
  const endlessBossBook={
    ...
    herbert:{name:'HERBERT GRÖNEMEYER',subtitle:'DIE ULTIMATIVE ZUGABE',hp:1450,speed:88,color:'#d7a880',minWave:Infinity}
  };
  function spawnEndlessBoss(fromRetry=false,forcedKind=null){
    const keys=Object.keys(endlessBossBook).filter(k=>(endlessBossBook[k].minWave||0)<=wave),kind=forcedKind||(wave===199?'herbert':keys[Math.floor(Math.random()*keys.length)]),cfg=endlessBossBook[kind];
    const waveScale=endlessBossHpScale(wave),buildScale=endlessFromHall?endlessBuildTier===2?3.8:endlessBuildTier===1?2.2:1.35:1,hp=cfg.hp*waveScale*buildScale*1.07*1.05;
    boss={x:W/2,y:175,type:'boss',endlessBoss:true,endlessKind:kind,endlessName:cfg.name,endlessSubtitle:cfg.subtitle,herbert:kind==='herbert',herbertPhase:1,r:kind==='herbert'?73:kind==='gnomeOverlord'||kind==='quarryBoar'?68:55,hp,maxHp:hp,speed:cfg.speed*(endlessFromHall?1.08:1)*endlessSpeedScale(wave),phase:0,hit:0,actionCd:1.15,attack:0,summonCd:5.8,enraged:false,dead:false,points:5000+wave*900,slow:0,windup:0,charge:0,aim:0,burstLeft:0,burstCd:0,supplyStep:0,supplyThresholds:[.76,.48,.24]};
  ...
  const regularEndlessBossUpdate=updateEndlessBoss;
  updateEndlessBoss=function(dt){
    const e=boss;if(!e?.herbert)return regularEndlessBossUpdate(dt);
    e.phase+=dt*5;e.hit=Math.max(0,e.hit-dt);e.slow=Math.max(0,(e.slow||0)-dt);
    const aim=Math.atan2(player.y-e.y,player.x-e.x),distance=dist(player,e),rate=(hardMode?1.12:1)*(impossibleMode?1.13:1);
    if(e.windup>0){e.windup-=dt;if(e.windup<=0){
      const pat=e.pendingPattern;
      if(pat===0){for(let ring=0;ring<2;ring++)for(let i=-5;i<=5;i++)enemyShot(e.x,e.y,e.aim+i*.12+ring*.035,240+ring*85+e.herbertPhase*18,true);}
      else if(pat===1){const n=20+e.herbertPhase*4;for(let i=0;i<n;i++){const dir=i*TAU/n;if(Math.abs(angleDelta(dir,e.aim))>.4)enemyShot(e.x,e.y,dir,220+e.herbertPhase*23,true);}}
      else if(pat===2){for(let i=-2;i<=2;i++)hazards.push({type:'shock',x:clamp(e.targetX+i*130,65,W-65),y:clamp(e.targetY+(i%2)*90,145,H-65),r:70,wait:.7+Math.abs(i)*.11,life:.3,hit:false,damage:2});}
      else {e.charge=.52;for(let i=0;i<12;i++)enemyShot(e.x,e.y,TAU*i/12,235,true);}
    }}else if(e.charge>0){e.charge-=dt;e.x+=Math.cos(e.aim)*540*dt;e.y+=Math.sin(e.aim)*540*dt;}
    else {if(distance>205){e.x+=Math.cos(aim)*e.speed*(e.slow>0?.78:1)*dt;e.y+=Math.sin(aim)*e.speed*(e.slow>0?.78:1)*dt;}e.actionCd-=dt*rate;if(e.actionCd<=0){e.pendingPattern=e.attack++%4;e.aim=aim;e.targetX=player.x;e.targetY=player.y;e.windup=e.pendingPattern===3?1.05:.92;e.actionCd=Math.max(1.5,2.3-e.herbertPhase*.22);}}
    e.x=clamp(e.x,80,W-80);e.y=clamp(e.y,155,H-70);
    e.summonCd-=dt;if(e.summonCd<=0&&enemies.length<12){for(let i=0;i<Math.min(4,e.herbertPhase+1);i++)spawnEnemy(impossibleMode?['rotWasp','plagueSinger','bileBoar'][i%3]:hardMode?['platedHare','ashThrower'][i%2]:['runner','pigRammer'][i%2]);e.summonCd=8.5;}
    if(dist(player,e)<player.r+e.r)hurtPlayer(2,e);
  };
  ...
  const skillBook = {
    ...
    bombs:{icon:'◎',title:'NUSS-ESKALATION',short:'Bombenvorrat',desc:'3 zusätzliche Ladungen deiner persönlichen Startwaffe sofort.',apply:p=>{const id=characterRoster[p.characterId]?.starter||'nutBomb';p.weaponUses[id]=(p.weaponUses[id]||0)+3;}},
  ...
  function availableRewards(includeSpecial=false){
    ...
    if(newGamePlus||(endlessMode&&((endlessFromHall&&endlessBuildTier>=1)||wave>=5)))books.push(ngPlusSkillBook,ngPlusWeaponUpgrades);
    if(gamePlusLevel===2||(endlessMode&&((endlessFromHall&&endlessBuildTier>=2)||wave>=13)))books.push(ngPlus2SkillBook,ngPlus2WeaponUpgrades);
  ...
  function updateSkills(){
    const ids=Object.keys(player.skills);$('skillHud').innerHTML=ids.map(id=>{const info=skillBook[id]||legendarySkillBook[id]||ngPlusSkillBook[id]||ngPlus2SkillBook[id]||survivalSkillBook[id];return info?`<span class="${info.legendary?'legendary-skill-chip':''}" title="${escapeHTML(info.title+(info.requires?' · Verbessert: '+(allSkillInfo(info.requires)?.title||info.requires):''))}">${info.icon}<span>${info.short}</span>${player.skills[id]>1?` ×${player.skills[id]}`:''}</span>`:'';}).join('');
  ...
  function startWave(next,keepRetries=false){
    ...
    announce(endlessMode?`ENDLOS-WELLE ${String(wave+1).padStart(2,'0')}`:`WELLE ${String(wave+1).padStart(2,'0')} / 09`,endlessMode?getStageName(stageVisual)+' · ENDLOSMODUS':getStageName(stageVisual)+' · '+waveNames[wave]);
  ...
  function bossUpgradeScreen(kind){
    ...
    choices.forEach((c,i)=>{$(`specialUpgrade${i}`).onclick=()=>{if(mode!=='bossUpgrade')return;grantReward(c);player.weaponUses[player.weapons[0]]=maxWeaponAmmo(player.weapons[0]);tone(620,.25,'triangle',.08,1240);afterBoss();};});
  }
  ...
  function continueAfterWave(){
    player.waveShuffle=0;player.weaponUses[player.weapons[0]]=maxWeaponAmmo(player.weapons[0]);player.invuln=2;mode='playing';hideOverlay();
  ...
  function darkEnding(level,impossible){
    const hard=[
      ['BLUTIGER FRIEDEN.','Snickers lässt die rote Hasenjagd hinter sich. Der Garten ist verwüstet, aber die letzte Nuss ist gerettet. Selbst Karnil schweigt, als Snickers einen neuen Setzling in den Boden drückt.','DER GARTEN ATMET WIEDER.'],
      ['ZWEITE CHANCE, ROTE SPUREN.','Hasenbein und Karnil kamen stärker zurück. Snickers überlebt ihre blutige zweite Jagd und hängt an die Ruine ein Schild: „Bissige Hasen bitte draußen bleiben.“','DIE JAGD IST VORBEI. VORERST.'],
      ['DER LETZTE ROTE MORGEN.','Auch Ottah ist gefallen. Snickers steht im Morgengrauen zwischen den Blutlachen, teilt seine Sahne mit dem erschöpften Warzenschwein und erklärt die Hasenjagd für beendet.','SAHNE NACH DER SCHLACHT.']
    ];
    const nightmare=[
      ['NACH DER SEUCHE.','Die verdorbene Arena verstummt. Snickers hat die tollwütige Jagd überlebt, verschließt den Eingang zur Ruine und trägt die letzte unversehrte Nuss ans Tageslicht.','EIN STÜCK HIMMEL ÜBER DER RUINE.'],
      ['KEIN HASE STEHT MEHR AUF.','Ein zweites Mal hat Snickers die unmögliche Jagd überstanden. Er pflanzt mitten im Moder eine Eiche. Ausgerechnet Karnil muss das erste Gießkännchen holen.','LEBEN IM VERDORBENEN LAND.'],
      ['DER GEFÄHRLICHSTE HAMSTER.','Ottah und Pafti kämpften gemeinsam und verloren gemeinsam. Snickers verlässt die zerfallene Arena mit der Nuss und seinem Orden. Hinter ihm wächst die erste grüne Knospe aus den Trümmern.','OTTAH + PAFTI BESIEGT · DER ORDEN GEHÖRT DIR.']
    ];
    const [headline,copy,caption]=(impossible?nightmare:hard)[clamp(level,0,2)];
    const bg=impossible?'#191820':'#331c20',soil=impossible?'#252632':'#642b2a',blood=impossible?'#822939':'#b5403d';
    const fallen=impossible?'<path d="M50 173l55-61 39 61m365 4l39-72 47 72" fill="#39333d" stroke="#66505b" stroke-width="5"/><path d="M102 108v-32l24-12 16 17v36M511 100V60l28-17 15 15v48" fill="none" stroke="#615359" stroke-width="8"/>':'<path d="M28 175V98m0 17l-18-24m18 18l27-35m555 104V78m0 28l-24-28m24 45l26-26" stroke="#684e47" stroke-width="12" fill="none"/>';
    const enemies=level===2?`<g opacity=".85"><circle cx="493" cy="143" r="20" fill="#7b5c63"/><circle cx="546" cy="137" r="18" fill="#74636f"/><path d="M480 124l-7-33 15 18m62 8l12-24-3 31" fill="#bcb0a1"/><circle cx="487" cy="139" r="3" fill="#ff8a80"/><circle cx="541" cy="134" r="3" fill="#f6a1b6"/></g>`:`<g opacity=".8"><path d="M505 146l-18-47 16 18 15-27 20 54" fill="#715c66"/><circle cx="510" cy="133" r="4" fill="#fa7374"/></g>`;
    const badge=impossible&&level===2?'<circle cx="347" cy="66" r="24" fill="#e8c06d" stroke="#fff1af" stroke-width="4"/><text x="347" y="74" fill="#4a3044" font-size="24" text-anchor="middle">♛</text>':'';
    return {headline,copy,art:`<div class="ending-gag-art" aria-label="${impossible?'Verdorbene Ruine mit Snickers und den besiegten Bossen':'Blutiger Garten mit Snickers nach der Jagd'}"><svg viewBox="0 0 640 230" role="img" aria-label="${headline}"><rect width="640" height="230" rx="18" fill="${bg}"/><circle cx="325" cy="72" r="42" fill="${impossible?'#b9b294':'#e7a791'}" opacity=".72"/>${fallen}<path d="M0 176Q125 158 234 177T470 179T640 170V230H0Z" fill="${soil}"/><ellipse cx="170" cy="193" rx="82" ry="12" fill="${blood}" opacity=".8"/><ellipse cx="472" cy="188" rx="68" ry="9" fill="${blood}" opacity=".7"/><path d="M345 182v-28m0 12l-11-12m11 7l13-13" stroke="#83b76f" stroke-width="4" fill="none"/><ellipse cx="345" cy="148" rx="8" ry="4" fill="#9cce7e"/>${enemies}<g transform="translate(250 141)"><path d="M-25 34l-12 32m60-31l12 30" stroke="#bb8b57" stroke-width="10"/><ellipse cy="16" rx="31" ry="30" fill="#c89458"/><path d="M-21 -9L-27 -39 0 -22 27 -39 21 -9" fill="#bc874f"/><ellipse cy="-12" rx="26" ry="23" fill="#dbab6e"/><circle cx="-10" cy="-17" r="3" fill="#302a2b"/><circle cx="10" cy="-17" r="3" fill="#302a2b"/><ellipse cy="-5" rx="5" ry="3" fill="#774e46"/><circle cx="31" cy="19" r="11" fill="#e9c47b"/></g>${badge}<text x="320" y="216" fill="#f6e5d5" font-size="13" font-family="Arial" text-anchor="middle">${caption}</text></svg></div><blockquote class="snickers-quote"><b>ABSPANN · ${impossible?'IMPOSSIBLE':'HARD'} · ${level===0?'NORMAL':level===1?'NEW GAME+':'NEW GAME+2'}</b>${impossible?'Sogar in dieser Ruine kann wieder etwas wachsen.':'Morgen gießt Snickers die Eiche. Heute schläft er erst einmal.'}</blockquote>`};
  }
  function finish(won) {
    ...
    if(won){
      score+=Math.round((5000+player.hp*250+Math.max(0,Math.round((620-runTime)*15)))*(impossibleMode?1.75:hardMode?1.55:1));setAchievementProgress('wave_runner',9);
      if(lastDefeatedBoss==='karnil'||lastDefeatedBoss==='ottah')addAchievementProgress('housepig');
      if(retriesLeft>0)addAchievementProgress('nutty_survivor');if(hardMode&&!impossibleMode&&retriesLeft>=2)addAchievementProgress('hard_spare_retries');if(hardMode&&!impossibleMode&&gamePlusLevel===2&&!(player.retriesSpent||0))addAchievementProgress('hard_ng2_perfect_retries');if(gamePlusLevel===1)addAchievementProgress('ngplus_clear');
    ...
    else $('retryButton').onclick=()=>{if(endlessMode){const hard=hardMode,impossible=impossibleMode;goMenu();setTimeout(()=>showEndlessSelect(hard,impossible),0);}else chooseRunMode(0);};
  ...
  function retryCurrentWave(){
    ...
    retriesLeft--;player.retriesSpent=(player.retriesSpent||0)+1;addAchievementProgress('retry_hero');addAchievementProgress('three_strikes');
  ...
  function tryDropSpecialAmmo(x,y,life){
    ...
    // Twice as many half refills preserve the value of ammunition perks and boss supplies.
    const chance=2*(.026+.018*progress+.008*Math.max(0,Math.ceil(remaining/2)-1))*(player.ammoDropLuck||1);
    const force=player.ammoDryKills>=Math.max(5,Math.ceil((16-remaining)/2));
  ...
  function damageEnemy(e, amount, blast = false) {
    ...
    if (isBoss(e)) {
      ...
      if(e.endlessBoss){lastDefeatedBoss='endless';endlessBossesDefeated++;setAchievementProgress('endless_bosses',endlessBossesDefeated);const survived=wave+1;recordEndlessBest(survived);for(const n of [75,100,150,200])if(survived>=n)addAchievementProgress('endless_'+n);if(impossibleMode)for(const n of [10,50,100])if(survived>=n)addAchievementProgress('impossible_endless_'+n);if(hardMode&&survived>=30&&retriesLeft===3)addAchievementProgress('hard_endless_30');if(e.herbert)for(const d of ['normal',...(hardMode?['hard']:[]),...(impossibleMode?['impossible']:[])])addAchievementProgress('herbert_'+d);beginVictory(()=>bossUpgradeScreen('endless'));return;}
  ...
  function fire() {
    ...
    for (const shot of shots) {
      ...
      bullets.push({x:player.x+Math.cos(a)*22,y:player.y+Math.sin(a)*22,vx:Math.cos(a)*755,vy:Math.sin(a)*755,life:1.2,damage:commonDamage*shot.mul*(crit?2:1),critShot:Boolean(crit),normalNut:true,pierce:player.pierce+(hunnaShot?2:0),hitIds:new Set(),r:player.powerups.giantNut>0?10:6,a,chain:canChain,ricochet:player.ricochet,frost:player.frost});
    }
  ...
  function updateSkillsInPlay(dt){
    ...
    player.pockenCd=Math.max(0,(player.pockenCd||0)-dt);
    if(player.powerups.carrotFlak>0){
      player.carrotFlakCd=Math.max(0,(player.carrotFlakCd||0)-dt);
      if(player.carrotFlakCd<=0){const incoming=enemyBullets.find(b=>b.life>0&&dist(player,b)<95);
        if(incoming){incoming.life=0;player.carrotFlakCd=1.6;burst(incoming.x,incoming.y,'#f4b65d',10,80);}}
    }
    ...
    if(player.orbit&&player.orbitCd<=0){
      ...
      const count=evolved('orbit',3)?3:2;
      for(let i=0;i<count;i++){
        const a=runTime*(evolved('orbit',1)?4.1:3.5)+i*TAU/count,orb={x:player.x+Math.cos(a)*62,y:player.y+Math.sin(a)*62};
    ...
    if(player.lominarWorm){
      player.lominarX+=(player.lominarDir||1)*(evolved('lominarWorm',1)?448:390)*dt;player.lominarY+=Math.sin(runTime*3.2)*34*dt;
  ...
  function update(dt) {
    ...
    if (!boss && waveTime < currentWaveLength()) {
      ...
      if(spawnTimer<=0){
        if(endlessMode){
          const pool=endlessEnemyPool(wave),density=endlessDensityScale(wave)*(endlessFromHall?endlessBuildTier===2?1.18:endlessBuildTier===1?1.1:1.03:1);
          const baseCap=38+wave*1.3+(endlessFromHall?endlessBuildTier===2?12:endlessBuildTier===1?7:3:0),cap=Math.min(78,Math.round(baseCap));
    ...
    for(const e of enemies){
      ...
      else if(e.knoll)updateKnollVulture(e,dt);
    ...
    for(const b of bullets){
      ...
      for(const e of all){
        ...
        if(Math.hypot(oldX+dx*t-e.x,oldY+dy*t-e.y)<e.r+b.r){
          b.hitIds.add(e);if(b.narrath)e.narrathHit=true;if(b.frost)e.slow=1.5;
          let impact=b.damage;
          if(player.duden&&b.normalNut){
            const spot=dudenSpot(e),target={x:e.x+spot.x,y:e.y+spot.y},radius=isBoss(e)?11:Math.max(6,e.r*.32);
            if(segmentDistance(target,{x:oldX,y:oldY},b)<=radius){
              const targetChance=isBoss(e)?.68:.78,extra=clamp((targetChance-(player.crit||0))/Math.max(.01,1-(player.crit||0)),0,1);
              if(b.critShot||Math.random()<extra){if(!b.critShot)impact*=2;floater(target.x,target.y-14,'DUDEN-KRIT!','#f8d17a');}
              e.dudenSpot=null;
            }
          }
          if(b.ronnyBullet&&player.skills.ronnyUp6&&(isBoss(e)||e.miniBoss))impact*=1.15;
          damageEnemy(e,impact*((b.auto||b.narrath||b.ping)&&player.professorCouncil?1.18:1));
          if(b.slimePaw){e.slow=Math.max(e.slow||0,isBoss(e)?.6:2);player.slimeHits=(player.slimeHits||0)+1;
            if(player.slimeHits%6===0){const other=combatTargets().find(t=>t!==e&&!t.dead&&dist(t,e)<160);if(other){other.slow=Math.max(other.slow||0,1);damageEnemy(other,.75*Math.sqrt(player.damage));}}}
          if(b.characterAttack==='raffzahn'&&player.shotCount%8===0&&!isBoss(e))infectEnemy(e,2);
          if(b.characterAttack==='krustenbraten'&&player.shotCount%4===0){const other=combatTargets().find(t=>t!==e&&!t.dead&&dist(t,e)<60);if(other)damageEnemy(other,b.damage*.30,true);}
          if(b.characterAttack==='koettitroeter'&&!isBoss(e)&&!e.miniBoss){e.x=clamp(e.x+Math.cos(b.a)*9,35,W-35);e.y=clamp(e.y+Math.sin(b.a)*9,110,H-40);}
          if(b.ricochet&&player.ricochet){b.ricochet=false;chainHit(e,b.damage*.55);}
          if(mode!=='playing')return;
          if(b.ronnyLongFrost)e.slow=Math.max(e.slow||0,isBoss(e)||e.miniBoss?1.7:2.1);
          if(b.ronnySplash){b.ronnySplash=false;for(const other of combatTargets().filter(other=>other!==e&&!other.dead&&dist(other,e)<75+other.r).slice(0,2)){damageEnemy(other,.75*Math.sqrt(player.damage||1),true);if(mode!=='playing')return;}burst(e.x,e.y,'#dec6ac',7,70);}
          if(b.normalNut&&player.powerups.splinterStorm>0&&!b.splintered){
            b.splintered=true;const other=combatTargets().filter(t=>t!==e&&!t.dead&&dist(t,e)<180).sort((a,c)=>dist(a,e)-dist(c,e))[0];
            if(other){particles.push({type:'beam',x:e.x,y:e.y,x2:other.x,y2:other.y,life:.16,maxLife:.16,color:'#f8c584'});damageEnemy(other,b.damage*.35);if(mode!=='playing')return;}
          }
          if(b.normalNut&&player.pockenstrombose){
            player.pockenHits=(player.pockenHits||0)+1;
            if(player.pockenHits%7===0&&(player.pockenCd||0)<=0){player.pockenCd=2.4;
              const targets=combatTargets().filter(t=>t!==e&&!t.dead&&dist(t,e)<200).sort((a,c)=>dist(a,e)-dist(c,e)).slice(0,2);
              if(!targets.length&&isBoss(e)&&!e.dead)targets.push(e);
              for(const t of targets){particles.push({type:'beam',x:e.x,y:e.y,x2:t.x,y2:t.y,life:.19,maxLife:.19,color:'#c9abf4'});if(!isBoss(t))t.slow=Math.max(t.slow||0,.7);damageEnemy(t,(t===e?.8:1.6)*Math.sqrt(player.damage||1));if(mode!=='playing')return;}
            }
          }
  ...
  Object.assign(skillBook.bombs,{desc:'Deine persönliche Startwaffe erhält dauerhaft +2 maximale Ladungen und wird sofort gefüllt.',apply:p=>{p.bombBonus=(p.bombBonus||0)+2;const id=characterRoster[p.characterId]?.starter||'nutBomb';p.weaponUses[id]=weaponBook[id].max+p.ammoBonus+p.bombBonus;}});
  ...
  function isRabbit(e){if(e.knoll)return false;if(e.miniKind)return ['ramCaptain','frostWarden'].includes(e.miniKind);if(e.troll)return false;if(extraEnemyBook[e.type])return extraEnemyBook[e.type].species==='rabbit';return isBoss(e)?e.endlessBoss?['general','cyber'].includes(e.endlessKind):!e.karnil&&!e.ottah:!e.type.startsWith('pig')&&e.type!=='gnomePig';}
  ...
  function spawnWaveMini(kind){
    const cfg=miniBossBook[kind],p=safeSpawnPoint(),scale=endlessMode?endlessCarryFactor()*endlessHpScale(wave):campaignHpScale();
  ...
  function prepareWaveReward(){
    ...
    if(endlessMode){const survived=wave+1,cleared=survived%5===0?survived-1:survived;recordEndlessBest(cleared);}
    if(hardMode&&!impossibleMode&&!endlessMode&&wave===8&&player.waveHits===0)addAchievementProgress('hard_last_clean');
    if(impossibleMode&&wave===8&&!(player.retriesSpent||0))addAchievementProgress('impossible_wave9_clean');
  ...
  function beginVictory(next){
    if(mode!=='playing')return;
    // The fanfare freezes the arena. Collect everything before callbacks can discard boss drops.
    const swept=pickups.length;
    for(const p of pickups){
      if(p.type==='nut')score+=Math.round(20*(player.powerups.jackpot>0?4:1)*(player.powerups.scoreRush>0?2:1)*runScoreMultiplier());
      else if(p.type==='heart')player.hp=Math.min(player.maxHp,player.hp+1);
      else if(p.type==='power')collectPowerup(p);
    ...
    pickups=[];if(swept){floater(player.x,player.y-42,`${swept} DROPS EINGESAMMELT`,accent());setAchievementProgress('score_hog',score);if(swept>=6)addAchievementProgress('pille_sweep');}
    if(boss?.dead&&!endlessMode){
      const kind=lastDefeatedBoss||'boss';player.retryBossRewards??={};
      if(!player.retryBossRewards[kind]){player.retryBossRewards[kind]=true;if(retriesLeft===0)addAchievementProgress('pille_last_retry');retriesLeft=Math.min(3,retriesLeft+1);floater(player.x,player.y-63,retriesLeft===3?'BOSS-BONUS · RETRIES VOLL':'BOSS-BONUS · +1 RETRY','#ffe0a2');}
    }
    updateHud();mode='victory';resetInput();victoryTime=2.1;victoryAfter=next;enemyBullets=[];hazards=[];bullets=[];ratSwarms=[];thrownWeapons=[];player.moving=false;player.dashTime=0;
  ...
  const achievementDescriptions={
    ...
    power_mix:'Sammle 5 verschiedene Power-up-Arten in einem Run.',power_encyclopedia:'Sammle über alle Runs hinweg alle 16 Power-up-Arten.',
  ...
  function drawNewBoss(e){
    ...
    if(k==='ratEmpress'){
      ...
      path(c,[[-40,47],[-29,-18],[0,-33],[31,-18],[41,48]],'#302936');
      ellipse(c,0,-33,28,35,col);ellipse(c,-20,-43,6,8,'#2b2533');ellipse(c,17,-43,6,8,'#2b2533');
      path(c,[[-27,-59],[-16,-82],[4,-76],[25,-65],[28,-53]],'#665449');
      ellipse(c,0,-20,13,5,'#563746');c.fillStyle='#d9bf8f';c.fillRect(19,-13,5,54);ellipse(c,21,-19,9,13,'#e5cfaa');
      c.strokeStyle='#f6db9e';c.lineWidth=3;for(let i=0;i<e.herbertPhase;i++){c.beginPath();c.arc(0,0,65+i*9,Math.PI*.15+i*.3,Math.PI*.82+i*.2);c.stroke();}
    }else{
  ...
  function drawRabbit(e){
    ...
    if(e.endlessBoss&&['ratEmpress','sugarGolem','starOwl','herbert'].includes(e.endlessKind)){drawNewBoss(e);return;}
    ...
    const bob=Math.sin(e.phase)*2.5,face=e.hasenbeinControlled?(e.face||1):player&&player.x<e.x?-1:1;
  ...
  function render(dt){
    ...
    if(player.orbit){
      ...
      const count=evolved('orbit',3)?3:2;for(let i=0;i<count;i++){const a=runTime*(evolved('orbit',1)?4.1:3.5)+i*TAU/count,x=player.x+Math.cos(a)*62,y=player.y+Math.sin(a)*62;ellipse(ctx,x,y,9,6,accent(),a);ellipse(ctx,x-2,y-1,3,3,'#fff5cb');}ctx.restore();
    }
    ...
    for(const b of bullets){
      if(b.characterAttack||b.carrotShot){ctx.save();ctx.translate(b.x,b.y);ctx.rotate(b.a);
        const type=b.characterAttack;
        if(type==='raffzahn'){ctx.strokeStyle='#f3d5d3';ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,0,48,-1.2,1.2);ctx.stroke();}
        else if(type==='krustenbraten')ellipse(ctx,0,0,72,24,'#d88f6077');
        else if(type==='slanny'||b.carrotShot){ctx.rotate(b.returnAge!==undefined?ambientTime*12:0);path(ctx,[[15,0],[-11,-6],[-5,0],[-11,6]],'#eea256');ellipse(ctx,-10,0,3,3,'#80aa6b');}
        else {ctx.strokeStyle='#d7edb0';ctx.lineWidth=7;ctx.beginPath();ctx.arc(0,0,b.r,-1.2,1.2);ctx.stroke();}
        ctx.restore();continue;
      }
      if(b.ronnyBullet){ctx.save();ctx.translate(b.x,b.y);ctx.rotate(b.a);ellipse(ctx,0,0,b.r,b.r*.72,b.ronnyColor||'#ddb984');ellipse(ctx,-b.r*.2,-b.r*.2,2,2,'#fff4d6');ctx.restore();continue;}
  ...
  Object.assign(skillBook,{
    ...
    wollenkamps:passive('FRAU WOLLENKAMPS','☂','Frau Wollenkamps eilt zu Gegnern und diskutiert übers Wetter. Etwa alle 6,1 s stoppt sie einen normalen Gegner 1,95 s; Bosse werden 1,55 s verlangsamt.','wollenkamps')
  });
  Object.assign(ngPlusSkillBook,{
  ...
  let hardProgress={unlocked:false,ngplus:false,ngplus2:false,endless:false,ngBuild:null,ng2Build:null},endlessSelectHard=false,endlessSelectImpossible=false;
  ...
  let endlessBest={normal:0,hard:0,impossible:0};try{Object.assign(endlessBest,JSON.parse(localStorage.getItem('snickers3-endless-best-v1')||'{}'));}catch{}
  let endlessHistory=[];try{const h=JSON.parse(localStorage.getItem('snickers3-endless-history-v1')||'[]');if(Array.isArray(h))endlessHistory=h.slice(0,40);}catch{}
  function recordEndlessBest(survived){
    if(!endlessMode)return;player.endlessCleared=Math.max(player.endlessCleared||0,survived);
    for(const key of ['normal',...(hardMode?['hard']:[]),...(impossibleMode?['impossible']:[])])endlessBest[key]=Math.max(endlessBest[key]||0,survived);
    try{localStorage.setItem('snickers3-endless-best-v1',JSON.stringify(endlessBest));}catch{}
  }
  function confirmEndlessFinish(){
    if(mode!=='paused'||!endlessMode)return;
    mode='endlessFinishConfirm';const last=Math.max(0,player.endlessCleared||0);
    showOverlay(`<span class="eyebrow">ENDLOSMODUS · FREIWILLIGES ENDE</span><h2 id="overlayTitle">BIS HIERHER UND NICHT WEITER?</h2><p>Dein Lauf wird mit <strong>${last} vollständig überstandenen Wellen</strong> als abgeschlossen gespeichert. Ein begonnener Boss zählt erst nach dem Sieg. Dein Kampagnen-Run bleibt gespeichert.</p><div class="overlay-actions"><button class="primary-button" id="confirmEndlessEnd">RUN ABSCHLIESSEN ↗</button><button class="secondary-button" id="cancelEndlessEnd">WEITERSPIELEN</button></div>`);
    $('confirmEndlessEnd').onclick=()=>{
      if(mode!=='endlessFinishConfirm')return;
      const difficulty=impossibleMode?'impossible':hardMode?'hard':'normal';recordEndlessBest(last);
      archiveEndlessRun(true);
      endlessHistory.unshift({difficulty,wave:last,score,kills,completedAt:new Date().toISOString(),voluntary:true});endlessHistory=endlessHistory.slice(0,40);
      try{localStorage.setItem('snickers3-endless-history-v1',JSON.stringify(endlessHistory));}catch{toast('Abgeschlossener Lauf konnte nicht in der Historie gespeichert werden.');}
      mode='won';endRunSession();updateNGPlusMenu();
      showOverlay(`<span class="eyebrow">ENDLOSMODUS ABGESCHLOSSEN</span><h2 id="overlayTitle">SNICKERS GEHT FEIERABEND MACHEN.</h2><p>${last} Wellen vollständig überstanden · ${displayNumber(score)} Punkte · ${difficulty.toUpperCase()}. Dein Kampagnen-Run ist weiterhin verfügbar.</p><button class="primary-button" id="endlessDone">HAUPTMENÜ ↗</button>`);
      $('endlessDone').onclick=goMenu;
    };
    $('cancelEndlessEnd').onclick=()=>{mode='playing';pauseGame();};
  }
  ...
  const hardUnlocked=()=>Boolean(hardProgress.unlocked||hallOfFame.some(r=>hallDifficulty(r)==='normal'&&hallTier(r)===2));
  ...
  function campaignSources(level,hard=false,impossible=false){
    const difficulty=impossible?'impossible':hard?'hard':'normal';
    return hallOfFame.map((r,i)=>({r,i})).filter(({r})=>hallDifficulty(r)===difficulty&&hallTier(r)===level-1);
  }
  function legacyCampaignBuild(level,hard=false,impossible=false){
    const source=impossible?impossibleProgress:hard?hardProgress:null;
    const remembered=level===1?(source?source.ngBuild:ngPlusBuild):(source?source.ng2Build:ngPlus2Build);
    if(remembered&&Boolean(remembered.hardMode)===Boolean(hard)&&Boolean(remembered.impossibleMode)===Boolean(impossible))return {...remembered,legacyDerived:true};
    const record=hallOfFame.find(r=>hallTier(r)===2&&hallDifficulty(r)===(impossible?'impossible':hard?'hard':'normal'));
    if(!record)return null;
    // Older versions discarded Normal/NG+ builds after NG+2. Rebuild a legal
    // stage-limited fallback without pretending that its original run survived.
    const original=hallBuild(record),books=level===1?[skillBook,legendarySkillBook]:[skillBook,legendarySkillBook,ngPlusSkillBook,ngPlusWeaponUpgrades];
    const allowed=new Set(books.flatMap(book=>Object.keys(book))),weaponIds=new Set(books.flatMap(book=>Object.values(book).map(c=>c.weapon).filter(Boolean)));
    const skills=Object.fromEntries(Object.entries(original.skills).filter(([id])=>allowed.has(id)&&!allSkillInfo(id)?.weapon));
    for(const id of Object.keys(skills)){const c=allSkillInfo(id);if(c?.requires&&!skills[c.requires])delete skills[id];}
    const restored=restoreBuildValues({skills,specials:original.specials,weapons:original.weapons.filter(id=>id==='nutBomb'||weaponIds.has(id)),scorePerks:{}});
    restored.hardMode=Boolean(hard);restored.impossibleMode=Boolean(impossible);
    const derived=captureNGPlusBuild(restored);delete derived.trollCarry;derived.legacyDerived=true;return derived;
  }
  function canStartMode(level,hard,build=null,impossible=false){
    if(![0,1,2,3].includes(level))return false;
    if(impossible){if(!hardProgress.endless)return false;if(level===3)return Boolean((impossibleProgress.clear||hallOfFame.some(r=>hallDifficulty(r)==='impossible'&&hallTier(r)===2))&&(!build||build.impossibleMode));}
    if(hard&&!hardUnlocked())return false;
    if(level===0)return true;
    if(level===1||level===2){
      const ready=impossible?(level===1?impossibleProgress.ngplus:impossibleProgress.ngplus2):hard?(level===1?hardProgress.ngplus:hardProgress.ngplus2):(level===1?ngPlusUnlocked||ngPlusEverUnlocked:ngPlus2Unlocked||ngPlus2EverUnlocked);
      if(!ready&&!campaignSources(level,hard,impossible).length&&!hallOfFame.some(r=>hallTier(r)===2&&hallDifficulty(r)===(impossible?'impossible':hard?'hard':'normal')))return false;
      if(build&&(Boolean(build.hardMode)!==Boolean(hard)||Boolean(build.impossibleMode)!==Boolean(impossible)||build.sourceTier!==undefined&&build.sourceTier!==level-1))return false;
      return Boolean(campaignSources(level,hard,impossible).length||legacyCampaignBuild(level,hard,impossible));
    }
    if(impossible?!impossibleProgress.clear:hard?!hardProgress.endless:!hallOfFame.some(r=>hallDifficulty(r)==='normal'&&hallTier(r)===2)&&localStorage.getItem('snickers3-normal-ng2-clear-v1')!=='1')return false;
    return !build||Boolean(build.hardMode)===Boolean(hard)&&!build.impossibleMode;
  }
  function runScoreMultiplier(){return (player.scoreRushBonus||1)*(impossibleMode?1.75:hardMode?1.55:1);}
  function persistHardWin(){
    ...
    else if(gamePlusLevel===2){hardProgress.endless=true;impossibleProgress.unlocked=true;addAchievementProgress('hard_ng2_clear');}
    archiveCampaignWin(build);
  ...
  updateNGPlusMenu=function(){contentUpdateMenu();$('hardModeButton')?.classList.toggle('hidden',!hardUnlocked());$('endlessButton')?.classList.toggle('hidden',!hallOfFame.some(r=>hallDifficulty(r)==='normal'&&hallTier(r)===2)&&localStorage.getItem('snickers3-normal-ng2-clear-v1')!=='1'&&!hardProgress.endless);updateContinueButton();};
  function requestRunStart(level,hard=false,build=null,impossible=false){
    ...
    if((level===1||level===2)&&!build){showCampaignBuildSelect(level,hard,impossible);return;}
    const launch=()=>showRunCharacterPick(level,hard,impossible,build);
    if(!hasSavedRun(level===3?ENDLESS_SAVE_KEY:RUN_SAVE_KEY)){launch();return;}
    mode='newRunConfirm';showOverlay('<span class="eyebrow">GESPEICHERTER RUN VORHANDEN</span><h2 id="overlayTitle">NEU ANFANGEN?</h2><p>Ein neuer Run ersetzt den pausierten Run desselben Modus. Kampagne und Endloslauf haben getrennte Speicher. Erfolge, freigeschaltete Modi und Hall of Fame bleiben erhalten.</p><div class="overlay-actions"><button class="primary-button" id="confirmFreshRun">NEUEN RUN STARTEN ↗</button><button class="secondary-button" id="keepSavedRun">GESPEICHERTEN RUN BEHALTEN</button></div>');
  ...
  function chooseRunMode(level=0){
    ...
    mode='modeSelect';showOverlay(`<span class="eyebrow">SCHWIERIGKEIT WÄHLEN</span><h2 id="overlayTitle">${level===3?'ENDLOS WEITER.':'FRISCH AUS DEM NEST.'}</h2><p>Wähle die Schwierigkeit für diesen Run. Jeder Modus hat eigene Freischaltungen und Builds.</p><div class="difficulty-options"><button class="upgrade" id="normalStart"><small>NORMAL</small><strong>Angenehme Hasenjagd</strong><span>Die bisherige Schwierigkeit.</span></button><button class="upgrade hard-card" id="hardStart"><small>HARD</small><strong>Hasen jagen mit Hummeln im Arsch</strong><span>Mehr Gegnerdruck und +55 % Punkte.</span></button>${hardProgress.endless?'<button class="upgrade impossible-card" id="impossibleStart"><small>IMPOSSIBLE</small><strong>Tollwütige Hasenjagd des Grauens</strong><span>Mehr verdorbene Gegner, zusätzliche Bossphasen und +75 % Punkte.</span></button>':''}</div><button class="secondary-button" id="modeBack">ZURÜCK</button>`);
    ...
    $('hardStart').onclick=()=>level===3?showEndlessSelect(true,false):requestRunStart(level,true);
    if(hardProgress.endless){$('impossibleStart').disabled=!canStartMode(level,true,null,true);$('impossibleStart').onclick=()=>level===3?showEndlessSelect(true,true):requestRunStart(level,true,null,true);}
    $('modeBack').onclick=goMenu;
  }
  function showHardModes(){
    ...
    labels.forEach((_,i)=>{$('hardLevel'+i).onclick=()=>i===3?showEndlessSelect(true,false):requestRunStart(i,true);});$('hardBack').onclick=goMenu;
  }
  const contentRunSummary=runSummary;
  runSummary=function(r,i,source){return contentRunSummary(r,i,source).replace(`${tierName(hallTier(r))} ABGESCHLOSSEN`,`${hallDifficulty(r).toUpperCase()} · ${tierName(hallTier(r))} ABGESCHLOSSEN`);};
  let campaignChoice=null;
  function showCampaignBuildSelect(level,hard=false,impossible=false){
    if(!canStartMode(level,hard,null,impossible))return;
    campaignChoice={level,hard:Boolean(hard||impossible),impossible:Boolean(impossible)};mode='campaignSelect';
    const entries=campaignSources(level,hard,impossible),difficulty=impossible?'IMPOSSIBLE':hard?'HARD':'NORMAL',fallback=entries.length?null:legacyCampaignBuild(level,hard,impossible);
    showOverlay(`<span class="eyebrow">${difficulty} · ${tierName(level)} · BUILD WÄHLEN</span><h2 id="overlayTitle">WELCHER BUILD DARF WEITER?</h2><p>Wähle einen abgeschlossenen ${difficulty}-${tierName(level-1)}-Build für ${tierName(level)}. Alle Skills, Waffen und permanenten Punkte-Boni dieses Runs werden übernommen. Danach wählst du die Spielfigur für den neuen Run.</p><div class="hall-run-list">${entries.map(({r,i})=>runSummary(r,i,'campaign')).join('')}${fallback?'<button class="run-card" id="campaignLegacy"><span class="run-card-medal">↻</span><span class="run-card-main"><strong>ÄLTEREN SPIELSTAND ÜBERNEHMEN</strong><small>Vor diesem Update wurden frühere Abschlüsse nicht vollständig archiviert. Die verfügbaren Werte wurden passend zur Stufe rekonstruiert.</small></span><span class="run-card-arrow">↗</span></button>':''}</div><button class="secondary-button" id="campaignBack">← HAUPTMENÜ</button>`);
    for(const {i} of entries)$('campaignRun'+i).onclick=()=>showRunDetail(i,'campaign');
    if(fallback)$('campaignLegacy').onclick=()=>{showOverlay(`<span class="eyebrow">ÜBERNOMMENER SPIELSTAND</span><h2 id="overlayTitle">REKONSTRUIERTER BUILD.</h2><p>Der genaue frühere ${tierName(level-1)}-Abschluss wurde von der alten Version nicht gespeichert. Dieser Build enthält die noch rekonstruierbaren Fähigkeiten dieser Stufe.</p>${buildDetailsMarkup(restoreBuildValues(fallback),false)}<button class="primary-button" id="campaignLegacyStart">MIT DIESEM BUILD STARTEN ↗</button><button class="secondary-button" id="campaignLegacyBack">← ZUR AUSWAHL</button>`);$('campaignLegacyStart').onclick=()=>requestRunStart(level,hard,fallback,impossible);$('campaignLegacyBack').onclick=()=>showCampaignBuildSelect(level,hard,impossible);};
    $('campaignBack').onclick=goMenu;
  }
  showEndlessSelect=function(hard=endlessSelectHard,impossible=endlessSelectImpossible){
    if(!canStartMode(3,hard,null,impossible)){toast('Der Endlosmodus wird mit dem passenden NG+2-Abschluss freigeschaltet.');return;}
    endlessSelectHard=Boolean(hard||impossible);endlessSelectImpossible=Boolean(impossible);mode='endlessSelect';
    const difficulty=impossible?'IMPOSSIBLE':hard?'HARD':'NORMAL',entries=hallOfFame.map((r,i)=>({r,i})).filter(({r})=>hallDifficulty(r)===difficulty.toLowerCase());
    const history=endlessHistory.filter(r=>r.difficulty===difficulty.toLowerCase()).slice(0,5);
    showOverlay(`<span class="eyebrow">${difficulty} · ENDLOSMODUS</span><h2 id="overlayTitle">KEIN FEIERABEND.</h2><p>Frisch starten oder einen abgeschlossenen ${difficulty}-Build aus Normal, New Game+ oder New Game+2 übernehmen. Die Gegner skalieren passend zum gewählten Ausgangs-Build. Dein Kampagnen-Run bleibt separat gespeichert.</p><button class="endless-build-card fresh" id="endlessFresh"><strong>◌ KOMPLETT FRISCH STARTEN</strong><small>${difficulty} · ohne übernommene Upgrades</small></button><div class="hall-run-list endless-run-list">${entries.map(({r,i})=>runSummary(r,i,'endless')).join('')}</div>${history.length?`<h3>ABGESCHLOSSENE ENDLOSLÄUFE</h3><div class="endless-history">${history.map(r=>`<p>${escapeHTML(runDate(r))} · ${r.wave} Wellen · ${displayNumber(r.score)} Punkte</p>`).join('')}</div>`:''}<button class="secondary-button" id="endlessBack">ZURÜCK ZUM MENÜ</button>`);
    $('endlessFresh').onclick=()=>requestRunStart(3,endlessSelectHard,null,endlessSelectImpossible);entries.forEach(({i})=>{$('endlessRun'+i).onclick=()=>showRunDetail(i,'endless');});$('endlessBack').onclick=goMenu;
  };
  startHallRun=function(index){const entry=hallOfFame[index];if(!entry)return;requestRunStart(3,Boolean(entry.hardMode),hallBuild(entry),Boolean(entry.impossibleMode||entry.build?.impossibleMode));};
  ...
  showRunDetail=function(index,source='hall'){
    ...
    const difficulty=r.impossibleMode||r.build?.impossibleMode?'IMPOSSIBLE':r.hardMode?'HARD':'NORMAL';
    $('overlayTitle').textContent=`${difficulty} · RUN ${hallOfFame.length-index}`;
    $('runDetailStart').textContent=`IN ${difficulty}-ENDLOSMODUS STARTEN ↗`;
    if(source==='endless'){
      const ids=hallOfFame.map((x,i)=>Boolean(x.hardMode)===endlessSelectHard&&Boolean(x.impossibleMode||x.build?.impossibleMode)===endlessSelectImpossible?i:-1).filter(i=>i>=0),pos=ids.indexOf(index);
  ...
  updateHud=function(){contentHud();if(!player)return;if(hardMode){$('waveLabel').textContent=(impossibleMode?'IMPOSSIBLE · ':'HARD · ')+$('waveLabel').textContent;if(boss?.hardFinal){$('bossPhase').textContent=impossibleMode?'IMPOSSIBLE · LETZTE PHASE':'HARD · ZUSATZPHASE';$('waveObjective').textContent='LETZTE PHASE · DOPPELT GEBACKEN';}}if(boss?.herbert&&!boss.hardFinal)$('bossPhase').textContent=`ULTIMATIVE CHALLENGE · PHASE ${boss.herbertPhase||1} / ${impossibleMode?5:hardMode?4:3}`;};
  function isLastBossPhase(e){return e.herbert?(e.herbertPhase||1)>=3:e.ottah?(e.ottahPhase||1)>=3:e.karnil?(newGamePlus?e.phaseThree&&e.paftiHealed:e.phaseTwo):true;}
  ...
  const ENDLESS_SAVE_KEY='snickers3-endless-active-run-v1';
  // Earlier releases stored both modes in one slot. Preserve an unfinished old endless run.
  try{
    const legacy=localStorage.getItem(RUN_SAVE_KEY);
    if(legacy&&!localStorage.getItem(ENDLESS_SAVE_KEY)){
      const saved=JSON.parse(legacy);
      if(saved?.schema===1&&saved?.summary?.level===3){localStorage.setItem(ENDLESS_SAVE_KEY,legacy);localStorage.removeItem(RUN_SAVE_KEY);}
    }
  }catch{}
  ...
  let contentEffects=[],contentCompanion=null,trollQuest=null,knollQuest=null;
  ...
  function snapshotRun(){
    ...
    return {savedMode,player,enemies,bullets,enemyBullets,particles,pickups,hazards,floaters,thrownWeapons,ossiWalls,poisonPatches,ratSwarms,boss,
      ...
      gamePlusLevel,newGamePlus,newGamePlus2,endlessMode,endlessFromHall,endlessBuildTier,endlessBossesDefeated,stageVisual,stagePrepared,generalDefeated,cyberDefeated,lastDefeatedBoss,
      minibossPlan,lastMiniWave,victoryTime,hardMode,impossibleMode,paftiBoss,contentEffects,contentCompanion,trollQuest,knollQuest,pendingBossKind,scoreFallback,victoryDestination,interludeKind,
  ...
  function readSavedRun(key=RUN_SAVE_KEY){
    try{const raw=localStorage.getItem(key);if(!raw)return null;const save=JSON.parse(raw);return save?.schema===1&&save.data&&save.id?save:null;}catch{return null;}
  }
  ...
  function beginRunSession(carryBuild){
    ...
    knollQuest=null;trollQuest=carryBuild?.trollCarry?cloneData(carryBuild.trollCarry):null;
    if(trollQuest){trollQuest.status='pending';trollQuest.wave=null;}
    if(hardMode){const p=$('overlayContent').querySelector('p');if(p)p.textContent+=impossibleMode?' IMPOSSIBLE: 75 % mehr Punkte, zusätzliche Gegner und Bossphasen. Ottah kämpft zusammen mit Pafti.':' HARD: 55 % mehr Punkte, neue Gegner und eine zusätzliche Phase je Hauptboss.';}
  ...
  function saveRunNow(){
    ...
    try{
      ...
      localStorage.setItem(endlessMode?ENDLESS_SAVE_KEY:RUN_SAVE_KEY,JSON.stringify(save));saveFailed=false;return true;
  ...
  function endRunSession(force=false){
    const key=endlessMode?ENDLESS_SAVE_KEY:RUN_SAVE_KEY;
    try{const save=readSavedRun(key);if(force||!save||save.id===runSessionId)localStorage.removeItem(key);}catch{}
    runSessionId=null;updateContinueButton();
  }
  function resumeSavedRun(key=RUN_SAVE_KEY){
    const saved=readSavedRun(key);if(!saved){toast('Kein gespeicherter Run vorhanden.');return;}
    let s;try{
      ...
      s.impossibleMode=Boolean(s.impossibleMode);s.paftiBoss??=null;s.endlessBuildTier=clamp(s.endlessBuildTier??(s.endlessFromHall?2:0),0,2);
    ...
    try{
      ({player,enemies,bullets,enemyBullets,particles,pickups,hazards,floaters,thrownWeapons,ossiWalls,poisonPatches,ratSwarms,boss,
        wave,waveTime,runTime,spawnTimer,kills,score,shake,bombFlash,shotTimer,ambientTime,uiTimer,retriesLeft,retryWave,retryBossKind,
        gamePlusLevel,newGamePlus,newGamePlus2,endlessMode,endlessFromHall,endlessBuildTier,endlessBossesDefeated,stageVisual,stagePrepared,generalDefeated,cyberDefeated,lastDefeatedBoss,
        minibossPlan,lastMiniWave,victoryTime,hardMode,impossibleMode,paftiBoss,contentEffects,contentCompanion,trollQuest,knollQuest,pendingBossKind,scoreFallback,victoryDestination,interludeKind,bossQuoteBag}=s);
  ...
  finish=function(won){contentFinish(won);if(mode==='won'||mode==='lost'){if(mode==='lost'&&endlessMode)archiveEndlessRun(false);endRunSession();}else if(mode==='retry'){saveRunNow();if(endlessMode&&wave+1>=50)$('retryMenuButton').onclick=()=>{archiveEndlessRun(false);goMenu();};}};
  ...
  function spawnBergtroll(){
    const e=spawnEnemy('brute'),p=safeSpawnPoint(),scale=endlessMode?endlessHpScale(wave)*endlessCarryFactor():campaignHpScale();
  ...
  function renderBossReward(){
    ...
    showOverlay(`<span class="eyebrow">${hardMode?'HARD · ':''}BOSS GESCHAFFT · ${getStageName(stageVisual)}</span><h2 id="overlayTitle">BEUTE AUS DEM BOSS.</h2><p>Wähle ein Upgrade. Alle Spezialwaffen sind nachgeladen.</p><div class="upgrades special-upgrades">${choices.map((c,i)=>`<button class="upgrade ${rarityClass(c)}" id="specialUpgrade${i}" data-skill="${c.id}"><small class="upgrade-kind">${c.special?'BOSS-UPGRADE':c.legendary?'LEGENDARY':c.weapon?'WAFFE':'SKILL'}</small><span class="upgrade-icon">${c.icon}</span><strong>${c.title}</strong><span>${c.desc}</span><em>WÄHLEN ↗</em></button>`).join('')}</div>`);
    choices.forEach((c,i)=>{$('specialUpgrade'+i).onclick=()=>{if(mode!=='bossUpgrade')return;grantReward(c);player.weaponUses[player.weapons[0]]=maxWeaponAmmo(player.weapons[0]);afterBossReward(pendingBossKind);};});
  }
  ...
  function brunoPosition(){if(player?.brunoOperation)return {x:player.brunoX??player.x+90,y:player.brunoY??player.y-35};const top=viewW<950?305:145;return {x:clamp(camX+viewW-57,65,W-50),y:clamp(camY+top,145,H-60)};}
  const contentExpansion=updateExpansion;
  updateExpansion=function(dt){contentExpansion(dt);if(mode!=='playing')return;updateContent(dt);if(mode!=='playing')return;
    if(player.endlessPicket){player.picketCd=(player.picketCd??3)-dt;if(player.picketCd<=0){const targets=combatTargets().filter(e=>!e.dead).sort((a,b)=>dist(player,a)-dist(player,b)).slice(0,3);if(targets.length){player.picketCd=7;for(const e of targets){if(!isBoss(e))e.slow=Math.max(e.slow||0,.7);particles.push({type:'beam',x:player.x,y:player.y,x2:e.x,y2:e.y,life:.17,maxLife:.17,color:'#b9d8af'});damageEnemy(e,2.8*Math.sqrt(player.damage||1));if(mode!=='playing')return;}}}}
    if(player.endlessPostbox){player.postboxCd=Math.max(0,(player.postboxCd||0)-dt);if(player.postboxCd<=0){const shots=enemyBullets.filter(b=>b.life>0&&dist(player,b)<145).slice(0,3);if(shots.length){for(const shot of shots){shot.life=0;burst(shot.x,shot.y,'#e7c489',5,50);}player.postboxCd=15;const target=combatTargets().filter(e=>!e.dead&&dist(player,e)<240).sort((a,b)=>dist(player,a)-dist(player,b))[0];if(target)damageEnemy(target,2.2*Math.sqrt(player.damage||1));}}}
  };
  function updateContent(dt){
    ...
    if(player.creamPuff){player.puffCd=(player.puffCd??1)-dt;if(player.puffCd<=0){player.puffCd=7;contentEffects.push({kind:'puff',x:player.x,y:player.y,r:14,life:5,angle:runTime*2,damage:3.2*rootDamage});if(evolved('creamPuff',3))contentEffects.push({kind:'puff',x:player.x,y:player.y,r:14,life:5,angle:runTime*2+Math.PI,damage:1.6*rootDamage});}}
    if(player.bruno){
      if(player.brunoOperation){
        player.brunoX??=player.x+90;player.brunoY??=player.y-35;
        const prey=nearestTarget(brunoPosition(),670),destination=prey&&dist(player,prey)<660?prey:{x:player.x+72,y:player.y-28};
        const gap=dist(brunoPosition(),destination),stop=prey&&destination===prey?136:15;
        if(gap>stop){const step=Math.min(gap-stop,205*dt);player.brunoX=clamp(player.brunoX+(destination.x-player.brunoX)/gap*step,40,W-40);player.brunoY=clamp(player.brunoY+(destination.y-player.brunoY)/gap*step,123,H-40);}
      }
      player.brunoCd=(player.brunoCd||0)-dt;const from=brunoPosition(),target=nearestTarget(from,player.brunoOperation?720:1500);
      if(target&&player.brunoCd<=0){player.brunoCd=player.brunoOperation?.68:.85;contentProjectile(from,Math.atan2(target.y-from.y,target.x-from.x),(player.brunoOperation?2.5:2)*rootDamage,{ping:true,pierce:1,speed:650,life:2.1,r:7});}
    }
    ...
    if(player.wollenkamps){
      player.wollenUmbrellaCd=Math.max(0,(player.wollenUmbrellaCd||0)-dt);
      ...
      if(d>35){const step=Math.min(d-35,175*dt);c.x+=(dest.x-c.x)/d*step;c.y+=(dest.y-c.y)/d*step;}
      if(t&&d<80&&c.cooldown<=0){c.cooldown=6.1;c.talk=2;c.line=shuffled(['Schon wieder Regen!','Dieses Wetter!','Früher war mehr Sonne.','Haben Sie einen Schirm?','Der Wind ist ja frech!','Die Wolken sind beleidigt.','Nass bis ins Fell!','Morgen wird es besser.','Das ist doch kein Sommer!','Ich melde das dem Wetteramt.','Wo ist mein Regenschirm?','Die Luft riecht nach Ärger.','Die Sonne macht Überstunden.','Das gibt Gewitter!','Der Luftdruck ist unverschämt!','Ich habe die Wolke gewarnt!','Bei dem Wind fliegt mein Kuchen weg!','Die Sonne schuldet mir Urlaub!','Wer hat hier Durchzug bestellt?','Das Thermometer lügt schon wieder!','Mein Schirm hat heute Kündigung eingereicht.'])[0];if(isBoss(t)||t.miniBoss)t.slow=Math.max(t.slow||0,1.55);else t.discussTime=1.95;burst(t.x,t.y,'#d3c7ef',6,45);
        if(player.skills.wollenkampsUp6){const nearby=targets.filter(e=>e!==t&&!e.dead&&dist(e,t)<105+e.r).slice(0,3);for(const e of nearby){damageEnemy(e,1.1*rootDamage,true);if(mode!=='playing')return;}}
        if(player.skills.wollenkampsUp7)for(const e of targets)if(e!==t&&!e.dead&&dist(e,t)<115+e.r){e.slow=Math.max(e.slow||0,(isBoss(e)||e.miniBoss)?.35:.85);}
        if(player.skills.wollenkampsUp8&&player.wollenUmbrellaCd<=0&&enemyBullets.some(b=>dist(b,player)<145)){eraseNearbyShots(player,145,2);player.wollenUmbrellaCd=16;burst(player.x,player.y,'#d3c7ef',8,70);}
      }
    }
  ...
  function drawContent(){
    ...
    if(player.bruno){const p=brunoPosition();ctx.save();ctx.translate(p.x,p.y);ellipse(ctx,0,30,32,7,'#101e2177');if(player.brunoOperation){const stride=Math.sin(ambientTime*13)*8;ctx.strokeStyle='#a27477';ctx.lineWidth=7;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(-9,16);ctx.lineTo(-13+stride,33);ctx.moveTo(10,16);ctx.lineTo(14-stride,33);ctx.stroke();}else{ctx.fillStyle='#785c65';ctx.fillRect(-29,-13,58,48);ctx.fillStyle='#a77a7c';ctx.fillRect(-31,9,14,25);ctx.fillRect(17,9,14,25);}ellipse(ctx,0,5,20,23,'#c99a96');ellipse(ctx,0,-16,18,19,'#dfb3a4');path(ctx,[[-13,-25],[-22,-38],[-5,-32]],'#d3a399');path(ctx,[[13,-25],[22,-38],[5,-32]],'#d3a399');ellipse(ctx,0,-10,10,7,'#efc1b4');ellipse(ctx,-6,-20,2,3,'#313438');ellipse(ctx,6,-20,2,3,'#313438');ellipse(ctx,27,1,9,12,'#dabb70',-.3);ctx.fillStyle='#e9d9bb';ctx.font='bold 10px Arial';ctx.textAlign='center';ctx.fillText('BRUNO STERNI',0,48);ctx.restore();}
  ...
  function initContentPatch(){
    ...
    if(hallOfFame.some(r=>hallDifficulty(r)==='normal'&&hallTier(r)===2))try{localStorage.setItem('snickers3-normal-ng2-clear-v1','1');}catch{}
    if(hardUnlocked()&&!hardProgress.unlocked){hardProgress.unlocked=true;saveHardProgress();}
    if((achievementUnlocked('hard_ng2_clear')||hallOfFame.some(r=>hallTier(r)===2&&['hard','impossible'].includes(hallDifficulty(r))))&&!hardProgress.endless){hardProgress.endless=true;hardProgress.unlocked=true;saveHardProgress();}
    if(hardProgress.endless&&!impossibleProgress.unlocked){impossibleProgress.unlocked=true;saveImpossibleProgress();}
    if(hallOfFame.some(r=>hallDifficulty(r)==='impossible'&&hallTier(r)===2)&&!impossibleProgress.clear){impossibleProgress.clear=true;saveImpossibleProgress();}
    if(!unlockedSkins().has(selectedSkin))selectedSkin='classic';
    $('continueRunButton').onclick=()=>resumeSavedRun();$('endlessContinueRunButton').onclick=()=>resumeSavedRun(ENDLESS_SAVE_KEY);$('startButton').onclick=()=>chooseRunMode(0);$('ngPlusButton').onclick=()=>requestRunStart(1,false);$('ngPlus2Button').onclick=()=>requestRunStart(2,false);$('hardModeButton').onclick=showHardModes;$('endlessButton').onclick=()=>chooseRunMode(3);$('impossibleButton').onclick=showImpossibleModes;$('skinsButton').onclick=()=>showSkins(false);$('buildShareButton').onclick=()=>showShare(false);
  ...
  Object.assign(skillBook,{
    duden:passive('DER DUDEN','▤','Markiert auf jedem Gegner eine wechselnde Schwachstelle. Trifft dein Standardangriff den markierten Punkt, steigt seine gesamte Krit-Chance auf 78 % (Bosse: 68 %). Ein Krit verursacht doppelten Schaden; Spezialwaffen profitieren nicht.','duden'),
  ...
  Object.assign(skillBook,{
    eichelkopf:passive('DER EICHELKOPF','◕','Alle 7 s platzt eine orbitierende Rieseneichel am nächsten Ziel im Radius 95: 4 × √Nussschaden. Bosse erhalten nur einen Treffer.','eichelkopf'),
    pigPatrol:passive('DIE SCHWEINEPATROUILLE','♞','Zwei tollpatschige Schweine begleiten Snickers und schießen abwechselnd alle 1,5 s eine Nuss für 1,9 × √Nussschaden auf nahe Gegner.','pigPatrol'),
    troutDormian:passive('DER FORELLEN DORMIAN','♓','Eine umherschwimmende Forelle erwischt alle 3,8 s ein Ziel für 4,2 × √Spezialschaden und verlangsamt es kurz.','troutDormian'),
    pockenstrombose:passive('POCKENSTROMBOSE','Ϟ','Jeder siebte normale Nusstreffer entfesselt höchstens alle 2,4 s zwei Blitze auf verschiedene Gegner in 200 Reichweite. Jeder Blitz verursacht 1,6 × √Nussschaden und bremst normale Gegner kurz. Steht nur ein Boss da, trifft ihn ein schwächerer Blitz für 0,8 × √Nussschaden.','pockenstrombose')
  });
  Object.assign(ngPlusSkillBook,{
  ...
  Object.assign(survivalSkillBook,{
    endlessMeteor:passive('METEOR-KANTINE','☄','Nur Endlosmodus: Alle 8 s drei angekündigte Einschläge nahe Gegnern; je 7 × √Spezialschaden im Radius 95.','endlessMeteor',{endless:true}),
    endlessWaveguard:passive('WELLENBRECHER-SCHIRM','◈','Nur Endlosmodus: Alle 9 s löscht ein Puls bis zu 4 nahe Geschosse und trifft Gegner im Radius 155 für 3,5 × √Nussschaden.','endlessWaveguard',{endless:true}),
    endlessCourier:passive('MUNITIONS-BRIEFTAUBE','✉','Nur Endlosmodus: Alle 30 Kills erhält eine benutzte Spezialwaffe eine Ladung zurück.','endlessCourier',{endless:true}),
    endlessDoctor:passive('SPÄTSCHICHT-SANITÄTER','✚','Nur Endlosmodus: Alle 45 Kills heilt Snickers ein Herz. Funktioniert auch in langen Bosswellen.','endlessDoctor',{endless:true}),
    endlessBloodMoon:passive('BOSSMOND','☾','Nur Endlosmodus: +15 % Schaden gegen Bosse und Minibosse; +5 % Krit-Chance.','endlessBloodMoon',{endless:true,apply:p=>{p.endlessBloodMoon=true;p.bossDamage=(p.bossDamage||1)*1.15;p.crit+=.05;}}),
    endlessSwarm:passive('HUMMEL-ÜBERSTUNDEN','✿','Nur Endlosmodus: Alle 9 s starten vier zielsuchende Hummeln mit je 2 × Spezialfaktor Schaden.','endlessSwarm',{endless:true}),
    endlessFrost:passive('FROSTIGE PAUSE','❄','Nur Endlosmodus: Alle 10 s werden Gegner in 180 Reichweite 1,5 s verlangsamt; Bosse nur kurz.','endlessFrost',{endless:true}),
    endlessShell:passive('UNENDLICHE PANZERSCHALE','▣','Nur Endlosmodus: +2 Herzen, volle Heilung und 7 % Blockchance.','endlessShell',{endless:true,apply:p=>{p.endlessShell=true;p.maxHp+=2;p.hp=p.maxHp;p.damageGuard+=.07;}}),
    endlessSpark:passive('SCHICHTSTROM','ϟ','Nur Endlosmodus: Alle 7 s treffen kurze Blitze bis zu drei verschiedene Gegner mit je 3,5 × √Nussschaden.','endlessSpark',{endless:true}),
    endlessCompass:passive('LETZTER KOMPASS','✥','Nur Endlosmodus: +10 % Tempo und +10 % Spezialschaden.','endlessCompass',{endless:true,apply:p=>{p.endlessCompass=true;p.speed*=1.10;p.specialDamage*=1.10;}})
  });
  Object.assign(survivalSkillBook,{
    endlessPicket:passive('MITTERNACHTS-STREIFE','♟','Nur Endlosmodus: Alle 7 s trifft die Streife bis zu drei verschiedene Gegner für je 2,8 × √Nussschaden. Normale Gegner werden 0,7 s gebremst.','endlessPicket',{endless:true}),
    endlessPostbox:passive('PANZERBRIEFKASTEN','▤','Nur Endlosmodus: Höchstens alle 15 s löscht ein Gegenpuls bis zu drei feindliche Geschosse in 145 Reichweite und trifft einen Gegner in 240 Reichweite für 2,2 × √Nussschaden.','endlessPostbox',{endless:true}),
    endlessSurge:passive('NACHTSCHICHT-FUNKEN','ϟ','Nur Endlosmodus: +11 % Feuerrate und +7 Prozentpunkte Krit-Chance.','endlessSurge',{endless:true,apply:p=>{p.endlessSurge=true;p.fireRate/=1.11;p.crit+=.07;}}),
    endlessLarder:passive('VORRATSBUNKER','▣','Nur Endlosmodus: +1 maximales Herz, sofort +1 Heilung und +1 maximale Ladung pro Spezialwaffe.','endlessLarder',{endless:true,apply:p=>{p.endlessLarder=true;p.maxHp++;p.hp=Math.min(p.maxHp,p.hp+1);p.ammoBonus++;}}),
    endlessRadar:passive('KAROTTENRADAR','✧','Nur Endlosmodus: +9 % Tempo und +12 % Power-up-Dropchance (bis zum allgemeinen Limit von 16 %).','endlessRadar',{endless:true,apply:p=>{p.endlessRadar=true;p.speed*=1.09;p.powerLuck*=1.12;}})
  });
  ...
  ngPlusCarryKeys.push('bingoMaster','potencyMinister','duden','krokette','steelMuzzle','starIndex','ngFlicker','ngLedger','ng2Pulse','ng2Shell','endlessHarvest','endlessShield','eichelkopf','pigPatrol','troutDormian','pockenstrombose','endlessMeteor','endlessWaveguard','endlessCourier','endlessDoctor','endlessBloodMoon','endlessSwarm','endlessFrost','endlessShell','endlessSpark','endlessCompass','endlessPicket','endlessPostbox','endlessSurge','endlessLarder','endlessRadar');
  const addedNormal=['duden','krokette','olangolil','potencyMinister','steelMuzzle','starIndex','eichelkopf','pigPatrol','troutDormian','pockenstrombose'];
  upgradePools.forEach((pool,i)=>pool.push(addedNormal[i%addedNormal.length],addedNormal[(i+3)%addedNormal.length],addedNormal[(i+6)%addedNormal.length]));
  upgradePools[1].push('pockenstrombose');upgradePools[4].push('pockenstrombose');upgradePools[7].push('pockenstrombose');
  ...
  // Kastenschwein II: every new card has a bounded, distinct combat role.
  Object.assign(skillBook,{
    pigCheekBomber:passive('SCHWEINEBACKENBOMBER','◉','Alle 6 Sekunden schleudern Snickers’ Backen eine Bombe zum nächsten Gegner (Reichweite 550): 5 × √Nussschaden im Radius 100.','pigCheekBomber'),
    hamsterTwister:passive('HAMSTERSCHNIEDELDREHER','↻','Alle 3,6 Sekunden wirbelt eine cartoonhafte Nuss um Snickers: Gegner in 115 Reichweite erleiden 2,8 × √Nussschaden und werden kurz verlangsamt.','hamsterTwister'),
    hamsterVape:passive('HAMSTER VAPE','☁','Alle 8 Sekunden entsteht am nächsten Gegner eine 3 Sekunden anhaltende Cartoon-Dampfwolke. Sie trifft höchstens viermal für je 1,6 × √Spezialschaden und bremst normale Gegner.','hamsterVape')
  });
  ...
  Object.assign(weaponBook,{
    diarrheaSling:{name:'DURCHFALLSCHLEUDER',icon:'◔',desc:'Waffe: Drei Cartoon-Schlammpfützen am Ziel. Jede bleibt 3,4 Sekunden, trifft bis zu viermal für 4 × Spezialfaktor Schaden im Radius 86 und bremst normale Gegner. 3 Ladungen.',max:3},
    ngCheeseGatling:{name:'KÄSE-GATLING',icon:'⋰',desc:'NG+-Waffe: sechs versetzte Käsegeschosse, je 9 × Spezialfaktor Schaden und ein Durchschlag. 3 Ladungen.',max:3},
    ng2StarRoaster:{name:'STERNENRÖSTER',icon:'☀',desc:'NG+2-Waffe: Vier angekündigte Einschläge am Ziel, je 23 × Spezialfaktor Schaden im Radius 92. 2 Ladungen.',max:2}
  });
  skillBook.diarrheaSling=weaponUnlock('diarrheaSling');
  ngPlusWeaponUpgrades.ngCheeseGatling={...weaponUnlock('ngCheeseGatling'),ngplus:true};
  ngPlus2WeaponUpgrades.ng2StarRoaster={...weaponUnlock('ng2StarRoaster'),ngplus2:true};
  Object.assign(specialUpgradeBook,{
  ...
  updateContent=function(dt){
    ...
    if(player.eichelkopf){player.eichelCd=(player.eichelCd??3)-dt;if(player.eichelCd<=0){const t=nearestTarget(player,590);if(t){player.eichelCd=7;burst(t.x,t.y,'#d5b877',15,110);for(const e of combatTargets())if(!e.dead&&dist(e,t)<95+e.r){damageEnemy(e,4*Math.sqrt(player.damage),true);if(mode!=='playing')return;}}}}
    if(player.pigPatrol){player.patrolCd=(player.patrolCd??.7)-dt;if(player.patrolCd<=0){const side=player.patrolSide=-(player.patrolSide||-1),from={x:player.x+side*43,y:player.y+22},t=nearestTarget(from,560);if(t){player.patrolCd=1.5;contentProjectile(from,Math.atan2(t.y-from.y,t.x-from.x),1.9*Math.sqrt(player.damage),{auto:true,life:1.5});}}}
    if(player.troutDormian){player.troutCd=(player.troutCd??2)-dt;if(player.troutCd<=0){const t=nearestTarget({x:W/2,y:H/2},720);if(t){player.troutCd=3.8;t.slow=Math.max(t.slow||0,isBoss(t)?.5:1.2);damageEnemy(t,4.2*Math.sqrt(player.specialDamage),true);if(mode!=='playing')return;floater(t.x,t.y-27,'FORELLE!','#9fd5cb');}}}
    if(endlessMode&&player.endlessMeteor){player.endlessMeteorCd=(player.endlessMeteorCd??3)-dt;if(player.endlessMeteorCd<=0){player.endlessMeteorCd=8;const targets=combatTargets().filter(e=>!e.dead);for(let i=0;i<3;i++){const t=targets[i%targets.length]||{x:player.x+Math.cos(i*2)*100,y:player.y+Math.sin(i*2)*100};hazards.push({type:'friendlyMortar',x:clamp(t.x+(i-1)*30,60,W-60),y:clamp(t.y,135,H-55),r:95,wait:.58+i*.16,life:.33,hit:false,friendly:true,damage:7*Math.sqrt(player.specialDamage)});}}}
    if(endlessMode&&player.endlessWaveguard){player.endlessGuardCd=(player.endlessGuardCd??5)-dt;if(player.endlessGuardCd<=0){player.endlessGuardCd=9;eraseNearbyShots(player,155,4);burst(player.x,player.y,'#b1d6e7',18,170);for(const e of combatTargets())if(!e.dead&&dist(e,player)<155+e.r){damageEnemy(e,3.5*Math.sqrt(player.damage));if(mode!=='playing')return;}}}
    if(endlessMode&&player.endlessSwarm){player.endlessSwarmCd=(player.endlessSwarmCd??4)-dt;if(player.endlessSwarmCd<=0){player.endlessSwarmCd=9;launchRats(4,2*player.specialDamage,'bee',2.8);}}
    if(endlessMode&&player.endlessFrost){player.endlessFrostCd=(player.endlessFrostCd??4)-dt;if(player.endlessFrostCd<=0){player.endlessFrostCd=10;for(const e of combatTargets())if(!e.dead&&dist(e,player)<180+e.r)e.slow=Math.max(e.slow||0,isBoss(e)?.55:1.5);burst(player.x,player.y,'#a8d5ef',20,165);}}
    if(endlessMode&&player.endlessSpark){player.endlessSparkCd=(player.endlessSparkCd??3)-dt;if(player.endlessSparkCd<=0){player.endlessSparkCd=7;for(const e of combatTargets().filter(e=>!e.dead).sort((a,b)=>dist(a,player)-dist(b,player)).slice(0,3)){particles.push({type:'beam',x:player.x,y:player.y,x2:e.x,y2:e.y,life:.24,maxLife:.24,color:'#bfe7f6'});damageEnemy(e,3.5*Math.sqrt(player.damage));if(mode!=='playing')return;}}}
  ...
  drawContent=function(){originalContentDraw();if(!player)return;
    if(player.bingoMaster){
      const angry=player.bingoRage>0,waveArm=Math.sin(ambientTime*(angry?14:3))*8;
      ctx.save();ctx.translate(W/2,H/2);ellipse(ctx,0,69,39,9,'#13231bbb');
      ctx.strokeStyle=angry?'#d65a55':'#8d8179';ctx.lineWidth=9;ctx.lineCap='round';ctx.beginPath();
      ctx.moveTo(-19,14);ctx.lineTo(-36,36);ctx.lineTo(-54,70);ctx.moveTo(19,14);ctx.lineTo(37,36);ctx.lineTo(54,70);
      ctx.moveTo(-23,-5);ctx.lineTo(-50,-22+waveArm);ctx.lineTo(-66,-6+waveArm);ctx.moveTo(23,-5);ctx.lineTo(52,-18-waveArm);ctx.lineTo(65,4-waveArm);ctx.stroke();
      ellipse(ctx,0,0,29,30,angry?'#db7368':'#b8a08b');
      ellipse(ctx,0,-12,11,12,'#fff5df');ellipse(ctx,2,-12,5,7,angry?'#f1303e':'#273a38');ellipse(ctx,3,-14,2,2,'#fff5db');
      ellipse(ctx,-7,13,15,7,angry?'#f25160':'#d58198');ellipse(ctx,8,13,15,7,angry?'#f25160':'#d58198');ellipse(ctx,0,20,17,6,angry?'#bd2c3c':'#b55e76');
      ctx.fillStyle='#f6e5ad';ctx.fillRect(-75,-17+waveArm,26,25);ctx.strokeStyle='#795b4e';ctx.lineWidth=2;ctx.strokeRect(-75,-17+waveArm,26,25);
      ctx.font='bold 8px Arial';ctx.textAlign='center';ctx.fillStyle='#4a453e';ctx.fillText(angry?'BINGO!':'BINGO',-62,-3+waveArm);
      ctx.restore();
    }
    if(player.eichelkopf){const a=ambientTime*2,x=player.x+Math.cos(a)*42,y=player.y+Math.sin(a)*27;ellipse(ctx,x,y,11,14,'#9b6a3f',a);path(ctx,[[x-8,y-9],[x,y-19],[x+8,y-9]],'#5e8b51');}
    if(player.pigPatrol)for(const side of [-1,1]){const x=player.x+side*43,y=player.y+24+Math.sin(ambientTime*8+side)*3;ellipse(ctx,x,y,13,12,'#c27f74');ellipse(ctx,x-8,y-10,5,7,'#c27f74');ellipse(ctx,x+8,y-10,5,7,'#c27f74');ellipse(ctx,x,y+3,8,5,'#edb8a5');ellipse(ctx,x-3,y+2,1.7,2,'#543a3f');ellipse(ctx,x+3,y+2,1.7,2,'#543a3f');ctx.fillStyle='#6b5e4d';ctx.fillRect(x+side*10,y-3,15,4);}
    if(player.troutDormian){const x=player.x+Math.cos(ambientTime*1.6)*116,y=player.y+Math.sin(ambientTime*1.6)*72;ctx.save();ctx.translate(x,y);ctx.rotate(Math.cos(ambientTime*1.6)*.55);ellipse(ctx,0,0,22,9,'#96bec0');path(ctx,[[-17,0],[-35,-13],[-33,12]],'#7d9ba8');path(ctx,[[-5,-5],[1,-15],[10,-6]],'#71999e');ellipse(ctx,13,-2,2,2,'#162c3a');ctx.restore();}
  ...
  onContentKill=function(e){originalKillEffects(e);if(hardMode&&!impossibleMode&&e.ottah&&player.bossHits===0)addAchievementProgress('hard_ottah_clean');
    if(endlessMode&&!isBoss(e)&&!e.miniBoss){
      if(player.endlessCourier){player.endlessCourierKills=(player.endlessCourierKills||0)+1;if(player.endlessCourierKills>=30){player.endlessCourierKills=0;const id=shuffled(player.weapons.filter(id=>weaponAmmo(id)<maxWeaponAmmo(id)))[0];if(id){player.weaponUses[id]++;updateWeaponHud();floater(player.x,player.y-40,'BRIEFTAUBE +1','#f0d6ab');}}}
      if(player.endlessDoctor){player.endlessDoctorKills=(player.endlessDoctorKills||0)+1;if(player.endlessDoctorKills>=45){player.endlessDoctorKills=0;player.hp=Math.min(player.maxHp,player.hp+1);updateHud();floater(player.x,player.y-40,'SPÄTSCHICHT +1 ♥','#bad5bb');}}
    }
  };
  ...
  achievements.push(
    ...[10,50,100].map(n=>({id:'impossible_endless_'+n,title:'FÄULNISWELLE '+n,desc:'Überstehe Impossible-Endloswelle '+n+' samt Boss.',goal:1,kind:'gag',impossible:true,impossibleEndless:true,secret:true})),
    ...['normal','hard','impossible'].map(d=>({id:'hasenbein_'+d,title:'HASENBEINS REVANCHE · '+d.toUpperCase(),desc:'Besiege eine Figur aus einem '+d.toUpperCase()+'-Hall-of-Fame-Build als Hasenbein.',goal:1,kind:'gag',hasenbein:true,secret:true})),
    ...['normal','hard','impossible'].map(d=>({id:'herbert_'+d,title:'LETZTER TON · '+d.toUpperCase(),desc:'Besiege Herbert Grönemeyer nach Endloswelle 200 auf '+d.toUpperCase()+'.',goal:1,kind:'gag',endless:true,secret:true}))
  );
  ...
  function showImpossibleModes(){
    ...
    mode='impossibleSelect';const names=['NORMAL','NEW GAME+','NEW GAME+2','ENDLOSMODUS'];
    showOverlay(`<span class="eyebrow impossible-kicker">IMPOSSIBLE MODE</span><h2 id="overlayTitle">TOLLWÜTIGE HASENJAGD DES GRAUENS.</h2><p>Freigeschaltet durch Hard NG+2. Impossible NG+2 endet mit Ottah und Pafti gemeinsam und öffnet den Impossible-Endlosmodus. Jede Stufe übernimmt nur ihren eigenen Sieger-Build.</p><div class="hard-mode-list">${names.map((name,i)=>`<button class="secondary-button impossible-card" id="impossibleLevel${i}" ${!canStartMode(i,true,null,true)?'disabled':''}>IMPOSSIBLE · ${name}${canStartMode(i,true,null,true)?' ↗':' · NOCH GESPERRT'}</button>`).join('')}</div><button class="secondary-button" id="impossibleBack">← HAUPTMENÜ</button>`);
    names.forEach((_,i)=>{$('impossibleLevel'+i).onclick=()=>i===3?showEndlessSelect(true,true):requestRunStart(i,true,null,true);});$('impossibleBack').onclick=goMenu;
  }
  ...
  Object.assign(extraEnemyBook,{
    ...
    rotWasp:{name:'PESTHUMMEL',stats:[20,150,20,820],color:'#c1b56d',species:'moth',hard:true,impossible:true,unlock:'Impossible Endlos · Welle 10'},
    plagueSinger:{name:'SEUCHENSÄNGER',stats:[29,81,27,1100],color:'#af84a9',species:'rabbit',hard:true,impossible:true,unlock:'Impossible Endlos · Welle 25'},
    bileBoar:{name:'GALLENKEILER',stats:[45,109,35,1400],color:'#96a368',species:'pig',hard:true,impossible:true,unlock:'Impossible Endlos · Welle 50'}
  });
  const earlierHardChoice=hardEnemyChoice;
  hardEnemyChoice=function(type){
    if(impossibleMode&&endlessMode&&wave>=9&&Math.random()<.32){
      const pool=['rotWasp',...(wave>=24?['plagueSinger']:[]),...(wave>=49?['bileBoar']:[])];
      const choices=pool.filter(id=>enemies.filter(e=>e.type===id&&!e.dead).length<3);
      if(choices.length)return shuffled(choices)[0];
    }
  ...
  updateContentEnemy=function(e,dt){
    ...
    if(e.windup>0){e.windup-=dt;if(e.windup<=0){if(e.type==='rottenHare'){for(let i=-2;i<=2;i++)enemyShot(e.x,e.y,e.aim+i*.23,210,true);}
      ...
      if(e.type==='marrowHunter'){e.charge=.32;for(const off of [-.4,0,.4])enemyShot(e.x,e.y,e.aim+off,235,true);}
      if(e.type==='rotWasp'){for(let i=-3;i<=3;i++)enemyShot(e.x,e.y,e.aim+i*.16,235,true);}
      if(e.type==='plagueSinger'){const n=14;for(let i=0;i<n;i++){const dir=i*TAU/n;if(Math.abs(angleDelta(dir,e.aim))>.36)enemyShot(e.x,e.y,dir,215,true);}}
      if(e.type==='bileBoar'){e.charge=.48;for(let i=-1;i<=1;i++)hazards.push({type:'mortar',x:clamp(e.targetX+i*78,65,W-65),y:clamp(e.targetY,145,H-65),r:44,wait:1.18+i*.1,life:.3,hit:false,damage:1.6});}}return;}
    if(e.charge>0){e.charge-=dt;e.x=clamp(e.x+Math.cos(e.aim)*(e.type==='bileBoar'?470:380)*dt,40,W-40);e.y=clamp(e.y+Math.sin(e.aim)*(e.type==='bileBoar'?470:380)*dt,112,H-44);return;}
    if(d>e.r+90){e.x+=Math.cos(a)*e.speed*slow*dt;e.y+=Math.sin(a)*e.speed*slow*dt;}
    if(e.fireCd<=0){e.fireCd=e.type==='bileBoar'?3.75:e.type==='plagueSinger'?3.3:3.2;e.windup=e.type==='bileBoar'?1.05:1.1;e.aim=a;e.targetX=player.x;e.targetY=player.y;}
  };

  const purpleRewards=new Set(['madelpulator','power','ironFur','nutSentry','waveRenew','duden','bingoMaster','olangolil','potencyMinister','narrath','ratKing','stormOrbit','ng2Pulse','ng2Shell','ngLedger','endlessPicket']);
  const blueRewards=new Set(['rapid','health','fortified','crit','specialCore','hunter','krokette','bruno','wollenkamps','steelMuzzle','snickersBar','powerLuck','ngFlicker','endlessShield','pockenstrombose']);
  ...
  const skins=[
    ...
    {id:'impossibleEndless10',name:'FAULIGER SCHWIMMRING',mark:'◉',color:'#a4ad80',hint:'Impossible Endloswelle 10 überstehen.'},
    {id:'impossibleEndless50',name:'SCHIMMEL-DJ',mark:'♫',color:'#a888c8',hint:'Impossible Endloswelle 50 überstehen.'},
    {id:'impossibleEndless100',name:'VERDORBENER WELLENKÖNIG',mark:'♚',color:'#e4af8a',hint:'Impossible Endloswelle 100 überstehen.'},
    {id:'hasenbeinVictor',name:'HASENBEINS ALBTRAUM',mark:'♞',color:'#eca36e',hint:'Im Hasenbein-Modus einen Hall-of-Fame-Snickers besiegen.'}
  ];
  function unlockedSkins(){
    const normal0=ngPlusEverUnlocked||ngPlusUnlocked||ngPlus2EverUnlocked||hallOfFame.some(r=>hallDifficulty(r)==='normal');
    const normal1=ngPlus2EverUnlocked||ngPlus2Unlocked||hallOfFame.some(r=>hallDifficulty(r)==='normal'&&hallTier(r)>=1);
    const normal2=localStorage.getItem('snickers3-normal-ng2-clear-v1')==='1'||hallOfFame.some(r=>hallDifficulty(r)==='normal'&&hallTier(r)===2);
    const active=readSavedRun(ENDLESS_SAVE_KEY)?.summary||readSavedRun()?.summary;
    if(active?.level===3){const key=active.impossible?'impossible':active.hard?'hard':'normal';endlessBest[key]=Math.max(endlessBest[key]||0,(active.wave||1)-1);}
    const normalBest=Math.max(endlessBest.normal||0,endlessBest.hard||0,endlessBest.impossible||0),hardBest=Math.max(endlessBest.hard||0,endlessBest.impossible||0),impossibleBest=endlessBest.impossible||0;
    const won=(difficulty,level)=>hallOfFame.some(r=>hallDifficulty(r)===difficulty&&hallTier(r)>=level);
    return new Set(['classic',...(normal0?['normal0']:[]),...(normal1?['normal1']:[]),...(normal2?['normal2']:[]),...(hardProgress.ngplus||achievementUnlocked('hard_clear')||won('hard',0)?['hard0']:[]),...(hardProgress.ngplus2||achievementUnlocked('hard_ng_clear')||won('hard',1)?['hard1']:[]),...(hardProgress.endless||achievementUnlocked('hard_ng2_clear')||won('hard',2)?['hard2']:[]),...(normalBest>=50?['endless50']:[]),...(normalBest>=100?['endless100']:[]),...(hardBest>=50?['hard50']:[]),...(hardBest>=100?['hard100']:[]),...(impossibleProgress.ngplus||won('impossible',0)?['impossible0']:[]),...(impossibleProgress.ngplus2||won('impossible',1)?['impossible1']:[]),...(impossibleProgress.clear||won('impossible',2)?['impossible2']:[]),...(impossibleBest>=10?['impossibleEndless10']:[]),...(impossibleBest>=50?['impossibleEndless50']:[]),...(impossibleBest>=100?['impossibleEndless100']:[]),...(['normal','hard','impossible'].some(d=>achievementUnlocked('hasenbein_'+d))?['hasenbeinVictor']:[])]);
  }
  ...
  function drawSkin(p,alpha=1){
    const skin=skins.find(s=>s.id===selectedSkin);if(!skin||skin.templateId==='classic'||skin.id==='classic'||alpha<.9)return;
    const template=skin.templateId||skin.id;
    ...
    if(template==='normal0'){ctx.strokeStyle='#d9eec8';ctx.lineWidth=3;ctx.beginPath();ctx.arc(13,-4,9,0,TAU);ctx.moveTo(19,2);ctx.lineTo(30,14);ctx.stroke();}
    else if(template==='normal1'){for(const x of [-12,0,12])ellipse(ctx,x,-43,11,10,'#fff4e3');ctx.fillStyle='#e6ad6e';ctx.fillRect(18,3,5,25);}
    else if(template==='normal2'){path(ctx,[[-5,5],[0,11],[5,5]],'#e7e6ed');ellipse(ctx,0,9,2,2,'#372637');ctx.fillStyle='#ebe0bb';ctx.fillRect(20,8,13,4);}
    else if(template==='hard0'){ctx.strokeStyle='#ffded1';ctx.lineWidth=5;ctx.beginPath();ctx.arc(0,3,20,0,TAU);ctx.stroke();}
    else if(template==='hard1'){for(let i=-2;i<=2;i++)path(ctx,[[i*7-4,-29],[i*7,-51-Math.abs(i)*3],[i*7+4,-29]],'#f1976c');}
    else if(template==='hard2'){ctx.fillStyle='#f7d064';for(const x of [-23,22]){ellipse(ctx,x,2,10,16,'#f0d16e99');ctx.fillRect(x-1,8,3,13);}}
    else if(template==='endless50'||template==='hard50'){ctx.strokeStyle='#b4e0f2';ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,-4,21,Math.PI,TAU);ctx.stroke();ctx.fillStyle=skin.color;ctx.fillRect(-22,-3,9,14);ctx.fillRect(13,-3,9,14);}
    else if(template==='endless100'||template==='hard100'){ctx.strokeStyle=skin.color;ctx.lineWidth=2;for(let i=0;i<4;i++){const a=ambientTime*2+i*TAU/4;ellipse(ctx,Math.cos(a)*29,Math.sin(a)*18-5,3,3,skin.color);}}
    else if(template==='impossible0'){ctx.fillStyle='#a1bd7a';for(const x of [-20,18]){ellipse(ctx,x,-35,7,10,'#8aa36a');ctx.fillRect(x-2,-26,4,9);}}
    else if(template==='impossible1'){ctx.fillStyle='#52395d99';ctx.fillRect(-22,-24,44,34);ellipse(ctx,0,-13,13,9,'#b6a0df77');}
    else if(template==='impossible2'){ctx.strokeStyle='#ffe199';ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,-31,26+Math.sin(ambientTime*3)*2,0,TAU);ctx.stroke();}
    else if(template==='impossibleEndless10'){ctx.strokeStyle='#c9d1a0';ctx.lineWidth=7;ctx.beginPath();ctx.arc(0,8,28,.12,Math.PI-.12);ctx.stroke();ellipse(ctx,-8,-42,5,4,'#92ac7a');}
    else if(template==='impossibleEndless50'){ctx.strokeStyle='#b99bd9';ctx.lineWidth=5;ctx.beginPath();ctx.arc(0,-10,26,Math.PI,TAU);ctx.stroke();for(const x of [-25,25])ellipse(ctx,x,-7,6,12,'#ae8dcc');}
    else if(template==='impossibleEndless100'){path(ctx,[[-21,-33],[-15,-55],[-4,-40],[0,-60],[8,-40],[18,-55],[21,-33]],'#edbf8e');for(let i=0;i<3;i++)ellipse(ctx,(i-1)*16,-32,3,3,'#7e3c4d');}
    else if(template==='hasenbeinVictor'){ellipse(ctx,-12,-56,6,23,'#e7ac80',-.15);ellipse(ctx,12,-57,6,25,'#e7ac80',.15);ellipse(ctx,-12,-57,3,17,'#b36a73',-.15);ellipse(ctx,12,-58,3,18,'#b36a73',.15);ctx.fillStyle='#5c3138';ctx.fillRect(-19,-17,38,11);ellipse(ctx,-8,-12,3,2,'#ffd692');ellipse(ctx,8,-12,3,2,'#ffd692');}
    else if(template==='superSaiyajin'){path(ctx,[[-19,-32],[-15,-52],[-8,-41],[0,-64],[8,-42],[17,-57],[20,-32]],'#ffe274');ctx.strokeStyle='#ffed98';ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,-9,29+Math.sin(ambientTime*8)*2,0,TAU);ctx.stroke();for(const x of [-21,20])path(ctx,[[x,-19],[x+6,-12],[x,-6]],'#fff3ad');}
    if(['hard0','hard1','hard2','hard50','hard100','impossible0','impossible1','impossible2','impossibleEndless10','impossibleEndless50','impossibleEndless100'].includes(template)){ctx.strokeStyle=skin.color+'aa';ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,-4,27,0,TAU);ctx.stroke();}
    ctx.restore();
  }
  ...
  function dudenSpot(e){if(e.dudenSpot)return e.dudenSpot;const a=Math.random()*TAU,r=Math.max(7,e.r*.68);return e.dudenSpot={x:Math.cos(a)*r,y:Math.sin(a)*r};}
  drawRabbit=function(e){previousDrawRabbit(e);if(e.dead)return;ctx.save();
    if(hardMode&&!e.hasenbeinAlly){ctx.shadowColor=impossibleMode?'#ff2525':'#d94740';ctx.shadowBlur=impossibleMode?16:9;for(const x of [-e.r*.33,e.r*.33])ellipse(ctx,e.x+x,e.y-7,impossibleMode?4:3,3,impossibleMode?'#ff2929':'#d84a42');}
    if(e.potencyMarked>0){ctx.font='bold 14px Arial';ctx.textAlign='center';ctx.fillStyle='#ffbb8d';ctx.fillText('✕',e.x,e.y-e.r-5);}
    if(player?.duden){const spot=dudenSpot(e);ctx.shadowBlur=8;ctx.shadowColor='#f9dfa1';ctx.strokeStyle='#ffe7a8';ctx.lineWidth=2;ctx.beginPath();ctx.arc(e.x+spot.x,e.y+spot.y,isBoss(e)?11:7,0,TAU);ctx.stroke();ctx.beginPath();ctx.moveTo(e.x+spot.x-4,e.y+spot.y);ctx.lineTo(e.x+spot.x+4,e.y+spot.y);ctx.moveTo(e.x+spot.x,e.y+spot.y-4);ctx.lineTo(e.x+spot.x,e.y+spot.y+4);ctx.stroke();}
    ctx.restore();};
  ...
  function sanitizeSharedBuild(raw){
    ...
    clean.sourceTier=[0,1,2].includes(raw.sourceTier)?raw.sourceTier:2;return clean;
  }
  ...
  function showShare(fromPause=false,hallIndex=null,source='hall'){
    ...
    showOverlay(`<span class="eyebrow">BUILDS TEILEN</span><h2 id="overlayTitle">EIN CODE FÜR DEINEN BUILD.</h2><p>Erzeuge einen Code für deinen aktuellen Run oder einen Hall-of-Fame-Build. Der Empfänger kann ihn ansehen und mit deiner Erlaubnis ausschließlich im freigeschalteten Endlosmodus der passenden Schwierigkeit spielen.</p>${!fromPause&&hallOfFame.length?`<label class="share-label" for="shareSource">BUILD AUS DER HALL OF FAME</label><select id="shareSource" class="share-input">${hallOfFame.map((r,i)=>`<option value="${i}" ${r===entry?'selected':''}>Run ${hallOfFame.length-i} · ${hallDifficulty(r).toUpperCase()} · ${tierName(hallTier(r))} · ${escapeHTML(runDate(r))}</option>`).join('')}</select>`:''}${p?`<div class="share-export"><label><input id="sharePlayable" type="checkbox"> Empfänger darf den Build im Endlosmodus spielen</label><textarea id="shareOutput" class="share-input" readonly aria-label="Exportierter Build-Code"></textarea><button class="secondary-button" id="shareCopy">CODE KOPIEREN</button></div>`:'<p>Noch kein abgeschlossener Hall-of-Fame-Build vorhanden. Du kannst trotzdem einen fremden Code ansehen.</p>'}<div class="share-export"><label class="share-label" for="shareInput">CODE IMPORTIEREN</label><textarea id="shareInput" class="share-input" maxlength="40000" placeholder="SN3-1-…"></textarea><button class="primary-button" id="shareImport">BUILD ANSEHEN ↗</button></div><div id="sharePreview" aria-live="polite"></div><button class="secondary-button" id="shareBack">← ZURÜCK</button>`);
    const refresh=()=>{if(p){const compact=compactSharedBuild(p,difficulty());compact.sourceTier=fromPause?(endlessMode?endlessBuildTier:gamePlusLevel):hallTier(entry);$('shareOutput').value=encodeShareCode(compact,difficulty(),$('sharePlayable').checked);}};
    ...
    $('shareImport').onclick=()=>{let parsed;try{parsed=decodeShareCode($('shareInput').value);}catch(err){$('sharePreview').innerHTML=`<p class="share-error">${escapeHTML(err.message)}</p>`;return;}
      const view=restoreBuildValues(parsed.build),allowed=parsed.allowUse&&canStartMode(3,parsed.difficulty!=='normal',parsed.build,parsed.difficulty==='impossible');
      $('sharePreview').innerHTML=`<div class="share-preview"><h3>IMPORTIERTER BUILD · ${parsed.difficulty.toUpperCase()}</h3><p>${!parsed.allowUse?'Dieser Code ist nur zum Anschauen freigegeben.':allowed?'Nutzung im Endlosmodus erlaubt.':'Nutzung erlaubt; der Endlosmodus dieser Schwierigkeit ist noch gesperrt. Du kannst den Build ansehen.'}</p>${allowed?'<button class="primary-button" id="shareStart">MIT DIESEM BUILD STARTEN · ENDLOS ↗</button>':''}${buildDetailsMarkup(view,false)}</div>`;
      bindBuildJumps();if(allowed)$('shareStart').onclick=()=>requestRunStart(3,parsed.difficulty!=='normal',parsed.build,parsed.difficulty==='impossible');
    };
  ...
  const categorizedRunDetail=showRunDetail;
  showRunDetail=function(index,source='hall'){
    const r=hallOfFame[index];if(!r)return;categorizedRunDetail(index,source);
    const category=`${hallDifficulty(r)}:${hallTier(r)}`,$start=$('runDetailStart'),hero=characterRoster[hallBuild(r).characterId]?.name||'SNICKERS';
    $('overlayTitle').textContent=`${hallDifficulty(r).toUpperCase()} · ${tierName(hallTier(r))} · RUN ${hallOfFame.length-index}`;
    const eyebrow=$('overlayContent').querySelector('.run-detail-header .eyebrow');if(eyebrow)eyebrow.textContent=`HALL OF FAME · ${hallDifficulty(r).toUpperCase()} · ${tierName(hallTier(r))} · ${hero}`;
    if(source==='campaign'){
      const choice=campaignChoice;
      $start.textContent=`MIT DIESEM BUILD ${tierName(choice.level)} STARTEN ↗`;
      $start.onclick=()=>requestRunStart(choice.level,choice.hard,hallBuild(r),choice.impossible);
      $('runDetailBack').onclick=()=>showCampaignBuildSelect(choice.level,choice.hard,choice.impossible);
    }else if(source==='hall'){
      $('runDetailBack').onclick=()=>showHallOfFame(hallCategory);
      $start.textContent=`IN ${hallDifficulty(r).toUpperCase()}-ENDLOS STARTEN ↗`;
    }
    if(source==='campaign'||source==='hall'){
      const ids=(source==='campaign'?campaignSources(campaignChoice.level,campaignChoice.hard,campaignChoice.impossible):hallOfFame.map((entry,i)=>({r:entry,i})).filter(({r:entry})=>`${hallDifficulty(entry)}:${hallTier(entry)}`===category)).map(x=>x.i),pos=ids.indexOf(index);
      $('runPrev').disabled=pos<=0;$('runNext').disabled=pos>=ids.length-1;
      $('runPrev').onclick=()=>{if(pos>0)showRunDetail(ids[pos-1],source);};$('runNext').onclick=()=>{if(pos<ids.length-1)showRunDetail(ids[pos+1],source);};
    }
    if(source==='hall'){
      $('runDetailShare').insertAdjacentHTML('afterend','<button class="secondary-button hall-delete-button" id="runDetailDelete">DIESEN BUILD LÖSCHEN</button>');
      $('runDetailDelete').onclick=()=>confirmHallDeletion(index);
    }
  };
  function retainHallUnlocks(r){
    const difficulty=hallDifficulty(r),level=hallTier(r);
    if(difficulty==='normal'){
      ngPlusEverUnlocked=true;ngPlusUnlocked=true;
      if(level>=1){ngPlus2EverUnlocked=true;ngPlus2Unlocked=true;}
      try{localStorage.setItem('snickers3-ngplus-ever-v1','1');if(level>=1)localStorage.setItem('snickers3-ngplus2-ever-v1','1');if(level===2)localStorage.setItem('snickers3-normal-ng2-clear-v1','1');}catch{}
    }else if(difficulty==='hard'){
      hardProgress.unlocked=true;hardProgress.ngplus=true;
      if(level>=1)hardProgress.ngplus2=true;
      if(level===2){hardProgress.endless=true;impossibleProgress.unlocked=true;saveImpossibleProgress();}
      saveHardProgress();
    }else{
      impossibleProgress.unlocked=true;impossibleProgress.ngplus=true;
      if(level>=1)impossibleProgress.ngplus2=true;
      if(level===2)impossibleProgress.clear=true;
      saveImpossibleProgress();
    }
  }
  function forgetDeletedCachedBuild(r){
    const same=build=>build&&r.build&&JSON.stringify(build)===JSON.stringify(r.build),level=hallTier(r);
    if(hallDifficulty(r)==='normal'){
      if(level===0&&same(ngPlusBuild)){ngPlusBuild=null;try{localStorage.removeItem('snickers3-ngplus-build-v1');}catch{}}
      if(level===1&&same(ngPlus2Build)){ngPlus2Build=null;try{localStorage.removeItem('snickers3-ngplus2-build-v1');}catch{}}
    }else{
      const progress=hallDifficulty(r)==='impossible'?impossibleProgress:hardProgress;
      if(level===0&&same(progress.ngBuild))progress.ngBuild=null;
      if(level===1&&same(progress.ng2Build))progress.ng2Build=null;
      if(hallDifficulty(r)==='impossible')saveImpossibleProgress();else saveHardProgress();
    }
  }
  function confirmHallDeletion(index){
    if(mode!=='hallDetail'||!hallOfFame[index])return;
    const record=hallOfFame[index],category=`${hallDifficulty(record)}:${hallTier(record)}`;
    mode='hallDeleteConfirm';
    showOverlay(`<span class="eyebrow">HALL OF FAME · BUILD ENTFERNEN</span><h2 id="overlayTitle">DIESEN RUN LÖSCHEN?</h2><p><strong>${hallDifficulty(record).toUpperCase()} · ${tierName(hallTier(record))} · ${escapeHTML(runDate(record))}</strong></p><p>Der Hall-Eintrag und sein Build werden von diesem Gerät gelöscht. Du kannst ihn danach weder für New Game+ noch für Endlos oder als Hasenbein-Gegner wählen. Bereits freigeschaltete Modi, Skins und Achievements bleiben erhalten; laufende Runs und zuvor geteilte Codes bleiben bestehen.</p><div class="overlay-actions"><button class="secondary-button" id="cancelHallDeletion">ABBRECHEN</button><button class="secondary-button hall-delete-button" id="confirmHallDeletion">BUILD ENDGÜLTIG LÖSCHEN</button></div>`);
    $('cancelHallDeletion').onclick=()=>showRunDetail(index,'hall');
    $('confirmHallDeletion').onclick=()=>{
      if(mode!=='hallDeleteConfirm'||hallOfFame[index]!==record)return;
      const remaining=hallOfFame.filter(entry=>entry!==record);
      try{localStorage.setItem('snickers3-hall-of-fame-v1',JSON.stringify(remaining));}
      catch{toast('Der Hall-Eintrag konnte nicht gelöscht werden.');return;}
      retainHallUnlocks(record);forgetDeletedCachedBuild(record);hallOfFame=remaining;updateNGPlusMenu();
      mode='hallDetail';if(remaining.length||endlessHallOfFame.length)showHallOfFame(remaining.length?category:'endless');else goMenu();
    };
  }
  function showEndlessHallDetail(index){
    const r=endlessHallOfFame[index];if(!r)return;mode='hallDetail';
    const view=restoreBuildValues(hallBuild(r)),difficulty=hallDifficulty(r).toUpperCase(),hero=characterRoster[view.characterId]?.name||'SNICKERS';
    showOverlay(`<div class="run-detail-header"><span class="eyebrow">HALL OF FAME · ENDLOSMODUS · ${difficulty} · ${hero}</span><h2 id="overlayTitle">WELLE ${r.endlessWave} · ${difficulty}</h2><p>${escapeHTML(runDate(r))} · ${displayNumber(r.score)} Punkte · ${displayNumber(r.kills)} Gegner · ${formatTime(r.time||0)} · ${r.voluntary?'freiwillig beendet':'im Kampf gefallen'}</p><p>Dieser Build ist nur als Gegner im Hasenbein-Modus spielbar.</p></div><div class="run-detail-actions"><button class="secondary-button" id="runDetailBack">← ENDLOSRUNS</button><button class="secondary-button hall-delete-button" id="runDetailDelete">DIESEN BUILD LÖSCHEN</button></div>${buildDetailsMarkup(view,false)}<div class="run-detail-nav"><button class="secondary-button" id="runPrev" ${index===0?'disabled':''}>← NEUERER RUN</button><button class="secondary-button" id="runNext" ${index===endlessHallOfFame.length-1?'disabled':''}>ÄLTERER RUN →</button></div>`);
    $('runDetailBack').onclick=()=>showHallOfFame('endless');$('runDetailDelete').onclick=()=>confirmEndlessHallDeletion(index);
    $('runPrev').onclick=()=>showEndlessHallDetail(index-1);$('runNext').onclick=()=>showEndlessHallDetail(index+1);bindBuildJumps();
  }
  function confirmEndlessHallDeletion(index){
    const r=endlessHallOfFame[index];if(mode!=='hallDetail'||!r)return;mode='hallDeleteConfirm';
    showOverlay(`<span class="eyebrow">HALL OF FAME · ENDLOS-BUILD LÖSCHEN</span><h2 id="overlayTitle">DIESEN RUN LÖSCHEN?</h2><p>${hallDifficulty(r).toUpperCase()} · Welle ${r.endlessWave} · ${escapeHTML(runDate(r))}. Der Build steht anschließend auch als Hasenbein-Gegner nicht mehr bereit.</p><div class="overlay-actions"><button class="secondary-button" id="cancelHallDeletion">ABBRECHEN</button><button class="secondary-button hall-delete-button" id="confirmHallDeletion">BUILD ENDGÜLTIG LÖSCHEN</button></div>`);
    $('cancelHallDeletion').onclick=()=>{mode='hallDetail';showEndlessHallDetail(index);};
    $('confirmHallDeletion').onclick=()=>{
      if(mode!=='hallDeleteConfirm'||endlessHallOfFame[index]!==r)return;
      const remaining=endlessHallOfFame.filter(entry=>entry!==r);
      try{localStorage.setItem('snickers3-endless-hall-v1',JSON.stringify(remaining));}catch{toast('Der Endlos-Build konnte nicht gelöscht werden.');return;}
      endlessHallOfFame=remaining;mode='hallDetail';updateNGPlusMenu();if(remaining.length||hallOfFame.length)showHallOfFame('endless');else goMenu();
    };
  }
  ...
  function syncBossCelebration(){
    ...
    const hero=heroStory[player.characterId];
    $('loadingNotice').classList.add('hidden');$('waveLabel').textContent='BOSS BESIEGT';$('waveObjective').textContent=hero?.dance||'SCHNIEDEL PILLY PILLY';
    const speaking=d.duration-victoryTime>=BOSS_DANCE_SECONDS;
    $('bossDanceEyebrow').textContent=speaking?(characterRoster[player.characterId||'snickers'].name+' RUFT:'):'BOSS BESIEGT · '+(hero?.dance||'TANZFLÄCHE FREI');
    const line=speaking?'„'+d.quote+'“':hero?`${characterRoster[player.characterId].name} tanzt ${hero.dance}. Der Boss bleibt liegen.`:'Die Pfoten fliegen. Der Boss liegt. Snickers dreht auf!';
  ...
  drawHamster=function(p,alpha=1){
    if(p?._dancePortrait){eichelkopfHamster(p,alpha);return;}
    const d=mode==='victory'&&player?.bossDance;
    ...
    const actor=p.characterId||'snickers';
    const jump=reducedMotion?Math.sin(t*3)*2:-Math.abs(Math.sin(t*(dancing?(actor==='slanny'?16:actor==='krustenbraten'?7:12):5)))*(dancing?(actor==='slanny'?39:actor==='krustenbraten'?12:24):7);
    const spin=reducedMotion?0:dancing?(actor==='raffzahn'?t*TAU/1.05:actor==='slanny'?Math.sin(t*13)*.27:actor==='koettitroeter'?Math.sin(t*7)*.18:actor==='krustenbraten'?Math.sin(t*6)*.15:Math.sin(t*11)*.35+t*TAU/1.2):Math.sin(t*5)*.07;
    ...
    const kick=Math.sin(t*(actor==='krustenbraten'?10:actor==='koettitroeter'?14:18))*(reducedMotion?3:actor==='slanny'?14:9);
    ctx.strokeStyle=characterRoster[actor]?.color||'#b9763c';ctx.lineWidth=7;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(-13,2);ctx.lineTo(-26,-12-kick);ctx.moveTo(13,2);ctx.lineTo(26,-12+kick);ctx.moveTo(-8,14);ctx.lineTo(-17-kick,23);ctx.moveTo(8,14);ctx.lineTo(17-kick,23);ctx.stroke();
    drawHamster({...p,_dancePortrait:true,x:0,y:0,face:1,angle:-.35+Math.sin(t*8)*.6,invuln:0,shieldReady:false,dashTime:0},1);
  ...
  // Hasenbein is an isolated duel. Both saved run slots survive entering and leaving it.
  ...
  let hasenbeinGuardCd=0,hasenbeinGuardTime=0,hasenbeinCallCd=0,hasenbeinRamAutoAim=false,hasenbeinIdleTime=0;
  ...
  updateNGPlusMenu=function(){hasenbeinMenuUpdate();$('hasenbeinButton').classList.toggle('hidden',!hasenbeinUnlocked());};
  const endlessHallMenuUpdate=updateNGPlusMenu;
  updateNGPlusMenu=function(){endlessHallMenuUpdate();$('hallOfFameButton').classList.toggle('hidden',!hallOfFame.length&&!endlessHallOfFame.length);};
  function showHasenbeinSelect(){
    if(!hasenbeinUnlocked()||!['menu','hasenbeinResult','hasenbeinSelect'].includes(mode))return;
    mode='hasenbeinSelect';
    const opponents=[...hallOfFame,...endlessHallOfFame];
    showOverlay(`<span class="eyebrow">EINE STAGE · DIE RACHE DES HASENBEIN</span><h2 id="overlayTitle">JETZT JAGT DER HASE.</h2><p>Spiele General Hasenbein gegen einen abgeschlossenen Figuren-Build aus deiner Hall of Fame, auch gegen gespeicherte Endlosläufe. Du steuerst seine Möhrensalven, Bodenbeben, Hasenhelfer und den angekündigten Rammangriff. Hasenbein erhält drei bis fünf eigene Lebensphasen; sein Gegner hat einen Lebensbalken. WASD: bewegen · Q: gezielte Salve · E: Möhrenorkan · R: Erdbeben · Leertaste: Rammen · F: Felsdeckel zum Blocken · G: zwei Hasen rufen.</p>${opponents.length?`<div class="hall-run-list">${opponents.map((r,i)=>`<button class="run-card hasenbein-card" id="hasenbeinRun${i}"><span class="run-card-medal">♞</span><span class="run-card-main"><strong>${characterRoster[hallBuild(r).characterId]?.name||'SNICKERS'} · ${hasenbeinDifficultyOf(r).toUpperCase()} · ${r.endlessMode?'ENDLOS · WELLE '+r.endlessWave:tierName(hallTier(r))}</strong><small>${escapeHTML(runDate(r))} · ${Object.keys(hallBuild(r).skills||{}).length} Skills · ${hallBuild(r).weapons?.length||1} Waffen</small></span><span class="run-card-arrow">↗</span></button>`).join('')}</div>`:'<p>Schließe einen Kampagnen-Run ab, damit sein Build in der Hall of Fame erscheint.</p>'}<button class="secondary-button" id="hasenbeinBack">← ZURÜCK ZUM MENÜ</button>`);
    opponents.forEach((r,i)=>{$('hasenbeinRun'+i).onclick=()=>startHasenbeinDuel(i);});
    $('hasenbeinBack').onclick=goMenu;
  }
  function startHasenbeinDuel(index){
    if(mode!=='hasenbeinSelect'||!hasenbeinUnlocked())return;
    const record=[...hallOfFame,...endlessHallOfFame][index];if(!record)return;
    ...
    hasenbeinPower=1+(hasenbeinBuildThreat-1)*.53;
    const factor=hasenbeinDifficulty==='impossible'?1.37+(hasenbeinBuildThreat-1)*.05:hasenbeinDifficulty==='hard'?1.20+(hasenbeinBuildThreat-1)*.06:1.34;
    hasenbeinMode=true;runSessionId=null;resetInput();clearTimeout(announcementTimer);
    $('touchControls').classList.add('hasenbein-touch');for(const id of ['touchShock','touchGuard','touchCall'])$(id).classList.remove('hidden');
    ...
    hasenbeinRival=createPlayer();hasenbeinRival.characterId=build.characterId||'snickers';
    const rivalHp=clamp((425+Math.sqrt(Math.max(1,build.maxHp||5)*Math.max(1,build.damage||1))*29+(build.weapons?.length||1)*22)*factor+170*Math.pow(Math.max(0,hasenbeinBuildThreat-2),1.4),465,2200);
    Object.assign(hasenbeinRival,{x:W*.76,y:H*.56,r:23,hp:rivalHp,speed:clamp((build.speed||264)*.7,165,252),invuln:.7,phase:0,fireCd:1.15,patternCd:4.6,dodgeCd:3.8,companionCd:4.5,shieldCd:0,lifeStealCd:0,charge:null,weaponCount:build.weapons?.length||1,angle:Math.PI,face:-1,moving:true,damage:clamp(Math.sqrt((build.damage||1)/1.4)*(build.madelpulator?1.12:1),1,2.8),fireRate:clamp(build.fireRate||.32,.08,.55),dodgeChance:clamp(build.dodgeChance||0,0,.22),damageGuard:clamp(build.damageGuard||0,0,.35),crit:clamp(build.crit||0,0,.45),shieldReady:Boolean(build.shield),shieldUsed:false,madelpulator:Boolean(build.madelpulator),nutFan:Boolean(build.nutFan||build.spread),rearNut:Boolean(build.rearNut),orbit:Boolean(build.orbit),sentry:Boolean(build.nutSentry),frost:Boolean(build.frost),vampire:Boolean(build.vampire),companionCount:['bruno','narrath','knisterKnight','wollenkamps','erweinLewy','soapedRat','daimDuo','pigPatrol','ronnySquirrel'].filter(k=>build[k]||build.skills?.[k]).length});
    hasenbeinRival.maxHp=hasenbeinRival.hp;
    ...
    updateHasenbeinHud();const rivalName=characterRoster[hasenbeinRival.characterId]?.name||'SNICKERS';showOverlay(`<span class="eyebrow">HASENBEIN MODUS · ${hasenbeinDifficulty.toUpperCase()}</span><h2 id="overlayTitle">DIE RACHE DES GENERAL HASENBEIN.</h2><p>${rivalName} hat ${Object.keys(build.skills||{}).length} Skills und ${build.weapons?.length||1} Waffen. Hasenbeins Leben pro Phase und seine Bossangriffe wachsen mit diesem Build; ${rivalName} behält starke Angriffe. Du hast ${hasenbeinMaxPhases} Phasen gegen einen einzigen Lebensbalken. In der letzten Phase eilt Karnil zu Hilfe.</p><p>Automatische Bossangriffe halten ${rivalName} in Bewegung. Deinen Rammangriff löst nur die Leertaste aus. <strong>WASD</strong> bewegen · <strong>Q</strong> gezielte Salve · <strong>E</strong> Möhrenorkan · <strong>R</strong> angekündigtes Erdbeben · <strong>SPACE</strong> Rammen · <strong>F</strong> Felsdeckel: kurz schützen und Schüsse abwehren · <strong>G</strong> zwei Hasen rufen. Auf Touch gibt es für jede Aktion eine Taste.</p><button class="primary-button" id="hasenbeinGo">REVANCHE STARTEN ↗</button>`);
    $('hasenbeinGo').onclick=()=>{mode='playing';hideOverlay();lastFrame=performance.now();};
  }
  function updateHasenbeinHud(){
    if(!hasenbeinMode||!hasenbeinRival)return;
    $('hasenbeinHealth').textContent=`GENERAL HASENBEIN · PHASE ${hasenbeinPhase}/${hasenbeinMaxPhases} · ${Math.max(0,player.hp).toFixed(1)} / ${player.maxHp} ♥`;
    $('hasenbeinHealthFill').style.width=clamp(player.hp/player.maxHp*100,0,100)+'%';
    $('hasenbeinRivalHealth').textContent=`${characterRoster[hasenbeinRival.characterId||'snickers'].name} · ${Math.ceil(Math.max(0,hasenbeinRival.hp))} / ${Math.ceil(hasenbeinRival.maxHp)}`;
    $('hasenbeinRivalFill').style.width=clamp(hasenbeinRival.hp/hasenbeinRival.maxHp*100,0,100)+'%';
    const ally=$('hasenbeinKarnilStatus');ally.textContent=hasenbeinKarnil?(hasenbeinKarnil.hp>0?`KARNIL HILFT DIR · ${Math.ceil(hasenbeinKarnil.hp)} / ${hasenbeinKarnil.maxHp} ♥`:'KARNIL IST ERSCHÖPFT'):'';ally.classList.toggle('hidden',!hasenbeinKarnil);
    ...
    $('hasenbeinDefense').textContent=`F FELSDECKEL ${hasenbeinGuardTime>0?'AKTIV':ready(hasenbeinGuardCd)} · G HASENRUF ${ready(hasenbeinCallCd)}`;
  }
  function hasenbeinShot(x,y,a,speed,damage,owner,homing=false){hasenbeinShots.push({x,y,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,damage,owner,homing,r:owner==='hasenbein'?9:7,life:2.8});}
  function hasenbeinHitRival(damage){
    const rival=hasenbeinRival;if(!rival||rival.invuln>0||Math.random()<rival.dodgeChance)return;
    if(rival.shieldReady&&!rival.shieldUsed){damage*=.35;rival.shieldUsed=true;rival.shieldCd=15;burst(rival.x,rival.y,'#a1e5bc',8,80);}
    if(rival.damageGuard&&Math.random()<rival.damageGuard)damage*=.65;
    rival.hp-=damage;rival.hit=.16;
  }
  function hasenbeinHurt(damage){
    if(player.invuln>0)return;
    const guarded=hasenbeinGuardTime>0;
    player.hp-=guarded?damage*.38:damage;player.invuln=guarded?.28:.45;
    burst(player.x,player.y,guarded?'#f7d691':'#eabc89',7,65);
  }
  function hasenbeinSalvo(){
    if(!hasenbeinMode||mode!=='playing'||hasenbeinFanCd>0)return;
    hasenbeinFanCd=4.3;const a=Math.atan2(hasenbeinRival.y-player.y,hasenbeinRival.x-player.x),count=hasenbeinPhase>=2?7:5;
    for(let i=0;i<count;i++)hasenbeinShot(player.x,player.y,a+(i-(count-1)/2)*.14,hasenbeinPhase>=2?320:265,hasenbeinDamage(7.2),'hasenbein',i===(count-1)/2);
  ...
  function hasenbeinEarthquake(){
    ...
    hasenbeinHazards.push({x:rival.x,y:rival.y,r:hasenbeinPhase>=2?147:128,wait:1.05,track:.82,life:.35,hit:false,damage:hasenbeinDamage(15)});
  ...
  function summonHasenbeinBunnies(count,limit=8){
    const total=hasenbeinPhase>=2?4:3;
    for(let i=0;i<count&&hasenbeinMinions.length<limit;i++){
      const a=TAU*(i+hasenbeinMinions.length)/Math.max(3,total);
      hasenbeinMinions.push({x:clamp(player.x+Math.cos(a)*75,38,W-38),y:clamp(player.y+Math.sin(a)*75,145,H-50),r:18,hp:3,type:i%3===2?'runner':'bunny',phase:0,hit:0,fireCd:.6+i*.3});
    }
    burst(player.x,player.y,'#ebd4ab',12,90);
  }
  function hasenbeinGuard(){
    if(!hasenbeinMode||mode!=='playing'||hasenbeinGuardCd>0)return;
    hasenbeinGuardCd=10.5;hasenbeinGuardTime=1.15;
    let intercepted=0;
    for(const shot of hasenbeinShots){if(shot.owner==='snickers'&&shot.life>0&&dist(shot,player)<155&&intercepted<6){shot.life=0;intercepted++;}}
    if(intercepted)hasenbeinHitRival(hasenbeinDamage(Math.min(6,intercepted*1.6)));
    burst(player.x,player.y,'#eed6a0',17,140);tone(205,.18,'triangle',.05,100);updateHasenbeinHud();
  }
  function hasenbeinCall(){
    if(!hasenbeinMode||mode!=='playing'||hasenbeinCallCd>0||hasenbeinMinions.length>=6)return;
    hasenbeinCallCd=13;const before=hasenbeinMinions.length;summonHasenbeinBunnies(2,6);
    if(hasenbeinMinions.length>before)tone(480,.14,'square',.035,350);
    updateHasenbeinHud();
  }
  function hasenbeinJump(){
    if(!hasenbeinMode||mode!=='playing'||hasenbeinDashCd>0||hasenbeinWindup>0)return;
    // Moving players can aim the charge without releasing WASD or the touch stick.
    hasenbeinRamAutoAim=true;
    hasenbeinRamAngle=Math.atan2(hasenbeinRival.y-player.y,hasenbeinRival.x-player.x);
    hasenbeinWindup=.5;hasenbeinDashHit=false;hasenbeinDashCd=6.4;
    player.invuln=Math.max(player.invuln,.64);
    tone(180,.15,'sawtooth',.035,440);updateHasenbeinHud();
  }
  function nextHasenbeinPhase(){
    if(hasenbeinPhase>=hasenbeinMaxPhases){endHasenbeinDuel(false);return;}
  ...
  function hasenbeinCinematicArt(victory){
    const rival=characterRoster[hasenbeinRival?.characterId]||characterRoster.snickers;
    const rivalFace=hasenbeinRival?.characterId==='raffzahn'?'<path d="M-32 11q-52-4-39 31" fill="none" stroke="#d6a6bb" stroke-width="6"/><ellipse cx="-18" cy="-25" rx="10" ry="15" fill="#d4b8cc"/><ellipse cx="19" cy="-25" rx="10" ry="15" fill="#d4b8cc"/>':hasenbeinRival?.characterId==='krustenbraten'?'<path d="M-25-14l-8-25 22 17m24 0 22-17-8 25" fill="#f2b5a6"/><ellipse cy="3" rx="17" ry="10" fill="#f2b5a6"/><circle cx="-6" cy="3" r="2"/><circle cx="6" cy="3" r="2"/>':hasenbeinRival?.characterId==='slanny'?'<path d="M-17-23l-9-55q-12-16-11 5l2 52m49-2 9-55q12-16 11 5l-2 52" fill="#d5daa9" stroke="#f3aeb7" stroke-width="3"/>':hasenbeinRival?.characterId==='koettitroeter'?'<path d="M22 3l44-5v14l-44-5" fill="#e9d997"/><circle cx="67" cy="5" r="9" fill="#e9d997"/>':'';
    return `<div class="hasenbein-cinematic ${victory?'hasenbein-campfire':'hasenbein-arrival'}" role="group" aria-label="${victory?'Hasenbein und Karnil jubeln am Lagerfeuer; '+rival.name+' hängt als Cartoonfigur über den Flammen':'Karnil eilt Hasenbein gegen '+rival.name+' zur Hilfe'}">
      <svg viewBox="0 0 840 315" role="img" aria-label="${victory?'Hasenbein und Karnil feiern mit '+rival.name+' am Grillspieß':'Hasenbein und Karnil stellen sich gemeinsam '+rival.name}">
        ...
        ${victory?`<ellipse cx="632" cy="237" rx="144" ry="72" fill="url(#hbGlow)" class="hb-fire-glow"/>
          ...
          <g transform="translate(642 122)" class="hb-roasting"><ellipse cy="12" rx="29" ry="31" fill="${rival.color}"/><path d="M-23-14l-10-27 23 11m20 0 23-11-10 27" fill="${rival.color}"/><ellipse cy="-5" rx="24" ry="20" fill="${rival.color}"/>${rivalFace}<circle cx="-8" cy="-8" r="3" fill="#352f35"/><circle cx="9" cy="-8" r="3" fill="#352f35"/><path d="M-6 5q7-7 15 0" fill="none" stroke="#513633" stroke-width="3"/><path d="M-32 15l-13 16m77-16 13 16" stroke="${rival.color}" stroke-width="8" stroke-linecap="round"/></g>
          ...
          <g transform="translate(717 203)" class="hb-snickers-run"><ellipse cy="12" rx="27" ry="30" fill="${rival.color}"/><ellipse cy="-11" rx="25" ry="22" fill="${rival.color}"/><path d="M-18-29l-7-24 17 17m21-2 18-19-4 29" fill="${rival.color}"/>${rivalFace}<circle cx="-8" cy="-14" r="3"/><circle cx="9" cy="-14" r="3"/><ellipse cy="-3" rx="6" ry="4" fill="#a75e57"/><path d="M-20 31l-19 16m54-18 21 10" stroke="${rival.color}" stroke-width="8" stroke-linecap="round"/></g>`}
  ...
  function startHasenbeinKarnilIntro(){
    ...
    const hp=Math.round(65+(hasenbeinBuildThreat-1)*22);
  ...
  function showHasenbeinCredits(){
    ...
    const rivalName=characterRoster[hasenbeinRival?.characterId]?.name||'SNICKERS',skinId=hasenbeinRival?.characterId&&hasenbeinRival.characterId!=='snickers'?`${hasenbeinRival.characterId}__hasenbeinVictor`:'hasenbeinVictor',rewardSkin=skins.find(s=>s.id===skinId)?.name||'HASENBEINS ALBTRAUM';showOverlay(`<span class="eyebrow">ABSPANN · HASENBEINS RACHE · ${hasenbeinDifficulty.toUpperCase()}</span><h2 id="overlayTitle">HEUTE JAGT DER HASE.</h2><p>Hasenbein hat ${rivalName} aus der Hall of Fame besiegt. Karnil bewacht das Feuer, und der General tanzt endlich seinen Siegestanz.</p>${hasenbeinCinematicArt(true)}<div class="hasenbein-credits"><span>IN DER HAUPTROLLE <strong>GENERAL HASENBEIN</strong></span><span>${hasenbeinKarnil?'RETTER IN LETZTER MINUTE':'ÜBERRASCHUNGSGAST AM FEUER'} <strong>KARNIL, DAS SAUFERKEL</strong></span><span>AM LAGERFEUER <strong>${rivalName}</strong></span><span>DAUER DER REVANCHE <strong>${formatTime(hasenbeinDuelTime)}</strong></span><span>FREIGESCHALTET <strong>${rewardSkin}</strong></span></div><div class="overlay-actions"><button class="primary-button" id="hasenbeinAgain">ANDEREN BUILD WÄHLEN ↗</button><button class="secondary-button" id="hasenbeinLeave">HAUPTMENÜ</button></div>`);
  ...
  function updateHasenbeinKarnil(dt){
    ...
    if(d<540&&k.shotCd<=0){k.shotCd=2.9;const a=Math.atan2(dy,dx);for(const off of [-.12,0,.12])hasenbeinShot(k.x,k.y,a+off,330,hasenbeinDamage(3.5),'hasenbein',off===0);burst(k.x,k.y,'#eab58b',5,65);}
    if(d<650&&k.rockCd<=0){k.rockCd=7.2;hasenbeinHazards.push({x:rival.x,y:rival.y,r:94,wait:.8,life:.35,hit:false,damage:hasenbeinDamage(8),karnil:true});}
    if(d<k.r+rival.r&&k.contactCd<=0){k.contactCd=1.5;hasenbeinHitRival(hasenbeinDamage(2.5));}
  }
  function endHasenbeinDuel(won){
    ...
    resetInput();if(won){
      ...
      showOverlay(`<span class="eyebrow">HASENBEIN MODUS · DER SIEG</span><h2 id="overlayTitle">HASENBEIN HAT SICH GERÄCHT!</h2>${hasenbeinCinematicArt(true)}<p class="hasenbein-scene-copy">Der General jubelt. Karnil schürt das Lagerfeuer und ${characterRoster[hasenbeinRival?.characterId]?.name||'SNICKERS'} schmort als Cartoonfigur am Grillspieß. Hasenbein hat seinen Erzfeind besiegt!</p><button class="secondary-button" id="hasenbeinSkipVictory">ZUM ABSPANN ↗</button>`);
    ...
    showOverlay(`<span class="eyebrow">HASENBEIN MODUS · ${hasenbeinDifficulty.toUpperCase()}</span><h2 id="overlayTitle">${characterRoster[hasenbeinRival?.characterId]?.name||'SNICKERS'} GEWINNT DIE REVANCHE.</h2><p>Hasenbein unterlag in seiner eigenen Phase ${hasenbeinPhase} von ${hasenbeinMaxPhases}. Wähle einen anderen Hall-Build oder versuche es erneut. Kampfzeit: ${formatTime(hasenbeinDuelTime)}. Kampagne und Endloslauf bleiben gespeichert.</p><div class="overlay-actions"><button class="primary-button" id="hasenbeinAgain">ANDEREN BUILD WÄHLEN ↗</button><button class="secondary-button" id="hasenbeinLeave">HAUPTMENÜ</button></div>`);
    $('hasenbeinAgain').onclick=showHasenbeinSelect;$('hasenbeinLeave').onclick=goMenu;
  }
  // Reverse of General Hasenbein's campaign encounter: the player owns the boss phases.
  // The chosen Hall of Fame Snickers retains one health bar for the whole duel.
  function updateHasenbein(dt){
    const rival=hasenbeinRival;if(!rival)return;
    hasenbeinDuelTime+=dt;music(dt);
    player.invuln=Math.max(0,player.invuln-dt);rival.invuln=Math.max(0,rival.invuln-dt);
    rival.shieldCd=Math.max(0,(rival.shieldCd||0)-dt);if(rival.shieldReady&&rival.shieldUsed&&rival.shieldCd===0)rival.shieldUsed=false;
    rival.lifeStealCd=Math.max(0,(rival.lifeStealCd||0)-dt);
    hasenbeinSpecialCd=Math.max(0,hasenbeinSpecialCd-dt);hasenbeinDashCd=Math.max(0,hasenbeinDashCd-dt);
    hasenbeinFanCd=Math.max(0,hasenbeinFanCd-dt);hasenbeinShockCd=Math.max(0,hasenbeinShockCd-dt);
    hasenbeinGuardCd=Math.max(0,hasenbeinGuardCd-dt);hasenbeinGuardTime=Math.max(0,hasenbeinGuardTime-dt);hasenbeinCallCd=Math.max(0,hasenbeinCallCd-dt);
    let mx=(keys.has('KeyD')||keys.has('ArrowRight')?1:0)-(keys.has('KeyA')||keys.has('ArrowLeft')?1:0)+touch.x;
    let my=(keys.has('KeyS')||keys.has('ArrowDown')?1:0)-(keys.has('KeyW')||keys.has('ArrowUp')?1:0)+touch.y;
    const len=Math.hypot(mx,my);if(len>1){mx/=len;my/=len;}player.moving=len>.1;
    hasenbeinIdleTime=len>.1||hasenbeinWindup>0||hasenbeinDashTime>0?Math.max(0,hasenbeinIdleTime-dt*2):Math.min(3,hasenbeinIdleTime+dt);
    if(hasenbeinWindup>0){hasenbeinWindup-=dt;player.moving=false;if(hasenbeinRamAutoAim&&hasenbeinWindup>.16)hasenbeinRamAngle=Math.atan2(rival.y-player.y,rival.x-player.x);if(hasenbeinWindup<=0){hasenbeinDashTime=.68;player.dashX=Math.cos(hasenbeinRamAngle);player.dashY=Math.sin(hasenbeinRamAngle);}}
    if(hasenbeinDashTime>0){
      hasenbeinDashTime=Math.max(0,hasenbeinDashTime-dt);
      if(hasenbeinRamAutoAim){const target=Math.atan2(rival.y-player.y,rival.x-player.x);hasenbeinRamAngle+=clamp(angleDelta(target,hasenbeinRamAngle),-dt*1.2,dt*1.2);player.dashX=Math.cos(hasenbeinRamAngle);player.dashY=Math.sin(hasenbeinRamAngle);}
      player.x+=player.dashX*555*dt;player.y+=player.dashY*555*dt;player.invuln=Math.max(player.invuln,.13);
    }
    ...
    if(hasenbeinDashTime>0&&!hasenbeinDashHit&&dist(player,rival)<player.r+rival.r+13){hasenbeinDashHit=true;hasenbeinHitRival(hasenbeinDamage(27+hasenbeinPhase*2));burst(rival.x,rival.y,'#eab46e',18,170);}
    if(dist(player,rival)<player.r+rival.r-8&&hasenbeinDashTime<=0)hasenbeinHurt(.65);

    // Boss pressure remains automatic, but only the player may order the ram.
    // It no longer interrupts movement or silently spends the SPACE cooldown.
    hasenbeinAttackCd-=dt;
    if(hasenbeinAttackCd<=0&&hasenbeinWindup<=0){
      ...
      if(hasenbeinAttackPattern===0){for(const off of [-.11,0,.11])hasenbeinShot(player.x,player.y,a+off,260,hasenbeinDamage(2.8),'hasenbein',off===0);}
      else if(hasenbeinAttackPattern===1){const count=enraged?18:13;for(let i=0;i<count;i++)hasenbeinShot(player.x,player.y,a+i*TAU/count,enraged?225:185,hasenbeinDamage(2.5),'hasenbein');}
      else if(hasenbeinAttackPattern===2){hasenbeinHazards.push({x:rival.x,y:rival.y,r:enraged?138:112,wait:1.05,life:.35,hit:false,damage:hasenbeinDamage(7)},{x:clamp(rival.x+rnd(-180,180),65,W-65),y:clamp(rival.y+rnd(-150,150),140,H-55),r:88,wait:1.45,life:.35,hit:false,damage:hasenbeinDamage(5)});}
      else {const count=enraged?7:5;for(let i=0;i<count;i++)hasenbeinShot(player.x,player.y,a+(i-(count-1)/2)*.14,enraged?275:220,hasenbeinDamage(3),'hasenbein');}
      hasenbeinAttackCd=enraged?1.34:1.74;
    }
    for(const hazard of hasenbeinHazards){
      hazard.wait-=dt;
      if(hazard.track>0&&hazard.wait>.22){const follow=Math.min(1,dt*4.5);hazard.x+=(rival.x-hazard.x)*follow;hazard.y+=(rival.y-hazard.y)*follow;hazard.track-=dt;}
      if(hazard.wait<=0&&!hazard.hit){hazard.hit=true;burst(hazard.x,hazard.y,'#e4b075',14,120);if(dist(hazard,rival)<hazard.r+rival.r)hasenbeinHitRival(hazard.damage);}
      if(hazard.hit)hazard.life-=dt;
    }
    hasenbeinHazards=hasenbeinHazards.filter(h=>h.life>0);

    // General Hasenbein summons three rabbits every nine seconds, four while enraged.
    hasenbeinSummonCd-=dt;
    if(hasenbeinSummonCd<=0){hasenbeinSummonCd=hasenbeinPhase>=2?6.5:9;summonHasenbeinBunnies(hasenbeinPhase>=2?4:3);}
    for(const bunny of hasenbeinMinions){bunny.phase+=dt*5;bunny.hit=Math.max(0,bunny.hit-dt);bunny.fireCd-=dt;
      ...
      if(d<280&&bunny.fireCd<=0){hasenbeinShot(bunny.x,bunny.y,Math.atan2(dy,dx),240,hasenbeinDamage(1.7),'hasenbein');bunny.fireCd=2.2;}
    }
    ...
    const ideal=260+Math.sin(hasenbeinDuelTime*.8)*35,dir=d<ideal-50?-1:d>ideal+50?1:0,strafe=Math.sin(hasenbeinDuelTime*1.9)>0?1:-1;
    rival.x=clamp(rival.x+(dx/d*dir*.75-dy/d*strafe*.55)*rival.speed*dt,65,W-65);
    rival.y=clamp(rival.y+(dy/d*dir*.75+dx/d*strafe*.55)*rival.speed*dt,157,H-65);
    rival.dodgeCd-=dt;
    if(rival.dodgeChance>0&&rival.dodgeCd<=0){rival.dodgeCd=3.8;rival.x=clamp(rival.x-dy/d*85,65,W-65);rival.y=clamp(rival.y+dx/d*85,157,H-65);burst(rival.x,rival.y,'#e4a86e',7,60);}
    rival.face=player.x<rival.x?-1:1;rival.angle=Math.atan2(player.y-rival.y,player.x-rival.x);rival.phase+=dt*8;rival.hit=Math.max(0,(rival.hit||0)-dt);
    rival.companionCd-=dt;
    if(rival.companionCount>0&&rival.companionCd<=0){
      rival.companionCd=Math.max(3.2,5.4-rival.companionCount*.22);
      const n=Math.min(3,rival.companionCount),a=Math.atan2(player.y-rival.y,player.x-rival.x);
      for(let i=0;i<n;i++)hasenbeinShot(rival.x+(i-1)*15,rival.y-30,a+(i-(n-1)/2)*.18,280+16*n,.46+(hardMode?.14:0),'snickers');
      burst(rival.x,rival.y-30,'#d8c495',5,55);
    }
    rival.fireCd-=dt;if(rival.fireCd<=0){
      ...
      for(let i=0;i<n;i++)hasenbeinShot(rival.x,rival.y,a+(i-(n-1)/2)*.20,345+15*rival.weaponCount,clamp(.62*rival.damage*(Math.random()<rival.crit?1.35:1)*(hasenbeinDifficulty==='impossible'?1.35:hasenbeinDifficulty==='hard'?1.15:1),.58,2.4),'snickers');
      if(rival.rearNut&&Math.random()<.18)hasenbeinShot(rival.x,rival.y,a+Math.PI,290,.55,'snickers');
      if(rival.orbit&&Math.random()<.28)for(let i=0;i<5;i++)hasenbeinShot(rival.x,rival.y,i*TAU/5+hasenbeinDuelTime,240,.7,'snickers');
      if(rival.sentry&&Math.random()<.25)hasenbeinShot(rival.x,rival.y-35,rival.angle,430,.95,'snickers');
    }
    rival.patternCd-=dt;if(rival.patternCd<=0&&!rival.charge){
      rival.charge={x:player.x,y:player.y,wait:1.05,r:hasenbeinDifficulty==='impossible'?112:88};
      rival.patternCd=((hasenbeinDifficulty==='impossible'?3.3:hasenbeinDifficulty==='hard'?3.9:4.7)-(rival.weaponCount>=6?.3:0))*(hasenbeinIdleTime>1.5?.76:1);
    }
    if(rival.charge){rival.charge.wait-=dt;if(rival.charge.wait<=0){
      const strike=rival.charge;rival.charge=null;burst(strike.x,strike.y,'#ef885a',24,150);
      if(dist(player,strike)<strike.r+player.r*.35)hasenbeinHurt(hasenbeinDifficulty==='impossible'?3.8:hasenbeinDifficulty==='hard'?2.95:2.4);
      if(rival.weaponCount>=4)for(let i=0;i<8;i++)hasenbeinShot(strike.x,strike.y,i*TAU/8,260,.75,'snickers');
    }}
    for(const s of hasenbeinShots){if(s.life<=0)continue;s.life-=dt;if(s.life<=0)continue;const before={x:s.x,y:s.y};if(s.homing&&s.owner==='hasenbein'){
      const speed=Math.hypot(s.vx,s.vy),angle=Math.atan2(s.vy,s.vx),target=Math.atan2(rival.y-s.y,rival.x-s.x);
      const next=angle+clamp(angleDelta(target,angle),-dt*1.1,dt*1.1);s.vx=Math.cos(next)*speed;s.vy=Math.sin(next)*speed;
      ...
      if(s.owner==='snickers'){
        ...
        if(minion){s.life=0;minion.hp-=1;minion.hit=.15;}
          ...
          s.life=0;hasenbeinKarnil.hp=Math.max(0,hasenbeinKarnil.hp-s.damage*.72);hasenbeinKarnil.hit=.2;
        ...
        else if(s.life>0&&segmentDistance(player,before,s)<player.r+s.r){s.life=0;const prior=player.hp;hasenbeinHurt(s.damage);if(rival.vampire&&player.hp<prior&&rival.lifeStealCd<=0){rival.lifeStealCd=18;rival.hp=Math.min(rival.maxHp,rival.hp+Math.min(8,rival.maxHp*.008));}}
      }else if(s.life>0&&segmentDistance(rival,before,s)<rival.r+s.r){s.life=0;hasenbeinHitRival(s.damage);}
    }
    hasenbeinMinions=hasenbeinMinions.filter(b=>b.hp>0);
    hasenbeinShots=hasenbeinShots.filter(s=>s.life>0&&s.x>0&&s.x<W&&s.y>110&&s.y<H);
    if(rival.hp<=0)endHasenbeinDuel(true);
    else if(player.hp<=0)nextHasenbeinPhase();
    else updateHasenbeinHud();
  }
  function renderHasenbein(){
    camX=clamp(player.x-viewW/2,0,Math.max(0,W-viewW));camY=clamp(player.y-viewH/2,0,Math.max(0,H-viewH));
    ctx.setTransform(dpr*scale,0,0,dpr*scale,-camX*dpr*scale,-camY*dpr*scale);ctx.drawImage(ground,0,0);
    if(hasenbeinRival.charge){const t=hasenbeinRival.charge;ctx.fillStyle='#d0493d44';ctx.strokeStyle='#ffab83';ctx.lineWidth=4;ctx.beginPath();ctx.arc(t.x,t.y,t.r,0,TAU);ctx.fill();ctx.stroke();}
    ...
    if(hasenbeinKarnil?.hp>0){
      ctx.save();ctx.strokeStyle='#f4c484';ctx.lineWidth=5;ctx.beginPath();ctx.ellipse(hasenbeinKarnil.x,hasenbeinKarnil.y+32,hasenbeinKarnil.r*.8,19,0,0,TAU);ctx.stroke();ctx.restore();
      drawRabbit({...hasenbeinKarnil,hasenbeinControlled:true,face:hasenbeinRival.x<hasenbeinKarnil.x?-1:1,dead:false});
      ctx.fillStyle='#352b2a';ctx.fillRect(hasenbeinKarnil.x-53,hasenbeinKarnil.y-119,106,8);
      ctx.fillStyle='#f7ca8d';ctx.fillRect(hasenbeinKarnil.x-52,hasenbeinKarnil.y-118,104*clamp(hasenbeinKarnil.hp/hasenbeinKarnil.maxHp,0,1),6);
    }
    ...
    if(hasenbeinGuardTime>0){ctx.save();ctx.fillStyle='#f7d99029';ctx.strokeStyle='#f7d990';ctx.lineWidth=5;ctx.beginPath();ctx.arc(player.x,player.y,player.r+16+Math.sin(ambientTime*13)*2,0,TAU);ctx.fill();ctx.stroke();ctx.restore();}
    for(const s of hasenbeinShots){ellipse(ctx,s.x,s.y,s.r,s.r,s.owner==='hasenbein'?'#dfad72':characterRoster[hasenbeinRival?.characterId||'snickers'].color);}
    if(hasenbeinDashTime>0){ctx.strokeStyle='#f4ce81';ctx.lineWidth=5;ctx.beginPath();ctx.arc(player.x,player.y,player.r+12,0,TAU);ctx.stroke();}
    for(const p of particles){ctx.save();ctx.globalAlpha=clamp(p.life/p.maxLife,0,1);if(p.type==='beam'){ctx.strokeStyle=p.color;ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(p.x2,p.y2);ctx.stroke();}else if(p.type==='ring'){ctx.strokeStyle=p.color;ctx.lineWidth=4;ctx.beginPath();ctx.arc(p.x,p.y,180*(1-p.life/p.maxLife),0,TAU);ctx.stroke();}else ellipse(ctx,p.x,p.y,p.r,p.r*.8,p.color);ctx.restore();}
    for(const f of floaters){ctx.save();ctx.globalAlpha=Math.min(1,f.life*2);ctx.font='bold 16px Barlow, Arial';ctx.textAlign='center';ctx.fillStyle=f.color;ctx.fillText(f.text,f.x,f.y);ctx.restore();}
  }
  const hasenbeinOriginalUpdate=update,hasenbeinOriginalRender=render,hasenbeinOriginalDash=useDash,hasenbeinOriginalWeapon=useWeapon,hasenbeinOriginalCycle=cycleWeapon,hasenbeinOriginalMenu=goMenu,hasenbeinOriginalPause=pauseGame;
  update=function(dt){if(hasenbeinMode){if(mode==='playing')updateHasenbein(dt);return;}hasenbeinOriginalUpdate(dt);};
  render=function(dt){if(hasenbeinMode&&hasenbeinRival){renderHasenbein();return;}hasenbeinOriginalRender(dt);};
  useDash=function(){if(hasenbeinMode){hasenbeinJump();return;}hasenbeinOriginalDash();};
  useWeapon=function(){if(hasenbeinMode){hasenbeinOrkan();return;}hasenbeinOriginalWeapon();};
  ...
  goMenu=function(){
    if(hasenbeinMode){
      hasenbeinMode=false;hasenbeinKarnil=null;hasenbeinCinematicTime=0;hasenbeinGuardTime=0;runSessionId=null;
      $('hasenbeinHud').classList.add('hidden');$('touchControls').classList.remove('hasenbein-touch');
      for(const id of ['touchShock','touchGuard','touchCall'])$(id).classList.add('hidden');
      $('touchCycle').innerHTML='WAFFE<span>↻</span>';$('touchCycle').setAttribute('aria-label','Spezialwaffe wechseln');
      $('touchBomb').innerHTML='NUSS<span>E</span>';$('touchBomb').setAttribute('aria-label','Nussbombe');
      $('touchDash').innerHTML='DASH<span>↗</span>';$('touchDash').setAttribute('aria-label','Ausweichen');
    }
    hasenbeinOriginalMenu();
  };
  pauseGame=function(){
    if(!hasenbeinMode){hasenbeinOriginalPause();return;}
    if(mode!=='playing'&&mode!=='paused')return;
    previousMode='playing';mode='paused';resetInput();
    showOverlay(`<span class="eyebrow">HASENBEINS PAUSE</span><h2 id="overlayTitle">${characterRoster[hasenbeinRival?.characterId]?.name||'SNICKERS'} WARTET.</h2><p>Q: gezielte Salve · E: Möhrenorkan · R: Erdbeben · SPACE: Rammen mit kurzer Vorbereitung · F: Felsdeckel blockt für 1,15 s einen Teil des Schadens und fängt bis zu sechs Schüsse ab · G: zwei Hasen rufen. Du kämpfst gegen einen ${hasenbeinDifficulty.toUpperCase()}-Hall-of-Fame-Build. Kampagnen- und Endlosspeicher bleiben erhalten.</p><div class="overlay-actions"><button class="primary-button" id="hasenbeinResume">WEITERKÄMPFEN ↗</button><button class="secondary-button" id="hasenbeinExit">HAUPTMENÜ</button></div>`);
    $('hasenbeinResume').onclick=resumeGame;$('hasenbeinExit').onclick=goMenu;
  };

  // Companion evolutions are run skills: each card requires its visible base actor.
  // The five ranks are independent choices, so no particular order is mandatory.
  const companionGroups={
    bruno:{flag:'bruno',cd:'brunoCd',icon:'▰',bonus:.36,names:['BRUNOS SCHNELLER AUFSCHLAG','SESSELKANTEN-TREFFER','PINGPONG-BANDE','TOPSPIN-STÖRUNG','BRUNOS SCHUTZBALL'],third:'splash',fourth:'slow',fifth:'shield'},
    narrath:{flag:'narrath',cd:'narrathCd',icon:'⚕',bonus:.085,names:['NARRATHS TAKTGEBER','KALIBRIERTER LASER','LABOR-REFLEX','BETÄUBUNGSSTRAHL','NOTFALL-REZEPT'],third:'fork',fourth:'interrupt',fifth:'heal'},
    knisterKnight:{flag:'knisterKnight',cd:'knister',icon:'⚔',bonus:.65,names:['KNISTERSPORN','GESCHÄRFTE KLINGE','QUERHIEB','FUNKENFESSEL','RITTERSCHILD'],third:'splash',fourth:'slow',fifth:'shield'},
    wollenkamps:{flag:'wollenkamps',cd:'wollenkamps',icon:'☂',bonus:.8,names:['WETTERWARNUNG','SCHIRMSPITZE','DOPPELTER VORTRAG','STURMDEBATTE','OMA-WETTERTEE'],third:'fork',fourth:'interrupt',fifth:'heal'},
    potencyMinister:{flag:'potencyMinister',cd:'ministerCd',icon:'♧',bonus:.65,names:['EILKONTROLLE','AMTSSTEMPEL','GRUPPENPRÜFUNG','VERORDNETE PAUSE','AKTENDECKEL'],third:'mark',fourth:'interrupt',fifth:'shield'},
    olangolil:{flag:'bingoMaster',cd:'bingoAttack',icon:'▦',bonus:.48,names:['EXTRAZIEHUNG','WUTLIPPE','DOPPELBINGO','UNGLÜCKSZAHL','BINGO-JUBEL'],third:'fork',fourth:'slow',fifth:'burst'},
    pigPatrol:{flag:'pigPatrol',cd:'patrolCd',icon:'♞',bonus:.33,names:['EILMARSCH DER SAU','KALIBER FERKEL','KREUZFEUER DER PATROUILLE','SCHLAMMSTIEFEL','SCHWEINEPANZER'],third:'fork',fourth:'slow',fifth:'shield'},
    troutDormian:{flag:'troutDormian',cd:'troutCd',icon:'♓',bonus:.7,names:['FLOTTENFLOSSE','HARTER FLOSSENSCHLAG','FORELLENSPRITZER','EISIGES WASSER','DORMIANS PAUSE'],third:'splash',fourth:'slow',fifth:'heal'},
    lominarWorm:{flag:'lominarWorm',cd:'worm',icon:'〰',bonus:.38,names:['TURBOWURM','WURMZÄHNE','UNTERIRDISCHER SCHLAG','SCHLEIMSPUR','WURMLOCH-SCHUTZ'],third:'splash',fourth:'slow',fifth:'shield'},
    nutSentry:{flag:'nutSentry',cd:'nutSentryCd',icon:'♜',bonus:.30,names:['SCHNELLES VISIER','HÄRTERE NÜSSE','ABPRALLSCHUSS','FROSTGETRIEBE','PANZERTURM'],third:'fork',fourth:'slow',fifth:'shield'},
    carrotDrone:{flag:'carrotDrone',cd:'carrotDroneCd',icon:'⌁',bonus:.85,names:['KURZE FLUGROUTE','SCHWERER WURF','MÖHRENSPLITTER','MÖHRENFROST','RETTUNGSROTOR'],third:'splash',fourth:'slow',fifth:'heal'},
    eggBooger:{flag:'eggBooger',cd:'eggBoogerCd',icon:'●',bonus:.42,names:['SCHNELLES GEHÜPFE','HARTER EIERSCHLEIM','ZWEITE NASE','KLEBRIGER TREFFER','EIERSCHALE'],third:'fork',fourth:'slow',fifth:'shield'},
    ratKing:{flag:'ratKing',cd:'ratKingCd',icon:'♛',bonus:.34,names:['RATTEN-ALARM','SCHARFE ZÄHNE','ZWEITES RUDEL','RATTENFALLE','KÖNIGLICHE RETTUNG'],third:'rats',fourth:'interrupt',fifth:'heal',endless:true},
    erweinLewy:{flag:'erweinLewy',cd:'erweinCd',icon:'✧',bonus:.40,names:['LEWYS RAKETENSCHUH','LASER DER ACHT BEINE','ZWEITER MONDSTRAHL','SCHWERELOSER FEIND','NOTLANDEKAPSEL'],third:'fork',fourth:'slow',fifth:'shield'},
    soapedRat:{flag:'soapedRat',cd:'soapCd',icon:'◕',bonus:.55,names:['SEIFIGE EILRATTE','SCHAUMZÄHNE','ZWEITE SCHLITTERSPUR','GLITSCHIGE PFOTEN','SAUBERE RETTUNG'],third:'splash',fourth:'slow',fifth:'heal'},
    daimDuo:{flag:'daimDuo',cd:'daimCd',icon:'♧',bonus:.36,names:['DAIMDAPPLERS TAKT','DAPPENDAIMLERS ZIEL','DOPPEL-DAPPEN','VERWIRRSCHUSS','DUO-DECKUNG'],third:'duo',fourth:'interrupt',fifth:'shield'},
    orbit:{flag:'orbit',cd:'orbitCd',icon:'◌',bonus:.32,names:['SCHNELLE SATELLITEN','STAHLNUSSHÜLLE','DRITTE UMLAUFBAHN','GRAVITATIONSBREMSE','NOTFALL-ORBIT'],third:'orbit',fourth:'slow',fifth:'shield'},
    creamPuff:{flag:'creamPuff',cd:'puffCd',icon:'◒',bonus:.42,names:['SCHNELLER WIND','SAHNEKERN','DOPPELTER WINDBEUTEL','KLEBRIGE SAHNE','SÜSSE NOTRATION'],third:'puff',fourth:'slow',fifth:'heal'},
    eichelkopf:{flag:'eichelkopf',cd:'eichelCd',icon:'◕',bonus:.62,names:['EICHEL-TURBO','HARTE SCHALE','PLATZENDE EICHEL','EICHELSTAUB','EICHELSCHUTZ'],third:'splash',fourth:'slow',fifth:'shield'}
  };
  const evolutionDescriptions={
    splash:'Am Ziel entsteht nach einem Angriff ein zweiter Flächentreffer mit 0,75 × √Nussschaden im Umkreis von 75. Derselbe Boss wird nur einmal zusätzlich getroffen.',
    fork:'Der Angriff trifft außerdem den nächsten Gegner für 0,55 × √Nussschaden. Gibt es keinen zweiten Gegner, trifft er das ursprüngliche Ziel für 0,26 × √Nussschaden.',
    mark:'Die Kontrolle markiert auch den nächsten Gegner 2,5 Sekunden lang als verwundbar. Markierte Ziele erleiden +14 % Schaden, Bosse +8 %.',
    rats:'Bei jedem Rudel starten zwei zusätzliche Ratten mit je 55 % des normalen Rudelschadens.',
    duo:'Bei jedem Angriff feuert die zweite Person eine zusätzliche Nuss mit 0,8 × √Nussschaden ab.',
    orbit:'Eine dritte Nuss umkreist dich. Alle drei teilen sich weiterhin denselben Treffer-Cooldown.',
    puff:'Bei jedem Angriff entsteht ein zweiter Windbeutel mit halbem Schaden. Beide platzen getrennt voneinander.',
    slow:'Treffer bremsen normale Gegner kurz ab. Bei Bossen hält die Verlangsamung kürzer an.',
    interrupt:'Treffer unterbrechen normale Gegner kurz. Bosse werden stattdessen verlangsamt.',
    shield:'Bei einem Angriff werden bis zu zwei feindliche Geschosse in 135 Reichweite um Snickers zerstört. Höchstens alle 18 Sekunden.',
    heal:'Bei einem Angriff wird Snickers um ein Herz geheilt, falls er verletzt ist. Höchstens alle 28 Sekunden.',
    burst:'Höchstens alle 16 Sekunden verursacht ein Angriff einen Zusatzpuls für 1,05 × √Nussschaden in 100 Reichweite um das Ziel.'
  };
  const newCompanionBases={
    erweinLewy:['ERWEIN LEWY, DER ACHTBEINIGE ASTRONAUT','✧','Ein Astronaut auf acht Beinen begleitet Snickers. Alle 4,2 s trifft sein Mondlaser ein Ziel für 3,1 × √Nussschaden.'],
    soapedRat:['DIE RATTE DER EINGESEIFTEN','◕','Eine eingeseifte Ratte schlittert durch die Arena. Alle 5,4 s entsteht eine Lache: bis zu 3,2 × √Nussschaden über 3 s und kurze Verlangsamung.'],
    daimDuo:['DAIMDAPPLER UND DAPPENDAIMLER','♧','Das schräge Duo läuft links und rechts von Snickers und feuert abwechselnd alle 1,6 s eine Nuss für 2 × √Nussschaden.']
  };
  for(const [id,[title,icon,desc]] of Object.entries(newCompanionBases))skillBook[id]={title,short:title,icon,desc,apply:p=>{p[id]=true;}};
  ngPlusCarryKeys.push(...Object.keys(newCompanionBases));
  upgradePools.forEach((pool,i)=>pool.push(['erweinLewy','soapedRat','daimDuo'][i%3]));
  for(const [base,g] of Object.entries(companionGroups))for(let rank=1;rank<=5;rank++){
    const id=`${base}Up${rank}`;
    const actor=(skillBook[base]||survivalSkillBook[base])?.short||base;
    const desc=`${actor}: ${rank===1?(base==='lominarWorm'?'Kriecht 15 % schneller durch die Arena.':base==='orbit'?'Die Nüsse umkreisen Snickers 17 % schneller.':base==='knisterKnight'?'Bereitet den nächsten Klingenflug rund 15 % schneller vor.':'Die Fähigkeit ist nach jedem Angriff rund 15 % schneller wieder bereit.'):rank===2?`Jeder eigene Angriff verursacht zusätzlich ${g.bonus.toLocaleString('de-DE')} × √Nussschaden.`:evolutionDescriptions[rank===3?g.third:rank===4?g.fourth:g.fifth]}`;
    const reward={title:g.names[rank-1],short:g.names[rank-1],icon:g.icon,desc,requires:base,requiresFlag:g.flag,apply:p=>{p.skills[id]=1;}};
    if(g.endless){reward.endless=true;survivalSkillBook[id]=reward;}else skillBook[id]=reward;
    if(rank===2||rank===3)blueRewards.add(id);
    if(rank===5)purpleRewards.add(id);
  }
  function evolved(base,rank,p=player){const g=companionGroups[base];return Boolean(g&&p?.[g.flag]&&p.skills?.[`${base}Up${rank}`]);}
  const evolvedAvailable=availableRewards;
  availableRewards=function(includeSpecial=false){return evolvedAvailable(includeSpecial).filter(c=>!c.requires||Boolean(player?.skills?.[c.requires]||player?.[c.requiresFlag]));};
  const evolvedRewardChoices=rewardChoices;
  rewardChoices=function(){const choices=evolvedRewardChoices();if(Math.random()<.52){const followups=availableRewards().filter(c=>c.requires&&!choices.some(x=>x.id===c.id));const replaceable=choices.map((c,i)=>({c,i})).filter(({c})=>!c.weapon&&!c.legendary);if(followups.length&&replaceable.length)choices[shuffled(replaceable)[0].i]=shuffled(followups)[0];}return choices;};
  const evolvedGrant=grantReward;
  grantReward=function(c){if(c?.requires&&!player?.skills?.[c.requires]&&!player?.[c.requiresFlag])return;evolvedGrant(c);};
  achievements.push({id:'companion_upgrade_trio',title:'GEMEINSAM SIND WIR NERVIG',desc:'Rüste in einem Run drei verschiedene Verbündete mit je einem eigenen Folge-Skill auf.',goal:1,kind:'gag'});
  const companionAchievementGrant=grantReward;
  grantReward=function(c){
    companionAchievementGrant(c);
    if(!player||mode==='menu')return;
    const upgraded=Object.entries(companionGroups).filter(([base,g])=>base!=='orbit'&&base!=='creamPuff'&&Boolean(player[g.flag])&&[1,2,3,4,5].some(rank=>player.skills[`${base}Up${rank}`]));
    if(upgraded.length>=3)addAchievementProgress('companion_upgrade_trio');
  };
  const evolvedSanitize=sanitizeSharedBuild;
  sanitizeSharedBuild=function(raw){const clean=evolvedSanitize(raw);for(const id of Object.keys(clean.skills)){const c=allSkillInfo(id);if(c?.requires&&!clean.skills[c.requires]&&!clean[c.requiresFlag])delete clean.skills[id];}return clean;};

  let soapPuddles=[];
  function companionOrigin(base){
    switch(base){
      case 'bruno':return brunoPosition();
      case 'narrath':return {x:player.profX??player.x-45,y:player.profY??player.y-28};
      case 'knisterKnight':return player.knisterState||player;
      case 'wollenkamps':return contentCompanion||player;
      case 'potencyMinister':return {x:player.ministerX??player.x,y:H/2};
      case 'olangolil':return {x:W/2,y:H/2};
      case 'troutDormian':return {x:W/2,y:H/2};
      case 'lominarWorm':return {x:player.lominarX??player.x,y:player.lominarY??player.y};
      case 'erweinLewy':return {x:player.erweinX??player.x-57,y:player.erweinY??player.y-55};
      case 'soapedRat':return {x:player.soapX??player.x+50,y:player.soapY??player.y+28};
      case 'daimDuo':return {x:player.x+(player.daimSide||1)*48,y:player.y+28};
      default:return player;
    }
  }
  function companionCycle(base){
    if(base==='knisterKnight')return player.knisterState?.phase==='slice'?1:0;
    if(base==='wollenkamps')return contentCompanion?.cooldown||0;
    if(base==='lominarWorm')return player.lominarDir||1;
    return player[companionGroups[base].cd]||0;
  }
  function companionTriggered(base,before,after){
    if(base==='knisterKnight')return before===0&&after===1;
    if(base==='lominarWorm')return before!==after;
    return after>before+.04;
  }
  function applyCompanionEvolution(base){
    const group=companionGroups[base],origin=companionOrigin(base),target=nearestTarget(origin,base==='bruno'?1500:base==='ratKing'?800:720);
    if(evolved(base,1)){
      if(base==='knisterKnight'){} // Route preparation is accelerated between flights below.
      else if(base==='wollenkamps'){if(contentCompanion)contentCompanion.cooldown*=.85;}
      else if(base!=='lominarWorm')player[group.cd]*=.85;
    }
    if(!target)return;
    const root=Math.sqrt(player.damage||1),bonus=group.bonus*root;
    if(evolved(base,2)){damageEnemy(target,bonus);if(mode!=='playing')return;}
    if(evolved(base,3)){
      if(group.third==='mark'){const second=combatTargets().find(e=>e!==target&&!e.dead&&dist(e,target)<160);if(second)second.potencyMarked=Math.max(second.potencyMarked||0,2.5);}
      else if(group.third==='orbit'||group.third==='puff'){} // Their extra visible actor is created in the base attack.
      else if(group.third==='rats')launchRats(2,.55*player.specialDamage);
      else if(group.third==='duo'){const from={x:player.x-(player.daimSide||1)*48,y:player.y+27},a=Math.atan2(target.y-from.y,target.x-from.x);contentProjectile(from,a,.8*root,{auto:true,life:1.3,r:5});}
      else if(group.third==='splash'){burst(target.x,target.y,'#e6d4ae',7,75);for(const e of combatTargets())if(!e.dead&&dist(e,target)<75+e.r){damageEnemy(e,Math.min(.75,group.bonus*.75)*root,true);if(mode!=='playing')return;}}
      else{const second=combatTargets().filter(e=>e!==target&&!e.dead).sort((a,b)=>dist(a,target)-dist(b,target))[0]||target;particles.push({type:'beam',x:origin.x,y:origin.y,x2:second.x,y2:second.y,life:.16,maxLife:.16,color:'#c1e7dc'});damageEnemy(second,(second===target?.26:.55)*root);if(mode!=='playing')return;}
    }
    if(evolved(base,4)){
      const big=isBoss(target)||target.miniBoss;
      if(group.fourth==='interrupt'&&!big)target.discussTime=Math.max(target.discussTime||0,base==='wollenkamps'?2.4:base==='potencyMinister'?1.65:.55);
      else target.slow=Math.max(target.slow||0,big?(base==='wollenkamps'?1.7:base==='lominarWorm'?1.5:base==='troutDormian'?.72:.3):(base==='lominarWorm'?2.8:base==='troutDormian'?1.6:1.05));
    }
    if(evolved(base,5)){
      const key=`${base}GuardCd`,time=group.fifth==='heal'?28:group.fifth==='burst'?16:18;
      if((player[key]||0)>0)return;
      if(group.fifth==='heal'){
        if(player.hp>=player.maxHp)return;
        player.hp=Math.min(player.maxHp,player.hp+1);floater(origin.x,origin.y-25,'+1 ♥','#f8dbaa');updateHud();
      }else if(group.fifth==='shield'){
        if(!enemyBullets.some(b=>dist(b,player)<135))return;
        eraseNearbyShots(player,135,2);burst(player.x,player.y,'#c5e3eb',8,85);
      }else{
        burst(target.x,target.y,'#f4c884',12,115);
        for(const e of combatTargets())if(!e.dead&&dist(e,target)<100+e.r){damageEnemy(e,1.05*root,true);if(mode!=='playing')return;}
      }
      player[key]=time;
    }
  }
  function updateNewCompanions(dt){
    const activated=[];
    if(player.erweinLewy){
      player.erweinX=(player.erweinX??player.x-60)+((player.x-60)-(player.erweinX??player.x-60))*Math.min(1,dt*4);
      player.erweinY=(player.erweinY??player.y-52)+((player.y-52)-(player.erweinY??player.y-52))*Math.min(1,dt*4);
      player.erweinCd=(player.erweinCd??1.5)-dt;
      if(player.erweinCd<=0){const from=companionOrigin('erweinLewy'),target=nearestTarget(from,640);if(target){player.erweinCd=4.2;particles.push({type:'beam',x:from.x,y:from.y,x2:target.x,y2:target.y,life:.24,maxLife:.24,color:'#a7e8eb'});damageEnemy(target,3.1*Math.sqrt(player.damage));activated.push('erweinLewy');}}
    }
    if(player.soapedRat){
      const target=nearestTarget({x:player.soapX??player.x+50,y:player.soapY??player.y+28},690);
      const dest=target||{x:player.x+55,y:player.y+28};
      player.soapX=(player.soapX??dest.x)+(dest.x-(player.soapX??dest.x))*Math.min(1,dt*1.7);
      player.soapY=(player.soapY??dest.y)+(dest.y-(player.soapY??dest.y))*Math.min(1,dt*1.7);
      player.soapCd=(player.soapCd??1.9)-dt;
      if(target&&player.soapCd<=0){player.soapCd=5.4;soapPuddles.push({x:target.x,y:target.y,r:67,life:3.1,tick:.1,damage:.8*Math.sqrt(player.damage)});if(soapPuddles.length>5)soapPuddles.shift();activated.push('soapedRat');}
    }
    if(player.daimDuo){
      player.daimCd=(player.daimCd??.8)-dt;
      if(player.daimCd<=0){const side=player.daimSide=-(player.daimSide||1),from={x:player.x+side*48,y:player.y+27},target=nearestTarget(from,560);if(target){player.daimCd=1.6;contentProjectile(from,Math.atan2(target.y-from.y,target.x-from.x),2*Math.sqrt(player.damage),{auto:true,life:1.3,r:5});activated.push('daimDuo');}}
    }
    for(const puddle of soapPuddles){puddle.life-=dt;puddle.tick-=dt;if(puddle.tick<=0){puddle.tick=.8;for(const e of combatTargets())if(!e.dead&&dist(e,puddle)<puddle.r+e.r){e.slow=Math.max(e.slow||0,isBoss(e)?.3:.9);damageEnemy(e,puddle.damage);if(mode!=='playing')return activated;}}}
    soapPuddles=soapPuddles.filter(f=>f.life>0);
    return activated;
  }
  const previousEvolutionUpdate=updateExpansion;
  updateExpansion=function(dt){
    const watched=Object.keys(companionGroups).filter(base=>player?.[companionGroups[base].flag]&&!Object.hasOwn(newCompanionBases,base));
    const before=new Map(watched.map(base=>[base,companionCycle(base)]));
    previousEvolutionUpdate(dt);if(mode!=='playing')return;
    const activated=updateNewCompanions(dt);if(mode!=='playing')return;
    for(const base of watched){
      const after=companionCycle(base),old=before.get(base);
      if(companionTriggered(base,old,after))activated.push(base);
      if(base==='knisterKnight'&&evolved(base,1)&&['rest','ready'].includes(player.knisterState?.phase))player.knisterState.timer-=dt*.3;
      if(base==='lominarWorm'&&evolved(base,2)){
        const worm=companionOrigin(base),victim=nearestTarget(worm,45);
        player.wormBiteCd=Math.max(0,(player.wormBiteCd||0)-dt);
        if(victim&&player.wormBiteCd<=0){player.wormBiteCd=.9;damageEnemy(victim,.38*Math.sqrt(player.damage));if(mode!=='playing')return;}
      }
    }
    for(const base of activated){applyCompanionEvolution(base);if(mode!=='playing')return;}
    for(const base of Object.keys(companionGroups)){const key=`${base}GuardCd`;if(player[key]>0)player[key]=Math.max(0,player[key]-dt);}
  };
  const previousEvolutionDraw=drawExpansion;
  drawExpansion=function(){previousEvolutionDraw();if(!player)return;
    for(const f of soapPuddles){ctx.save();ctx.globalAlpha=Math.min(.35,f.life*.13);ellipse(ctx,f.x,f.y,f.r,f.r*.44,'#a6e7e6');ctx.globalAlpha=.8;ctx.strokeStyle='#d8ffff';ctx.lineWidth=3;ctx.beginPath();ctx.ellipse(f.x,f.y,f.r,f.r*.44,0,0,TAU);ctx.stroke();ctx.restore();}
    if(player.erweinLewy){const x=player.erweinX??player.x-60,y=player.erweinY??player.y-52;ctx.save();ctx.translate(x,y);ctx.strokeStyle='#96c9d2';ctx.lineWidth=3;for(let i=0;i<8;i++){const a=i*TAU/8+ambientTime*.22;ctx.beginPath();ctx.moveTo(Math.cos(a)*9,Math.sin(a)*8);ctx.lineTo(Math.cos(a)*19,Math.sin(a)*22+Math.sin(ambientTime*3+i)*3);ctx.stroke();}ellipse(ctx,0,0,13,13,'#e8e6de');ellipse(ctx,0,-1,9,7,'#89b9cc');ellipse(ctx,-3,-2,2,2,'#f2f9fa');ctx.restore();}
    if(player.soapedRat){const x=player.soapX??player.x+55,y=player.soapY??player.y+28;ctx.save();ctx.translate(x,y);ellipse(ctx,0,4,18,10,'#788f83');ellipse(ctx,7,-3,10,8,'#b5c9ba');ellipse(ctx,12,-5,3,3,'#f7f7e6');ellipse(ctx,-7,-9,6,5,'#d8eeee');ellipse(ctx,0,-14,7,4,'#e4f7f4');ctx.strokeStyle='#adc8be';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(-15,4);ctx.quadraticCurveTo(-32,-9,-39,2);ctx.stroke();ctx.restore();}
    if(player.daimDuo)for(const side of [-1,1]){ctx.save();ctx.translate(player.x+side*48,player.y+27);ellipse(ctx,0,11,14,6,'#17252188');ctx.fillStyle=side<0?'#f3c27b':'#b7b0e3';ctx.fillRect(-9,-4,18,17);ellipse(ctx,0,-12,10,11,side<0?'#e5ad86':'#d9beaa');ellipse(ctx,side*3,-14,2,2,'#3d3546');path(ctx,[[-10,-19],[0,-29],[11,-19]],side<0?'#ecac5e':'#7e7ec1');ctx.restore();}
  };
  const previousEvolutionClear=clearCombatExtras;
  clearCombatExtras=function(){previousEvolutionClear();soapPuddles=[];};
  ...
  // The Knollengeier only takes reversible, unstacked passive skills without dependent upgrades.
  const knollStealable={frost:'frost',chain:'chain',orbit:'orbit',nutSentry:'nutSentry',carrotDrone:'carrotDrone',eggBooger:'eggBooger',merzEggs:'merzEggs',homing:'homing',shockDash:'shockDash',breadHalo:'breadHalo',ricochet:'ricochet',creamPuff:'creamPuff',bruno:'bruno',wollenkamps:'wollenkamps',duden:'duden',olangolil:'bingoMaster',potencyMinister:'potencyMinister',eichelkopf:'eichelkopf',pigPatrol:'pigPatrol',troutDormian:'troutDormian',pockenstrombose:'pockenstrombose',pigCheekBomber:'pigCheekBomber',hamsterTwister:'hamsterTwister',hamsterVape:'hamsterVape',ngAcornBarrage:'ngAcornBarrage',ngDashCinders:'ngDashCinders',ng2Graviton:'ng2Graviton',ng2TimeCrust:'ng2TimeCrust',ng2LastSnack:'ng2LastSnack',ng2Crossfire:'ng2Crossfire'};
  function knollCandidates(){return Object.keys(knollStealable).filter(id=>player.skills[id]===1&&player[knollStealable[id]]&&!(id==='nutSentry'&&(player.turretVolley||player.ng2Turret))&&
    !Object.keys(player.skills).some(other=>allSkillInfo(other)?.requires===id));}
  const beforeKnollAvailability=availableRewards;
  availableRewards=function(includeSpecial=false){const choices=beforeKnollAvailability(includeSpecial);return knollQuest&&['notice','pending','hunting'].includes(knollQuest.status)?choices.filter(c=>c.id!==knollQuest.id):choices;};
  const beforeKnollReward=prepareWaveReward;
  prepareWaveReward=function(){
    beforeKnollReward();
    const nextIsWave=endlessMode?(wave+1)%5!==0:![2,5,8].includes(wave);
    if(knollQuest||trollQuest||!nextIsWave)return;
    const choices=knollCandidates();
    if(choices.length&&Math.random()<.01){
      const id=choices[Math.floor(Math.random()*choices.length)],flag=knollStealable[id];
      knollQuest={status:'notice',id,flag,wave:wave+1,spawned:false,expiresAt:0};
      delete player.skills[id];player[flag]=false;updateSkills();updateHud();
    }
  };
  const beforeKnollRenderReward=renderWaveReward;
  renderWaveReward=function(){
    if(knollQuest?.status==='notice'){
      mode='upgrade';const title=escapeHTML(allSkillInfo(knollQuest.id)?.title||knollQuest.id);
      showOverlay(`<span class="eyebrow">1 % EREIGNIS · KNOLLENGEIER</span><h2 id="overlayTitle">EIN SKILL IST WEG!</h2><p>Ein Knollengeier hat <strong>${title}</strong> gestohlen. Wähle gleich dein normales Wellen-Upgrade. In der nächsten Welle erscheint ein Schwarm: Nur der Geier mit der <strong>goldenen Feder</strong> trägt deinen Skill. Er flieht nach 19 Sekunden. Erwischst du ihn vorher, erhältst du den Skill zurück; sonst ist er für diesen Run verloren.</p><button class="primary-button" id="knollNoticeContinue">UPGRADE AUSWÄHLEN ↗</button>`);
      $('knollNoticeContinue').onclick=()=>{if(mode!=='upgrade'||knollQuest?.status!=='notice')return;knollQuest.status='pending';renderWaveReward();saveRunNow();};return;
    }
    beforeKnollRenderReward();
  };
  const beforeKnollStartWave=startWave;
  startWave=function(next,keepRetries=false){
    beforeKnollStartWave(next,keepRetries);
    if(knollQuest&&knollQuest.wave===next&&(knollQuest.status==='pending'||keepRetries&&knollQuest.status==='hunting')){
      knollQuest.status='hunting';knollQuest.spawned=false;knollQuest.expiresAt=0;
    }
    saveRunNow();
  };
  function spawnKnollFlock(){
    if(knollQuest?.status!=='hunting'||knollQuest.spawned)return;
    knollQuest.spawned=true;knollQuest.expiresAt=waveTime+19;
    const count=impossibleMode?13:hardMode?11:9,correct=Math.floor(Math.random()*count);
    for(let i=0;i<count;i++){
      const e=spawnEnemy('runner');e.type='knollVulture';e.knoll=true;e.knollCorrect=i===correct;
      e.hp=e.maxHp=Math.max(3,e.maxHp*(e.knollCorrect?3.2:2.3));e.r=e.knollCorrect?21:19;
      e.speed=clamp(e.speed*(e.knollCorrect?.67:.76),70,170);e.fireCd=1.4+(i%4)*.4;e.spawnGrace=.7;
      const a=TAU*i/count,rr=clamp(315+i%3*35,270,400);
      e.x=clamp(player.x+Math.cos(a)*rr,55,W-55);e.y=clamp(player.y+Math.sin(a)*rr,145,H-55);
    }
    announce('KNOLLENGEIER-SCHWARM!','DIE GOLDENE FEDER TRÄGT DEINEN SKILL · 19 SEKUNDEN');saveRunNow();
  }
  function updateKnollVulture(e,dt){
    const dx=player.x-e.x,dy=player.y-e.y,d=Math.hypot(dx,dy)||1;
    const slow=(e.slow>0?.7:1)*(player.powerups.freeze>0?.6:1);
    const desired=e.knollCorrect?210:105,dir=d>desired+35?1:d<desired-40?-1:0;
    const turn=Math.sin(runTime*2.1+e.phase)>0?1:-1;
    e.x+=((dx/d)*dir*.8-(dy/d)*turn*.3)*e.speed*slow*dt;
    e.y+=((dy/d)*dir*.8+(dx/d)*turn*.3)*e.speed*slow*dt;
    if(e.fireCd<=0&&d<420){e.fireCd=e.knollCorrect?3.2:4.4;enemyShot(e.x,e.y,Math.atan2(dy,dx),e.knollCorrect?225:180,false);}
  }
  const beforeKnollKill=onContentKill;
  onContentKill=function(e){
    beforeKnollKill(e);
    if(!e.knoll||knollQuest?.status!=='hunting'||e.knollCorrect!==true){if(e.knoll)floater(e.x,e.y-27,'FALSCHE FEDER','#dfc9a1');return;}
    const {id,flag}=knollQuest,skill=allSkillInfo(id);
    if(skill&&!player.skills[id]){skill.apply(player);player.skills[id]=1;normalizeBuild(player);updateSkills();updateHud();}
    knollQuest.status='recovered';floater(e.x,e.y-34,'SKILL ZURÜCK!','#f9de7e');announce('GEIER ERWISCHT!',`${skill?.title||id} IST WIEDER DA`);saveRunNow();
  };
  const beforeKnollUpdateContent=updateContent;
  updateContent=function(dt){
    beforeKnollUpdateContent(dt);if(mode!=='playing')return;
    if(knollQuest?.status==='hunting'&&!boss){
      if(!knollQuest.spawned&&waveTime>=2.5)spawnKnollFlock();
      if(knollQuest.spawned&&waveTime>=knollQuest.expiresAt){
        for(const e of enemies)if(e.knollCorrect&&!e.dead){e.dead=true;burst(e.x,e.y,'#f0ce78',20,140);}
        knollQuest.status='lost';announce('KNOLLENGEIER ENTWISCHT',`${(allSkillInfo(knollQuest.id)?.title||'DER SKILL')} IST VERLOREN`);saveRunNow();
      }
    }
    player.cinderCd=Math.max(0,(player.cinderCd||0)-dt);
    const root=Math.sqrt(player.damage||1),specialRoot=Math.sqrt(player.specialDamage||1);
    const pulse=(key,seconds,range,fn)=>{player[key]=(player[key]??1)-dt;if(player[key]<=0){const target=nearestTarget(player,range);if(target){player[key]=seconds;fn(target);}}};
    if(player.pigCheekBomber)pulse('cheekBombCd',6,550,t=>{burst(t.x,t.y,'#e6a17b',12,115);for(const e of combatTargets())if(!e.dead&&dist(e,t)<100+e.r){damageEnemy(e,5*root,true);if(mode!=='playing')return;}});
    if(mode!=='playing')return;
    if(player.hamsterTwister)pulse('twisterCd',3.6,115,()=>{burst(player.x,player.y,'#eed6aa',16,135);for(const e of combatTargets())if(!e.dead&&dist(e,player)<115+e.r){e.slow=Math.max(e.slow||0,isBoss(e)?.35:.9);damageEnemy(e,2.8*root);if(mode!=='playing')return;}});
    if(mode!=='playing')return;
    if(player.hamsterVape)pulse('vapeCd',8,570,t=>{contentEffects.push({kind:'hamsterVapor',x:t.x,y:t.y,r:105,life:3,tick:.1,hits:0});});
    if(player.ngAcornBarrage)pulse('acornBarrageCd',7,620,t=>{const a=Math.atan2(t.y-player.y,t.x-player.x);for(const off of [-.16,-.05,.05,.16])contentProjectile(player,a+off,2.5*root,{speed:610,pierce:1,auto:true,r:7,life:1.5});});
    if(player.ng2Graviton)pulse('gravitonCd',9,175,()=>{burst(player.x,player.y,'#bca8e1',22,165);for(const e of combatTargets())if(!e.dead&&dist(player,e)<175+e.r){if(!isBoss(e)&&!e.miniBoss){const d=dist(player,e)||1;e.x+=(player.x-e.x)/d*24;e.y+=(player.y-e.y)/d*24;}damageEnemy(e,6*specialRoot,true);if(mode!=='playing')return;}});
    if(mode!=='playing')return;
    if(player.ng2TimeCrust){player.timeCrustCd=(player.timeCrustCd??8)-dt;if(player.timeCrustCd<=0){player.timeCrustCd=12;eraseNearbyShots(player,155,5);burst(player.x,player.y,'#cbdef1',12,115);for(const e of combatTargets())if(!e.dead&&dist(e,player)<155+e.r){damageEnemy(e,4*root,true);if(mode!=='playing')return;}}}
    if(mode!=='playing')return;
    if(player.ng2LastSnack&&player.hp<player.maxHp*.5){player.lastSnackCd=(player.lastSnackCd??24)-dt;if(player.lastSnackCd<=0){player.lastSnackCd=24;player.hp=Math.min(player.maxHp,player.hp+1);floater(player.x,player.y-32,'LETZTER SNACK +1 ♥','#d1e8a9');updateHud();}}
    if(player.ng2Crossfire)pulse('crossfireCd',5,630,t=>{const a=Math.atan2(t.y-player.y,t.x-player.x);for(const off of [-.09,.09])contentProjectile(player,a+off,3*root,{pierce:2,auto:true,speed:660,r:8,life:1.45});});
    for(const f of contentEffects)if((f.kind==='hamsterVapor'||f.kind==='diarrheaPuddle')&&f.tick<=0&&f.hits<4){f.tick=f.kind==='hamsterVapor'?.72:.8;f.hits++;const dmg=f.kind==='hamsterVapor'?1.6*specialRoot:4*player.specialDamage;for(const e of combatTargets())if(!e.dead&&dist(e,f)<f.r+e.r){if(!isBoss(e))e.slow=Math.max(e.slow||0,.8);damageEnemy(e,dmg,true);if(mode!=='playing')return;}}
  };
  const beforeKnollDash=useDash;
  useDash=function(){const old=player?.dashCount||0;beforeKnollDash();if(hasenbeinMode||!player?.ngDashCinders||player.dashCount<=old||(player.cinderCd||0)>0)return;player.cinderCd=5;burst(player.x,player.y,'#e7ad74',17,115);for(const e of combatTargets())if(!e.dead&&dist(player,e)<125+e.r)damageEnemy(e,5*Math.sqrt(player.damage||1),true);};
  const beforeKnollNewWeapon=useNewWeapon;
  useNewWeapon=function(id){
    if(!['diarrheaSling','ngCheeseGatling','ng2StarRoaster'].includes(id))return beforeKnollNewWeapon(id);
    player.weaponShots++;const t=weaponTarget(440),sm=player.specialDamage||1;
    if(id==='diarrheaSling'){for(const side of [-1,0,1])contentEffects.push({kind:'diarrheaPuddle',x:clamp(t.x+side*105,60,W-60),y:clamp(t.y+Math.abs(side)*26,140,H-60),r:86,life:3.4,tick:.1,hits:0});burst(t.x,t.y,'#aa8a65',16,100);tone(205,.16,'triangle',.05,75);}
    else if(id==='ngCheeseGatling'){for(let i=0;i<6;i++){const a=player.angle+(i-2.5)*.095;contentProjectile(player,a,9*sm,{speed:665,life:1.55,pierce:1,r:8,pretzel:true});}burst(player.x,player.y,'#f2d677',14,110);}
    else{for(let i=0;i<4;i++)hazards.push({type:'friendlyMortar',x:clamp(t.x+(i-1.5)*67,65,W-65),y:clamp(t.y+(i%2?24:-24),145,H-55),r:92,wait:.55+i*.17,life:.35,hit:false,damage:23*sm,friendly:true});tone(520,.2,'sawtooth',.07,150);}
    return true;
  };
  const beforeKnollDrawContent=drawContent;
  drawContent=function(){beforeKnollDrawContent();for(const f of contentEffects)if(f.kind==='hamsterVapor'||f.kind==='diarrheaPuddle'){ctx.save();ctx.globalAlpha=Math.min(.47,Math.max(.04,f.life*.15));ellipse(ctx,f.x,f.y,f.r,f.r*.45,f.kind==='hamsterVapor'?'#abacbc':'#977453');ctx.strokeStyle=f.kind==='hamsterVapor'?'#d7d1ed':'#bd9b70';ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(f.x,f.y,f.r,f.r*.45,0,0,TAU);ctx.stroke();ctx.restore();}};
  const beforeKnollRabbitDraw=drawRabbit;
  drawRabbit=function(e){beforeKnollRabbitDraw(e);if(!e.knoll||e.dead)return;ctx.save();ctx.translate(e.x,e.y);ctx.rotate(Math.sin(ambientTime*8+e.phase)*.1);
    path(ctx,[[-10,-7],[-37,-23],[-27,9],[-10,8]],'#736a65');path(ctx,[[10,-7],[37,-23],[27,9],[10,8]],'#736a65');ellipse(ctx,0,-4,17,17,'#827768');
    path(ctx,[[-13,-14],[0,-30],[13,-14]],'#a69874');path(ctx,[[-8,0],[13,4],[-8,10]],'#dfb572');ellipse(ctx,5,-12,3,3,'#312a30');
    if(e.knollCorrect){ctx.shadowColor='#ffe48a';ctx.shadowBlur=16;path(ctx,[[-9,-30],[0,-47],[9,-31]],'#ffe48a');ctx.font='bold 16px Arial';ctx.textAlign='center';ctx.fillStyle='#fff0b0';ctx.fillText(allSkillInfo(knollQuest?.id)?.icon||'✦',0,-53);}
    ctx.restore();};
  const beforeKnollHelp=helpMarkup;
  helpMarkup=function(){return beforeKnollHelp().replace('<article><b>Minibosse</b>','<article><b>Knollengeier · 1 %</b><p>Zwischen geeigneten Wellen kann ein Geier einen reversiblen Skill stehlen. Die nächste Welle bringt einen Schwarm: Der Geier mit goldener Feder hat den Skill. Besiege ihn innerhalb von 19 Sekunden; sonst bleibt der Skill in diesem Run verloren. Normale Wellen-Belohnungen bleiben erhalten.</p></article><article><b>Minibosse</b>');};
  const beforeKnollHud=updateHud;
  updateHud=function(){beforeKnollHud();if(knollQuest?.status==='hunting'&&knollQuest.spawned&&!boss){$('waveObjective').textContent+=` · GOLDENE FEDER: ${Math.max(0,Math.ceil(knollQuest.expiresAt-waveTime))} s`;}};
  blueRewards.add('pigCheekBomber');blueRewards.add('hamsterVape');

  // Ronny cycles through ten actual projectile patterns. His seven upgrades need
  // the base companion and never grant player-wide damage or a free weapon.
  const ronnyNuts=[
    {name:'Walnuss',color:'#c38b54',damage:2.9},
    {name:'Haselnuss',color:'#dfbd76',damage:1.6,shots:2,spread:.12},
    {name:'Eichel',color:'#c18d4f',damage:2.2,pierce:2},
    {name:'Pistazie',color:'#acc489',damage:2,frost:true},
    {name:'Kastanie',color:'#b57056',damage:3.6,r:11,speed:430,heavy:true},
    {name:'Mandel',color:'#e5bd91',damage:2,chain:true},
    {name:'Cashew',color:'#ecd295',damage:1.2,shots:3,spread:.18},
    {name:'Pekannuss',color:'#a27c60',damage:2.4,pierce:1,frost:true},
    {name:'Macadamia',color:'#e8d9b3',damage:2.7,splash:true},
    {name:'Pinienkern',color:'#e9e6bb',damage:1.7,pierce:1,homing:true}
  ];
  skillBook.ronnySquirrel=passive('RONNY HÖRNCHEN','♧','Ein mutiertes Eichhörnchen begleitet dich und schießt alle 1,8 s im Wechsel zehn Nusssorten: Walnuss, Haselnuss, Eichel, Pistazie, Kastanie, Mandel, Cashew, Pekannuss, Macadamia und Pinienkern. Verschiedene Streuung, Durchschlag und Effekte.','ronnySquirrel');
  ngPlusCarryKeys.push('ronnySquirrel');blueRewards.add('ronnySquirrel');
  upgradePools.forEach((pool,i)=>{if(i%2===0||i===7)pool.push('ronnySquirrel');});
  const ronnyUpgrades=[
    ['BUSCHFUNK-TURBO','Ronny feuert 12 % schneller: alle 1,58 statt 1,8 Sekunden.'],
    ['SCHÄRFERE KERNE','Jedes Geschoss von Ronny verursacht 18 % mehr Schaden.'],
    ['ZWEITE PFOTE','Jeder dritte Angriff schickt ein Zusatzgeschoss derselben Sorte für 50 % ihres Schadens.'],
    ['FROSTIGES NUSSLAGER','Ronnys Pistazien und Pekannüsse bremsen normale Gegner 2,1 s, Bosse 1,7 s statt 1,5 s.'],
    ['RONNYS KETTENKERN','Auch Ronnys Walnüsse und Macadamianüsse lösen bei einem Treffer eine Kettennuss aus.'],
    ['BOSS-KNABBERER','Ronnys Geschosse verursachen 15 % mehr Schaden gegen Bosse und Minibosse.'],
    ['NOTFALLVORRAT','Unter 35 % Leben vernichtet Ronny bis zu zwei Geschosse nahe Snickers; höchstens alle 22 s.']
  ];
  ronnyUpgrades.forEach(([title,desc],i)=>{
    const id=`ronnyUp${i+1}`;
    skillBook[id]={title,short:title,icon:'♧',desc:`Ronny Hörnchen: ${desc}`,requires:'ronnySquirrel',requiresFlag:'ronnySquirrel',apply:p=>{p.skills[id]=1;}};
    if([0,1,2,4,5].includes(i))blueRewards.add(id);
    if(i===6)purpleRewards.add(id);
  });
  const wollenkampsMore=[
    ['SCHIRMGEWITTER','Bei jeder Diskussion trifft sie bis zu drei weitere Gegner im Umkreis 105 für je 1,1 × √Nussschaden.'],
    ['KALTFRONT IM ANMARSCH','Ihre Diskussion bremst zusätzlich andere Gegner im Umkreis 115 für 0,85 s; Bosse und Minibosse 0,35 s.'],
    ['FALTSCHIRM-REFLEX','Bei einer Diskussion vernichtet sie bis zu zwei Geschosse in 145 Reichweite um Snickers; höchstens alle 16 s.']
  ];
  wollenkampsMore.forEach(([title,desc],i)=>{
    const id=`wollenkampsUp${i+6}`;
    skillBook[id]={title,short:title,icon:'☂',desc:`Frau Wollenkamps: ${desc}`,requires:'wollenkamps',requiresFlag:'wollenkamps',apply:p=>{p.skills[id]=1;}};
    (i===2?purpleRewards:blueRewards).add(id);
  });
  weaponBook.potatoSalvo={name:'KARTOFFELHAGEL',icon:'◉',desc:'Waffe: Zwei versetzte Kartoffeleinschläge am Ziel, je 21 × Spezialfaktor Schaden im Radius 96. Jeder Einschlag trifft dasselbe Ziel höchstens einmal. 4 Ladungen.',max:4};
  skillBook.potatoSalvo=weaponUnlock('potatoSalvo');
  upgradePools[2].push('potatoSalvo');upgradePools[5].push('potatoSalvo');upgradePools[8].push('potatoSalvo');
  const ronnyWeaponUse=useNewWeapon;
  useNewWeapon=function(id){
    if(id!=='potatoSalvo')return ronnyWeaponUse(id);
    player.weaponShots++;const t=weaponTarget(440),sm=player.specialDamage||1;
    for(const [i,dx] of [-46,46].entries())hazards.push({type:'friendlyMortar',x:clamp(t.x+dx,55,W-55),y:clamp(t.y+(i?20:-20),120,H-55),r:96,wait:.48+i*.24,life:.35,hit:false,damage:21*sm,friendly:true});
    burst(t.x,t.y,'#d7b580',14,110);tone(280,.18,'triangle',.055,85);return true;
  };
  const ronnyExpansion=updateExpansion;
  updateExpansion=function(dt){
    ronnyExpansion(dt);if(mode!=='playing'||!player?.ronnySquirrel)return;
    const px=player.x-42,py=player.y-32;
    player.ronnyX=(player.ronnyX??px)+(px-(player.ronnyX??px))*Math.min(1,dt*7);
    player.ronnyY=(player.ronnyY??py)+(py-(player.ronnyY??py))*Math.min(1,dt*7);
    const origin={x:player.ronnyX,y:player.ronnyY};
    player.ronnyCd=(player.ronnyCd??.65)-dt;
    const target=nearestTarget(origin,610);
    if(target&&player.ronnyCd<=0){
      player.ronnyCd=player.skills.ronnyUp1?1.58:1.8;
      const index=(player.ronnyIndex||0)%ronnyNuts.length,nut=ronnyNuts[index];player.ronnyIndex=index+1;
      player.ronnyShots=(player.ronnyShots||0)+1;
      const aim=Math.atan2(target.y-origin.y,target.x-origin.x),shots=nut.shots||1,root=Math.sqrt(player.damage||1);
      for(let i=0;i<shots;i++){
        const angle=aim+(i-(shots-1)/2)*(nut.spread||0),damage=nut.damage*root*(player.skills.ronnyUp2?1.18:1);
        contentProjectile(origin,angle,damage,{ronnyBullet:true,ronnyType:index,ronnyColor:nut.color,ronnySplash:Boolean(nut.splash),ronnyHoming:Boolean(nut.homing),ronnyLongFrost:Boolean(nut.frost&&player.skills.ronnyUp4),r:nut.r||6,speed:nut.speed||570,life:1.6,pierce:nut.pierce||0,chain:Boolean(nut.chain||player.skills.ronnyUp5&&(index===0||index===8)),frost:Boolean(nut.frost),heavy:Boolean(nut.heavy),auto:true});
      }
      if(player.skills.ronnyUp3&&player.ronnyShots%3===0)contentProjectile(origin,aim+.10,nut.damage*root*(player.skills.ronnyUp2?1.18:1)*.5,{ronnyBullet:true,ronnyType:index,ronnyColor:nut.color,auto:true,r:5,speed:570,life:1.5});
    }
    for(const b of bullets)if(b.ronnyHoming&&b.life>0){const t=nearestTarget(b,340);if(t){const aim=Math.atan2(t.y-b.y,t.x-b.x);b.a+=clamp(angleDelta(aim,b.a),-dt*3.3,dt*3.3);const speed=Math.hypot(b.vx,b.vy);b.vx=Math.cos(b.a)*speed;b.vy=Math.sin(b.a)*speed;}}
    if(player.skills.ronnyUp7){player.ronnyGuardCd=Math.max(0,(player.ronnyGuardCd||0)-dt);if(player.hp<player.maxHp*.35&&player.ronnyGuardCd<=0&&enemyBullets.some(b=>b.life>0&&dist(b,player)<135)){eraseNearbyShots(player,135,2);player.ronnyGuardCd=22;burst(player.ronnyX,player.ronnyY,'#e5d397',8,70);}}
  };
  const ronnyDrawExpansion=drawExpansion;
  drawExpansion=function(){ronnyDrawExpansion();if(!player?.ronnySquirrel)return;
    const x=player.ronnyX??player.x-42,y=player.ronnyY??player.y-32;ctx.save();ctx.translate(x,y);
    ctx.strokeStyle='#9a684d';ctx.lineWidth=5;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(-9,9);ctx.bezierCurveTo(-32,22,-36,-20,-18,-15);ctx.stroke();
    ellipse(ctx,0,7,12,14,'#a87951');ellipse(ctx,0,-7,12,11,'#be8a5a');
    path(ctx,[[-11,-10],[-13,-23],[-4,-15]],'#b88457');path(ctx,[[5,-15],[13,-24],[12,-9]],'#b88457');
    ellipse(ctx,-4,-8,2,3,'#282e28');ellipse(ctx,6,-8,2,3,'#282e28');ellipse(ctx,1,-1,6,4,'#efd1ac');ellipse(ctx,1,-2,2,2,'#423632');
    ellipse(ctx,13,10,5,4,ronnyNuts[(player.ronnyIndex||0)%10].color);ctx.fillStyle='#f8e3af';ctx.font='bold 9px Arial';ctx.textAlign='center';ctx.fillText('RONNY',0,-28);ctx.restore();
  };
  const knattertrollHelp=helpMarkup;
  helpMarkup=function(){return knattertrollHelp().replace('Der Endlosmodus wird nach NG+2 freigeschaltet und bietet frische oder Hall-of-Fame-Builds.','Der Endlosmodus wird nach NG+2 freigeschaltet und bietet frische Runs oder Kampagnen-Builds. Archivierte Endlos-Builds sind ausschließlich als Gegner im Hasenbein-Modus spielbar.');};

  // Pillepelier: five strong purple cards, two truly rare legendary cards and a
  // follower with five separate prerequisite upgrades. All damage uses existing caps.
  Object.assign(legendarySkillBook,{
    slimePaws:{icon:'☷',title:'SCHLEIMPFOTEN',short:'Schleimpfoten',legendary:true,desc:'Legendary: +28 % normaler Schaden. Normale Treffer bremsen Gegner für 2 s (Bosse kürzer); jeder sechste Treffer lässt Schleim auf ein zweites nahes Ziel spritzen.',apply:p=>{p.slimePaws=true;p.damage*=1.28;}},
    seasonRabies:{icon:'❂',title:'FÜRCHTERLICHE HAMSTERTOLLWUT DER JAHRESZEITEN',short:'Jahreszeiten-Tollwut',legendary:true,desc:'Legendary: +20 % Feuerrate. Alle 8 s wechselt die Jahreszeit: Frühling heilt 1 Herz, Sommer verbrennt nahe Gegner, Herbst trifft ein entferntes Ziel, Winter friert nahe Gegner kurz ein.',apply:p=>{p.seasonRabies=true;p.fireRate/=1.2;p.seasonClock=8;p.seasonIndex=0;}}
  });
  Object.assign(skillBook,{
    ...
    eiterUp2:{icon:'✹',title:'ÄTZENDE GESTALT',short:'Eiter: Schaden',requires:'eiterWesen',desc:'Verbessert DAS EITERNDE WESEN DER 3. DIMENSION: +25 % Schaden auf sein Hauptziel.',apply:p=>p.eiterUp2=true},
    eiterUp3:{icon:'◌',title:'NACHHALL DER DIMENSION',short:'Eiter: Echo',requires:'eiterWesen',desc:'Verbessert DAS EITERNDE WESEN DER 3. DIMENSION: nach jedem Schuss erhält ein zweites Ziel in 155 Reichweite 35 % des Hauptschadens.',apply:p=>p.eiterUp3=true},
    eiterUp4:{icon:'❄',title:'VERDERBTER HALT',short:'Eiter: Bremse',requires:'eiterWesen',desc:'Verbessert DAS EITERNDE WESEN DER 3. DIMENSION: getroffene Gegner werden 1,4 s verlangsamt (Bosse 0,6 s).',apply:p=>p.eiterUp4=true},
    eiterUp5:{icon:'◇',title:'KREIS AUS ANDERSWO',short:'Eiter: Schutz',requires:'eiterWesen',desc:'Verbessert DAS EITERNDE WESEN DER 3. DIMENSION: entfernt höchstens alle 18 s bis zu drei Geschosse direkt um dich.',apply:p=>p.eiterUp5=true}
  });
  for(const id of ['sniffAttack','unknofMachine','meatUnbopper','rabbitCommand','eiterWesen','eiterUp2','eiterUp3'])purpleRewards.add(id);
  for(const pool of upgradePools)pool.push('sniffAttack','unknofMachine','meatUnbopper','rabbitCommand','eiterWesen','eiterUp1','eiterUp2','eiterUp3','eiterUp4','eiterUp5');
  Object.assign(specialUpgradeBook,{
    pearlReserve:{icon:'◇',title:'PERLENRESERVE',short:'Perlenreserve',desc:'Boss-Upgrade: +2 Herzen, volle Heilung und +10 % Schaden gegen Bosse. Ohne weitere Schutzketten.',apply:p=>{p.maxHp+=2;p.hp=p.maxHp;p.bossDamage=(p.bossDamage||1)*1.10;}},
    saffronArsenal:{icon:'✧',title:'SAFRANARSENAL',short:'Safranarsenal',desc:'Boss-Upgrade: +22 % Spezialschaden, +1 maximale Ladung je Spezialwaffe und 15 % mehr Munitions-Dropchance.',apply:p=>{p.specialDamage*=1.22;p.ammoBonus++;p.ammoDropLuck*=1.15;}}
  });
  for(const pool of specialUpgradePools)pool.push('pearlReserve','saffronArsenal');
  for(const id of ['slimePaws','seasonRabies','sniffAttack','unknofMachine','meatUnbopper','rabbitCommand','eiterWesen','eiterUp1','eiterUp2','eiterUp3','eiterUp4','eiterUp5'])ngPlusCarryKeys.push(id);
  const pilleExpansion=updateExpansion;
  updateExpansion=function(dt){pilleExpansion(dt);if(mode!=='playing'||!player)return;
    const root=Math.sqrt(player.damage||1);
    if(player.sniffAttack){player.sniffCd=(player.sniffCd||0)-dt;if(player.sniffCd<=0){player.sniffCd=5;const t=nearestTarget(player,620);if(t)t.potencyMarked=Math.max(t.potencyMarked||0,6);}}
    if(player.meatUnbopper){player.meatCd=(player.meatCd||0)-dt;if(player.meatCd<=0){player.meatCd=14;eraseNearbyShots(player,115,4);}}
    if(player.rabbitCommand){player.rabbitCommandCd=(player.rabbitCommandCd||0)-dt;if(player.rabbitCommandCd<=0){player.rabbitCommandCd=6;const t=nearestTarget(player,640);if(t){for(const off of [-.13,0,.13]){const a=Math.atan2(t.y-player.y,t.x-player.x)+off;contentProjectile(player,a,1.35*root,{auto:true,life:1.35,speed:690,r:5});}burst(player.x,player.y,'#e6b771',10,70);}}}
    if(player.eiterWesen){const x=clamp(player.x+48,30,W-30),y=clamp(player.y-42,110,H-35);player.eiterX=(player.eiterX??x)+(x-(player.eiterX??x))*Math.min(1,dt*7);player.eiterY=(player.eiterY??y)+(y-(player.eiterY??y))*Math.min(1,dt*7);
      player.eiterCd=(player.eiterCd||0)-dt;const from={x:player.eiterX,y:player.eiterY},target=nearestTarget(from,540);
      if(target&&player.eiterCd<=0){player.eiterCd=player.eiterUp1?2:2.4;const dmg=3.2*root*(player.eiterUp2?1.25:1);
        particles.push({type:'beam',x:from.x,y:from.y,x2:target.x,y2:target.y,life:.23,maxLife:.23,color:'#a7eeac'});
        if(player.eiterUp4)target.slow=Math.max(target.slow||0,isBoss(target)?.6:1.4);damageEnemy(target,dmg,true);if(mode!=='playing')return;
        if(player.eiterUp3){const other=combatTargets().find(e=>e!==target&&!e.dead&&dist(e,target)<155);if(other){particles.push({type:'beam',x:target.x,y:target.y,x2:other.x,y2:other.y,life:.16,maxLife:.16,color:'#a7eeac'});damageEnemy(other,dmg*.35);if(mode!=='playing')return;}}
      }if(player.eiterUp5){player.eiterShieldCd=Math.max(0,(player.eiterShieldCd||0)-dt);if(player.eiterShieldCd===0&&enemyBullets.some(b=>dist(b,player)<105)){eraseNearbyShots(player,105,3);player.eiterShieldCd=18;}}}
    if(player.seasonRabies){player.seasonClock=(player.seasonClock??8)-dt;if(player.seasonClock<=0){player.seasonClock+=8;const season=player.seasonIndex%4;player.seasonIndex=(player.seasonIndex+1)%4;player.seasonsSeen=(player.seasonsSeen||0)+1;const near=combatTargets().filter(e=>!e.dead&&dist(e,player)<230);
      if(season===0)player.hp=Math.min(player.maxHp,player.hp+1);
      if(season===1)for(const e of near){damageEnemy(e,2.8*root,true);if(mode!=='playing')return;}
      if(season===2){const t=nearestTarget(player,650);if(t)damageEnemy(t,6*root,true);}
      if(season===3)for(const e of near)e.slow=Math.max(e.slow||0,isBoss(e)?.7:2.5);
      burst(player.x,player.y,['#b8dd91','#ec9168','#e7bf80','#a9ddf3'][season],18,120);if(player.seasonsSeen>=4)addAchievementProgress('pille_four_seasons');
    }}
  };
  const pilleDrawExpansion=drawExpansion;
  drawExpansion=function(){pilleDrawExpansion();if(player?.eiterWesen){const x=player.eiterX??player.x+48,y=player.eiterY??player.y-42;ctx.save();ctx.translate(x,y);ctx.rotate(Math.sin(ambientTime*2)*.12);ellipse(ctx,0,7,17,19,'#637c63');ellipse(ctx,0,-7,15,16,'#a3b98c');ellipse(ctx,0,-8,7,9,'#fff1d7');ellipse(ctx,0,-8,3,5,'#4b3759');ellipse(ctx,0,5,10,5,'#b26585');ctx.strokeStyle='#9bc685';ctx.lineWidth=3;for(let i=0;i<3;i++){ctx.beginPath();ctx.moveTo(-12+i*12,15);ctx.lineTo(-17+i*17,28);ctx.stroke();}ctx.restore();}};

  achievements.push(
    {id:'pille_sweep',title:'STAUBSAUGER IM FELLPALTO',desc:'Sammle beim Sieg mindestens sechs liegen gebliebene Drops auf einmal ein.',goal:1,kind:'gag'},
    {id:'pille_four_seasons',title:'WETTERGOTT MIT BACKENTASCHEN',desc:'Erlebe alle vier Jahreszeiten der Hamstertollwut in einem Run.',goal:1,kind:'gag'},
    {id:'pille_last_retry',title:'AUS NULL MACH EINS',desc:'Besiege einen Boss mit null Retry-Marken und gewinne dadurch eine zurück.',goal:1,kind:'gag'},
    {id:'pille_nose_friend',title:'DOPPELT GEKÄMMT',desc:'Freunde dich mit dem Nasenhaarentflechter an und wähle zwei verschiedene Verbesserungen.',goal:1,kind:'gag'},
    {id:'pille_eiter_choir',title:'DIMENSIONALER FRISÖR',desc:'Besitze das Eiternde Wesen und alle fünf seiner Folge-Skills gleichzeitig.',goal:1,kind:'gag'}
  );
  const pilleGrant=grantReward;
  grantReward=function(c){pilleGrant(c);if(player?.eiterWesen&&[1,2,3,4,5].every(i=>player.skills[`eiterUp${i}`]))addAchievementProgress('pille_eiter_choir');};
  Object.assign(extraEnemyBook,{
    whiskerScout:{name:'NASENPELZ-SPÄHER',stats:[5,106,19,185],color:'#c8a8c7',species:'rat',icon:'⌁',unlock:'Alle Modi · ab Welle 2'},
    tarSnail:{name:'TEERSCHNECK',stats:[14,44,26,290],color:'#859c80',species:'crab',icon:'◕',unlock:'Alle Modi · ab Welle 4'}
  });
  const pilleEnemyChoice=hardEnemyChoice;
  hardEnemyChoice=function(type){
    if(wave>=1&&Math.random()<(impossibleMode?.13:hardMode?.12:.10)&&enemies.filter(e=>['whiskerScout','tarSnail'].includes(e.type)&&!e.dead).length<4){
      if(wave>=3&&Math.random()<.44&&enemies.filter(e=>e.type==='tarSnail'&&!e.dead).length<2)return 'tarSnail';
      if(enemies.filter(e=>e.type==='whiskerScout'&&!e.dead).length<2)return 'whiskerScout';
    }return pilleEnemyChoice(type);
  };
  const pilleExtraEnemy=updateExtraEnemy;
  updateExtraEnemy=function(e,dt){
    if(e.type!=='whiskerScout'&&e.type!=='tarSnail')return pilleExtraEnemy(e,dt);
    const d=dist(e,player),a=Math.atan2(player.y-e.y,player.x-e.x),slow=(e.slow>0?.72:1)*(player.powerups.freeze>0?.60:1),act=dt*(e.actionRate||1);
    if(e.windup>0){e.windup-=act;if(e.windup<=0){
      if(e.type==='whiskerScout'){for(const off of [-.12,.12])enemyShot(e.x,e.y,e.aim+off,175+(impossibleMode?30:0));}
      else hazards.push({type:'mortar',x:clamp(e.targetX,55,W-55),y:clamp(e.targetY,135,H-55),r:48,wait:1.2,life:.32,hit:false,damage:1});
    }return;}
    if(d>(e.type==='whiskerScout'?280:170)){e.x+=Math.cos(a)*e.speed*dt*slow;e.y+=Math.sin(a)*e.speed*dt*slow;}
    else if(e.type==='whiskerScout'&&d<175){e.x-=Math.cos(a)*e.speed*dt*.45;e.y-=Math.sin(a)*e.speed*dt*.45;}
    if(e.fireCd<=0&&d<650){e.windup=e.type==='whiskerScout'?.70:.95;e.fireCd=(e.type==='whiskerScout'?4.3:5.0)/(e.actionRate||1);e.aim=a;e.targetX=player.x;e.targetY=player.y;}
  };
  const pilleDrawExtraEnemy=drawExtraEnemy;
  drawExtraEnemy=function(e){if(e.type!=='tarSnail')return pilleDrawExtraEnemy(e);
    ctx.save();ctx.translate(e.x,e.y);if(e.spawnGrace>0)ctx.globalAlpha=.6;
    ellipse(ctx,-4,12,32,9,'#658b6890');ellipse(ctx,1,7,25,13,'#9daf8a');ellipse(ctx,-9,-4,20,21,e.hit>0?'#f8ecdc':'#857860');
    ctx.strokeStyle='#c2ae83';ctx.lineWidth=3;ctx.beginPath();ctx.arc(-9,-4,12,0,TAU);ctx.arc(-9,-4,5,0,TAU);ctx.stroke();
    for(const x of [12,23]){ctx.strokeStyle='#95ac85';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(x,1);ctx.lineTo(x,-16);ctx.stroke();ellipse(ctx,x,-17,3,4,'#3c3a3e');}
    ctx.restore();if(e.hp<e.maxHp){ctx.fillStyle='#19241d';ctx.fillRect(e.x-28,e.y-e.r-28,56,5);ctx.fillStyle='#abbb91';ctx.fillRect(e.x-28,e.y-e.r-28,56*Math.max(0,e.hp/e.maxHp),5);}
  };
  const pilleFire=fire;
  fire=function(){const before=bullets.length,ok=pilleFire();if(ok&&player?.slimePaws)for(const b of bullets.slice(before))if(b.normalNut)b.slimePaw=true;return ok;};
  // A separate one-percent interwave story; all selections live on the saved player.
  const pilleReward=prepareWaveReward;
  prepareWaveReward=function(){pilleReward();const nextIsWave=endlessMode?(wave+1)%5!==0:![2,5,8].includes(wave);
    if(!nextIsWave||knollQuest&&['notice','pending','hunting'].includes(knollQuest.status)||trollQuest&&['soiled','pending','fighting','reward'].includes(trollQuest.status)||player.noseFriend||Math.random()>=.01)return;
    player.noseFriend={choices:[],pending:true};saveRunNow();
  };
  const noseStatOptions=[
    {id:'damage',title:'ANGRIFFSSCHADEN',apply:p=>p.damage*=1.2},
    {id:'speed',title:'BEWEGUNG',apply:p=>p.speed*=1.2},
    {id:'rate',title:'FEUERRATE',apply:p=>p.fireRate/=1.2},
    {id:'special',title:'SPEZIALSCHADEN',apply:p=>p.specialDamage*=1.2},
    {id:'boss',title:'BOSSSCHADEN',apply:p=>p.bossDamage=(p.bossDamage||1)*1.2}
  ];
  const pilleRenderRewards=renderWaveReward;
  renderWaveReward=function(){if(player?.noseFriend?.pending&&mode==='upgrade'){
      const chosen=player.noseFriend.choices;
      showOverlay(`<span class="eyebrow">1 % EREIGNIS · FREUNDSCHAFT</span><h2 id="overlayTitle">DER NASENHAARENTFLECHTER AUS EINER DÜSTEREN VERGANGENHEIT</h2><div class="nose-cutscene" aria-label="Ein haariger neuer Freund"><span aria-hidden="true">✂</span><span aria-hidden="true">♠</span><span aria-hidden="true">♥</span></div><p>Aus dem Nebel tritt ein einst gefürchteter Nasenhaarentflechter. Er steckt seine Schere weg, schließt Freundschaft mit dir und verbessert <strong>zwei verschiedene Werte deiner Wahl um je 20 %</strong>. Wähle ${chosen.length+1} von 2.</p><div class="upgrades reward-upgrades">${noseStatOptions.filter(o=>!chosen.includes(o.id)).map(o=>`<button class="upgrade rarity-purple" id="nose-${o.id}"><small class="upgrade-kind">FREUNDSCHAFT · +20 %</small><strong>${o.title}</strong><span>Dauerhaft für diesen Run; bestehende Obergrenzen gelten.</span></button>`).join('')}</div>`);
      for(const o of noseStatOptions.filter(o=>!chosen.includes(o.id)))$('nose-'+o.id).onclick=()=>{if(!player?.noseFriend?.pending||mode!=='upgrade')return;o.apply(player);player.noseFriend.choices.push(o.id);normalizeBuild(player);updateHud();if(player.noseFriend.choices.length===2){player.noseFriend.pending=false;addAchievementProgress('pille_nose_friend');}saveRunNow();renderWaveReward();};return;
    }pilleRenderRewards();};

  // Five playable characters use exactly the same skill pool and run format.
  // The default starting weapon is exchanged before the run is saved.
  const characterRoster={
    snickers:{name:'SNICKERS',species:'Hamster',starter:'nutBomb',attack:'Nuss',damageLabel:'Nussschaden',mark:'🐹',color:'#dfa24f',hint:'Von Anfang an verfügbar.',stats:{maxHp:5,damage:1.4,fireRate:.32,speed:264,dashCooldown:2.25}},
    raffzahn:{name:'RATTE RAFFZAHN',species:'Ratte',starter:'ratWhistle',attack:'Bissbogen',damageLabel:'Bissschaden',mark:'🐀',color:'#b5a4bd',hint:'Normal New Game+2 abschließen.',attackDesc:'Kurzer Nahkampfbogen nach vorn. Trifft mehrere Ziele; jeder achte Angriff vergiftet.',stats:{maxHp:4,damage:1.35,fireRate:.27,speed:246,dashCooldown:2.10}},
    krustenbraten:{name:'KORENLIUS KRUSTENBRATEN',species:'Schwein',starter:'pigPan',attack:'Schmalzlache',damageLabel:'Schmalzschaden',mark:'🐷',color:'#eaa992',hint:'Hard New Game+2 abschließen.',attackDesc:'Setzt am Ziel eine heiße Schmalzlache. Der Einschlag trifft sofort; der Boden brennt nach.',stats:{maxHp:7,damage:1.55,fireRate:.39,speed:232,dashCooldown:2.60}},
    slanny:{name:'SLAGS SLANNY',species:'Hase',starter:'carrotCyclone',attack:'Möhrenbumerang',damageLabel:'Möhrenschaden',mark:'🐰',color:'#ccd09e',hint:'Impossible New Game+2 abschließen.',attackDesc:'Wirft Möhrenklingen, die umkehren und auf dem Rückweg erneut treffen können.',stats:{maxHp:4,damage:1.28,fireRate:.315,speed:302,dashCooldown:1.85}},
    koettitroeter:{name:'DER KÖTTITRÖTER',species:'Troll',starter:'trollFanfare',attack:'Schallfront',damageLabel:'Schallschaden',mark:'🎺',color:'#a7b48a',hint:'Endlosmodus Welle 200 überstehen (jede Schwierigkeit).',attackDesc:'Eine breite Schallfront wächst nach vorn, trifft Gruppen und drängt Gegner zurück.',stats:{maxHp:6,damage:1.62,fireRate:.38,speed:250,dashCooldown:2.40}}
  };
  const starterWeaponIds=new Set(Object.values(characterRoster).map(c=>c.starter));
  Object.assign(weaponBook,{
    ratWhistle:{name:'RAFFZAHNS TUNNELNETZ',icon:'♧',starter:true,max:3,desc:'Exklusive Startwaffe: gräbt drei Fallen um den Zielpunkt. Jede lauert sieben Sekunden, schnappt einmal nach nahen Gegnern und vergiftet sie. 3 Ladungen.'},
    pigPan:{name:'KRUSTENBRATENS BACKOFENWAGEN',icon:'♨',starter:true,max:3,desc:'Exklusive Startwaffe: schiebt einen heißen Ofenwagen nach vorn. Er brennt Gegner über 2,6 Sekunden und fängt bis zu zwölf Geschosse ab. 3 Ladungen.'},
    carrotCyclone:{name:'SLANNYS WURZELFALLE',icon:'✺',starter:true,max:3,desc:'Exklusive Startwaffe: pflanzt eine Rankenfalle am Zielpunkt. Sie schlägt dreimal zu, verlangsamt Gegner und trifft auch Bosse. 3 Ladungen.'},
    trollFanfare:{name:'KÖTTITRÖTERS RESONANZWAND',icon:'♫',starter:true,max:3,desc:'Exklusive Startwaffe: eine wandernde Schallwand. Sie stößt vor, beschädigt Gegner in Pulsen und fängt bis zu 15 Geschosse ab. 3 Ladungen.'}
  });
  // Run-specific counters reset on NG+; only the selected character and skill flags carry.
  ngPlusCarryKeys.push('characterId');
  function characterUnlocked(id){
    if(id==='snickers')return true;
    if(id==='raffzahn')return localStorage.getItem('snickers3-normal-ng2-clear-v1')==='1'||hallOfFame.some(r=>hallDifficulty(r)==='normal'&&hallTier(r)===2);
    if(id==='krustenbraten')return Boolean(hardProgress.endless)||hallOfFame.some(r=>hallDifficulty(r)==='hard'&&hallTier(r)===2);
    if(id==='slanny')return Boolean(impossibleProgress.clear)||hallOfFame.some(r=>hallDifficulty(r)==='impossible'&&hallTier(r)===2);
    if(id==='koettitroeter')return Object.values(endlessBest).some(n=>n>=200)||endlessHallOfFame.some(r=>(r.endlessCleared||0)>=200);
    return false;
  }
  let selectedCharacter='snickers';try{selectedCharacter=localStorage.getItem('snickers3-selected-character-v1')||'snickers';}catch{}
  if(!characterUnlocked(selectedCharacter))selectedCharacter='snickers';
  function preparePlayableCharacter(p,id){
    if(!characterUnlocked(id))id='snickers';
    // A selected NG+ build retains its upgrades; swap only the old hero's base
    // profile for the chosen hero's profile. Repeating a run never stacks bases.
    const source=characterRoster[p.characterId]?.stats||characterRoster.snickers.stats;
    const target=characterRoster[id].stats;
    p.maxHp+=target.maxHp-source.maxHp;p.hp=p.maxHp;
    for(const key of ['damage','fireRate','speed','dashCooldown'])p[key]*=target[key]/source[key];
    normalizeBuild(p);p.characterId=id;
    const starter=characterRoster[id].starter,oldUses=p.weaponUses||{};
    p.weapons=[starter,...p.weapons.filter(k=>!starterWeaponIds.has(k)&&weaponBook[k])];
    p.weaponUses=Object.fromEntries(p.weapons.map(k=>[k,k===starter?Math.max(weaponBook[k].max+(p.ammoBonus||0)+(p.bombBonus||0),oldUses[k]||0):Math.min(maxWeaponAmmo(k),oldUses[k]??maxWeaponAmmo(k))]));
    p.weaponIndex=0;
  }
  const pilleMaxAmmo=maxWeaponAmmo;
  maxWeaponAmmo=function(id){return pilleMaxAmmo(id)+(id!=='nutBomb'&&weaponBook[id]?.starter?(player?.bombBonus||0):0);};
  const pilleUseNewWeapon=useNewWeapon;
  useNewWeapon=function(id){
    if(!weaponBook[id]?.starter||id==='nutBomb')return pilleUseNewWeapon(id);
    const sm=player.specialDamage||1;player.weaponShots++;
    if(id==='ratWhistle'){
      const target=weaponTarget(350);player.ratBurrows??=[];
      const sx=target.x<W/2?1:-1,sy=target.y<H/2?1:-1;
      for(const offset of [[0,0],[sx*165,sy*60],[sx*60,sy*165]])player.ratBurrows.push({x:clamp(target.x+offset[0],55,W-55),y:clamp(target.y+offset[1],120,H-55),life:7,damage:15*sm});
      player.ratBurrows=player.ratBurrows.slice(-9);burst(target.x,target.y,'#c8abc8',18,110);
    }else if(id==='pigPan'){
      player.pigCarts??=[];player.pigCarts.push({x:player.x,y:player.y,a:player.angle,life:2.6,tick:0,damage:7*sm,blocked:0});
      player.pigCarts=player.pigCarts.slice(-3);burst(player.x,player.y,'#eeb4a3',18,110);
    }else if(id==='carrotCyclone'){
      const target=weaponTarget(360);player.carrotVines??=[];player.carrotVines.push({x:target.x,y:target.y,life:2.45,tick:.12,pulses:0,damage:9*sm});
      player.carrotVines=player.carrotVines.slice(-3);burst(target.x,target.y,'#f1a457',22,170);
    }else if(id==='trollFanfare'){
      player.soundWalls??=[];player.soundWalls.push({x:player.x,y:player.y,a:player.angle,life:2.4,tick:0,damage:8*sm,blocked:0});
      player.soundWalls=player.soundWalls.slice(-3);burst(player.x,player.y,'#d5e39a',18,120);
    }
    tone(220,.24,'triangle',.07,570);return true;
  };
  // The four ordinary attacks retain normal-hit upgrades, but use different
  // geometries: a bite arc, an area burn, a returning blade, and a shockfront.
  const pilleCharacterFire=fire;
  fire=function(){const start=bullets.length,ok=pilleCharacterFire();if(!ok||!player||player.characterId==='snickers')return ok;
    const newBullets=bullets.splice(start),normal=newBullets.filter(b=>b.normalNut);
    bullets.push(...newBullets.filter(b=>!b.normalNut));
    if(player.characterId==='krustenbraten'){
      const target=weaponTarget(285),damage=normal.reduce((sum,b)=>sum+b.damage,0);
      if(normal.length){bullets.push({...normal[0],x:target.x,y:target.y,vx:0,vy:0,life:.09,r:72,pierce:99,damage:damage*.60,characterAttack:'krustenbraten',groundStrike:true,hitIds:new Set()});
        player.pigPools??=[];player.pigPools.push({x:target.x,y:target.y,life:1.5,tick:.55,damage:damage*.27});player.pigPools=player.pigPools.slice(-7);}
      return ok;
    }
    for(const b of normal){b.characterAttack=player.characterId;
      if(b.characterAttack==='raffzahn'){b.x=player.x+Math.cos(b.a)*82;b.y=player.y+Math.sin(b.a)*82;b.vx=0;b.vy=0;b.life=.11;b.r=63;b.pierce=99;b.damage*=1.45;b.biteArc=true;player.biteFx=.15;}
      if(b.characterAttack==='slanny'){b.vx=Math.cos(b.a)*560;b.vy=Math.sin(b.a)*560;b.life=.92;b.r=10;b.pierce=99;b.damage*=.52;b.returnAge=0;b.returning=false;}
      if(b.characterAttack==='koettitroeter'){b.vx=Math.cos(b.a)*455;b.vy=Math.sin(b.a)*455;b.life=.52;b.r=21;b.pierce=99;b.damage*=.80;b.sonicAge=0;}
      bullets.push(b);
    }return ok;
  };
  const knastUpdateExpansion=updateExpansion;
  updateExpansion=function(dt){knastUpdateExpansion(dt);if(!player||mode!=='playing'||hasenbeinMode)return;
    player.biteFx=Math.max(0,(player.biteFx||0)-dt);
    for(const b of bullets){
      if(b.returnAge!==undefined){b.returnAge+=dt;if(!b.returning&&b.returnAge>=.31){b.returning=true;b.hitIds.clear();b.damage*=.84;}
        if(b.returning){const angle=Math.atan2(player.y-b.y,player.x-b.x);b.a=angle;b.vx=Math.cos(angle)*690;b.vy=Math.sin(angle)*690;if(dist(b,player)<27)b.life=0;}}
      if(b.sonicAge!==undefined){b.sonicAge+=dt;b.r=21+Math.min(42,b.sonicAge*100);}
    }
    for(const pool of player.pigPools||[]){pool.life-=dt;pool.tick-=dt;if(pool.tick<=0&&pool.life>0){pool.tick+=.48;
      for(const e of combatTargets())if(!e.dead&&dist(pool,e)<72+e.r){damageEnemy(e,pool.damage,true);if(mode!=='playing')return;}
    }}player.pigPools=(player.pigPools||[]).filter(p=>p.life>0);
    for(const trap of player.ratBurrows||[]){trap.life-=dt;if(trap.life<=0)continue;
      if(combatTargets().some(e=>!e.dead&&dist(trap,e)<85+e.r)){trap.life=0;burst(trap.x,trap.y,'#c4a7b7',13,100);
        for(const e of combatTargets())if(!e.dead&&dist(trap,e)<112+e.r){damageEnemy(e,trap.damage,true);if(!isBoss(e))infectEnemy(e,2.5);if(mode!=='playing')return;}
      }}player.ratBurrows=(player.ratBurrows||[]).filter(p=>p.life>0);
    for(const cart of player.pigCarts||[]){cart.life-=dt;cart.x=clamp(cart.x+Math.cos(cart.a)*190*dt,45,W-45);cart.y=clamp(cart.y+Math.sin(cart.a)*190*dt,115,H-45);
      const before=enemyBullets.length;enemyBullets=enemyBullets.filter(b=>dist(cart,b)>94||cart.blocked++>=12);addAchievementProgress('stan_pig_shield',before-enemyBullets.length);
      cart.tick-=dt;if(cart.tick<=0){cart.tick+=.42;for(const e of combatTargets())if(!e.dead&&dist(cart,e)<91+e.r){damageEnemy(e,cart.damage,true);if(mode!=='playing')return;}}
    }player.pigCarts=(player.pigCarts||[]).filter(p=>p.life>0);
    for(const vine of player.carrotVines||[]){vine.life-=dt;vine.tick-=dt;if(vine.tick<=0&&vine.pulses<3){vine.tick+=.65;vine.pulses++;
      for(const e of combatTargets())if(!e.dead&&dist(vine,e)<115+e.r){damageEnemy(e,vine.damage,true);e.slow=Math.max(e.slow||0,isBoss(e)?.7:2);if(isBoss(e))addAchievementProgress('stan_rabbit_boss');if(mode!=='playing')return;}
    }}player.carrotVines=(player.carrotVines||[]).filter(v=>v.life>0&&v.pulses<3);
    for(const wall of player.soundWalls||[]){wall.life-=dt;wall.x=clamp(wall.x+Math.cos(wall.a)*185*dt,45,W-45);wall.y=clamp(wall.y+Math.sin(wall.a)*185*dt,115,H-45);
      const wing={x:-Math.sin(wall.a)*110,y:Math.cos(wall.a)*110},a={x:wall.x-wing.x,y:wall.y-wing.y},b={x:wall.x+wing.x,y:wall.y+wing.y};
      const before=enemyBullets.length;enemyBullets=enemyBullets.filter(shot=>segmentDistance(shot,a,b)>shot.r+18||wall.blocked++>=15);addAchievementProgress('stan_troll_shield',before-enemyBullets.length);
      wall.tick-=dt;if(wall.tick<=0){wall.tick+=.52;for(const e of combatTargets())if(!e.dead&&segmentDistance(e,a,b)<e.r+42){damageEnemy(e,wall.damage,true);if(mode!=='playing')return;}}
    }player.soundWalls=(player.soundWalls||[]).filter(w=>w.life>0);
  };
  const knastDrawExpansion=drawExpansion;
  drawExpansion=function(){knastDrawExpansion();if(!player||hasenbeinMode)return;
    if(player.biteFx>0){ctx.save();ctx.strokeStyle='#f4d2cc';ctx.lineWidth=10*player.biteFx/.15;ctx.beginPath();ctx.arc(player.x,player.y,137,player.angle-.62,player.angle+.62);ctx.stroke();ctx.restore();}
    for(const p of player.pigPools||[]){ellipse(ctx,p.x,p.y,73,27,'#a75f3a66');ellipse(ctx,p.x,p.y,47,16,'#efa16a70');}
    for(const p of player.ratBurrows||[]){ellipse(ctx,p.x,p.y,32,15,'#231b2daa');ctx.strokeStyle='#d6b1c5';ctx.lineWidth=3;ctx.beginPath();ctx.arc(p.x,p.y,36,0,TAU);ctx.stroke();}
    for(const p of player.pigCarts||[]){ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.a);ctx.fillStyle='#985a4d';ctx.fillRect(-34,-31,68,62);ctx.strokeStyle='#f7bf89';ctx.lineWidth=5;ctx.strokeRect(-34,-31,68,62);ellipse(ctx,2,0,20,18,'#e5a66a');ctx.restore();}
    for(const p of player.carrotVines||[]){ctx.strokeStyle='#bde08e';ctx.lineWidth=5;for(let i=0;i<5;i++){const a=i*TAU/5;ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.quadraticCurveTo(p.x+Math.cos(a+.4)*65,p.y+Math.sin(a+.4)*65,p.x+Math.cos(a)*110,p.y+Math.sin(a)*110);ctx.stroke();}}
    for(const p of player.soundWalls||[]){const dx=-Math.sin(p.a)*110,dy=Math.cos(p.a)*110;ctx.strokeStyle='#d2eaa8';ctx.lineWidth=13;ctx.beginPath();ctx.moveTo(p.x-dx,p.y-dy);ctx.lineTo(p.x+dx,p.y+dy);ctx.stroke();}
  };
  const knastClearExtras=clearCombatExtras;
  clearCombatExtras=function(){knastClearExtras();if(player)for(const key of ['pigPools','ratBurrows','pigCarts','carrotVines','soundWalls'])player[key]=[];};
  skins.push({id:'superSaiyajin',name:'SNICKERS SUPER SAIYAJIN',mark:'⚡',color:'#ffe873',hint:'Alle Achievements freischalten.'});
  const skinNames={
    raffzahn:['RAFFZAHN ORIGINAL','RATTENDETEKTIV','KÄSEKOCH','OTTAHS KELLERRATTE','BLUTSAUGER-RATTE','RATTENROCKER','HUMMELDIEB','KANALWART','HUNDERTSCHWANZ','BLUTPFOTEN','SCHRECKENSRATTE','SCHIMMELSPÜRER','SCHATTENRATTE','KÄSEKAISER','FAULSCHWANZ','MODER-DJ','WELLENRÄUBER','HASENBEINS ERZFEIND','RAFFZAHN SUPER SAIYAJIN'],
    krustenbraten:['KORENLIUS ORIGINAL','SPECKDETEKTIV','BRATENKOCH','OTTAHS KRUSTENBUTLER','BLUTBAD-FERKEL','GRILLROCKER','HUMMELSCHWEIN','WELLENKOCH','HUNDERT-SPECK','ROTE KRUSTE','SCHRECKENSBACON','VERDORBENER KOCH','SCHATTENSCHWEIN','SCHWEINEKAISER','SCHIMMELSCHNITZEL','MODERGRILLER','WELLENBRATEN','HASENBEINS KÜCHENCHEF','KORENLIUS SUPER SAIYAJIN'],
    slanny:['SLANNY ORIGINAL','SPURENHASE','MÖHRENKOCH','OTTAHS HOPPELMAJOR','ROTOHR','HASENROCKER','HUMMELHOPPER','WELLENLÖFFEL','HUNDERT-HOPPS','BLUTLÖFFEL','SCHRECKENSHASE','MOORHOPPLER','SCHATTENLÖFFEL','HASENKAISER','FAULPFOTE','MODERSPRINGER','WELLENHASE','HASENBEINS ALBTRAUMHASE','SLANNY SUPER SAIYAJIN'],
    koettitroeter:['KÖTTITRÖTER ORIGINAL','TROMPETENDETEKTIV','SAHNETRÖTER','OTTAHS KONZERT','BLUTJAZZ','KRUSTENBRASS','HUMMELFANFARE','WELLENTAKT','HUNDERT TÖNE','ROTE NOTE','SCHRECKENSHORN','VERDORBENER JAZZ','SCHATTENTROLL','TROLLKAISER','SCHIMMELMARSCH','MODER-DJ','WELLENORCHESTER','HASENBEINS ENDTON','KÖTTITRÖTER SUPER SAIYAJIN']
  };
  const baseSkinTemplates=[...skins];
  for(const [id,names] of Object.entries(skinNames))for(const [i,s] of baseSkinTemplates.entries())skins.push({...s,id:`${id}__${s.id}`,name:names[i],mark:i===0?characterRoster[id].mark:s.mark,characterId:id,templateId:s.id,hint:i===0?characterRoster[id].hint:s.id==='superSaiyajin'?'Alle Achievements freischalten.':s.id==='hasenbeinVictor'?`Im Hasenbein-Modus einen Hall-of-Fame-Build von ${characterRoster[id].name} besiegen.`:s.hint+' (mit dieser Figur)'});
  let selectedSkins={snickers:selectedSkin};try{Object.assign(selectedSkins,JSON.parse(localStorage.getItem('snickers3-selected-skins-v1')||'{}'));}catch{}
  function allAchievementsWon(){return achievements.every(a=>achievementUnlocked(a.id));}
  const pilleBaseSkins=unlockedSkins;
  let characterEndlessBest={};try{characterEndlessBest=JSON.parse(localStorage.getItem('snickers3-character-endless-best-v1')||'{}')||{};}catch{}
  const hasenbeinSkinKey='snickers3-hasenbein-skin-v1';
  let hasenbeinSkinWins=new Set();try{hasenbeinSkinWins=new Set(JSON.parse(localStorage.getItem(hasenbeinSkinKey)||'[]'));}catch{}
  // Old achievements did not record the defeated character. Those wins were
  // against Snickers; never grant the four other costumes from them.
  if(localStorage.getItem(hasenbeinSkinKey)===null&&['normal','hard','impossible'].some(d=>achievementUnlocked('hasenbein_'+d))){hasenbeinSkinWins.add('snickers');try{localStorage.setItem(hasenbeinSkinKey,JSON.stringify([...hasenbeinSkinWins]));}catch{}}
  const knastHasenbeinDuel=endHasenbeinDuel;
  endHasenbeinDuel=function(won){if(won&&mode==='playing'){
      hasenbeinSkinWins.add(hasenbeinRival?.characterId||'snickers');try{localStorage.setItem(hasenbeinSkinKey,JSON.stringify([...hasenbeinSkinWins]));}catch{}
    }return knastHasenbeinDuel(won);};
  const pilleEndlessBest=recordEndlessBest;
  recordEndlessBest=function(survived){pilleEndlessBest(survived);if(!player?.characterId||player.characterId==='snickers')return;
    const best=characterEndlessBest[player.characterId]??={normal:0,hard:0,impossible:0};
    for(const difficulty of ['normal',...(hardMode?['hard']:[]),...(impossibleMode?['impossible']:[])])best[difficulty]=Math.max(best[difficulty]||0,survived);
    try{localStorage.setItem('snickers3-character-endless-best-v1',JSON.stringify(characterEndlessBest));}catch{}
  };
  unlockedSkins=function(){const result=pilleBaseSkins();result.delete('hasenbeinVictor');if(hasenbeinSkinWins.has('snickers'))result.add('hasenbeinVictor');if(allAchievementsWon())result.add('superSaiyajin');
    for(const id of Object.keys(skinNames)){if(!characterUnlocked(id))continue;
      result.add(`${id}__classic`);
      const records=hallOfFame.filter(r=>(r.build?.characterId||'snickers')===id),best=characterEndlessBest[id]||{};
      const wins=(d,t)=>records.some(r=>hallDifficulty(r)===d&&hallTier(r)>=t);
      for(const [difficulty,prefix] of [['normal','normal'],['hard','hard'],['impossible','impossible']])for(let t=0;t<3;t++)if(wins(difficulty,t))result.add(`${id}__${prefix}${t}`);
      const normalBest=Math.max(best.normal||0,best.hard||0,best.impossible||0),hardBest=Math.max(best.hard||0,best.impossible||0),impBest=best.impossible||0;
      for(const [template,n] of [['endless50',50],['endless100',100],['hard50',50],['hard100',100],['impossibleEndless10',10],['impossibleEndless50',50],['impossibleEndless100',100]])if((template.startsWith('impossible')?impBest:template.startsWith('hard')?hardBest:normalBest)>=n)result.add(`${id}__${template}`);
      if(hasenbeinSkinWins.has(id))result.add(`${id}__hasenbeinVictor`);
      if(allAchievementsWon())result.add(`${id}__superSaiyajin`);
    }return result;
  };
  function selectCharacter(id){if(!characterUnlocked(id))return;selectedCharacter=id;
    try{localStorage.setItem('snickers3-selected-character-v1',id);}catch{}
    const owned=unlockedSkins(),choice=selectedSkins[id]||(id==='snickers'?'classic':`${id}__classic`);
    selectedSkin=owned.has(choice)?choice:id==='snickers'?'classic':`${id}__classic`;
    if(selectedSkins[id]!==selectedSkin){selectedSkins[id]=selectedSkin;try{localStorage.setItem('snickers3-selected-skins-v1',JSON.stringify(selectedSkins));}catch{}}
    updateNGPlusMenu();
  }
  ...
  function showRunCharacterPick(level,hard=false,impossible=false,build=null){
    mode='runCharacter';const difficulty=impossible?'IMPOSSIBLE':hard?'HARD':'NORMAL',choice=characterRoster[selectedCharacter],stat=choice.stats;
    pilleShowOverlay(`<span class="eyebrow">${difficulty} · ${level===3?'ENDLOSMODUS':level===0?'KAMPAGNE':tierName(level)} · LETZTER SCHRITT</span><h2 id="overlayTitle">MIT WEM SPIELST DU?</h2><p>Wähle die Figur für diesen Run. Ein übernommener Build behält seine Skills und Werteverbesserungen; die Grundwerte und exklusive Startwaffe wechseln zur gewählten Figur.</p><div class="run-character-grid">${Object.entries(characterRoster).map(([id,c])=>`<button class="run-character-card" id="runChar-${id}" ${characterUnlocked(id)?'':'disabled'} aria-pressed="${id===selectedCharacter}" style="--skin:${c.color}"><span>${c.mark}</span><strong>${c.name}</strong><small>${c.attackDesc||'Klassische Nuss-Salven aus der Distanz.'}</small><b>♥ ${c.stats.maxHp} · Schaden ${c.stats.damage.toLocaleString('de-DE')} · ${Number(1/c.stats.fireRate).toLocaleString('de-DE',{maximumFractionDigits:2})} Salven/s · Tempo ${c.stats.speed}</b><em>${weaponBook[c.starter].name}</em><small>${weaponBook[c.starter].desc}</small>${characterUnlocked(id)?'':`<small>GESPERRT · ${c.hint}</small>`}</button>`).join('')}</div><p class="run-character-focus"><strong>${choice.name}:</strong> ${choice.attackDesc||'Klassische Nuss-Salven aus der Distanz.'} <strong>${weaponBook[choice.starter].name}:</strong> ${weaponBook[choice.starter].desc} Dash ${stat.dashCooldown.toLocaleString('de-DE')} s.</p><div class="overlay-actions"><button class="primary-button" id="runCharacterGo">MIT ${choice.name} STARTEN ↗</button><button class="secondary-button" id="runCharacterBack">ZURÜCK ZUM MENÜ</button></div>`);
    for(const id of Object.keys(characterRoster))if(characterUnlocked(id))$('runChar-'+id).onclick=()=>{selectCharacter(id);showRunCharacterPick(level,hard,impossible,build);};
    $('runCharacterGo').onclick=()=>{if(mode!=='runCharacter')return;endlessSelectedBuild=level===3?build:null;endlessFromHall=Boolean(level===3&&build);startGame(level,hard,impossible,level===1||level===2?build:null);};
    $('runCharacterBack').onclick=goMenu;
  }
  ...
  showSkins=function(fromPause=false){if(fromPause?mode!=='paused':mode!=='menu')return;
    ...
    if(!owned.has(selectedSkin)||!choices.some(s=>s.id===selectedSkin))selectedSkin=id==='snickers'?'classic':`${id}__classic`;
    showOverlay(`<span class="eyebrow">${characterRoster[id].name} · GARDEROBE</span><h2 id="overlayTitle">${choices.length} SKINS FÜR ${characterRoster[id].species.toUpperCase()}</h2><p>Jede Figur hat ihre eigenen Outfits. Kampagnen- und Endlosabschlüsse zählen für die Figur, mit der du gespielt hast. Hasenbein-Outfits erhält nur die Figur, deren Hall-Build du als Hasenbein besiegt hast. Alte eindeutige Siege zählen rückwirkend.</p><div class="skin-grid">${choices.map(s=>`<button class="skin-card" id="skin-${s.id}" style="--skin:${s.color}" ${owned.has(s.id)?'':'disabled'} aria-pressed="${selectedSkin===s.id}"><span class="skin-icon">${s.mark}</span><strong>${s.name}</strong><small>${owned.has(s.id)?'FREIGESCHALTET · ':''}${s.hint}</small></button>`).join('')}</div><button class="secondary-button" id="skinBack">← ${fromPause?'ZUR PAUSE':'ZUM MENÜ'}</button>`);
    for(const s of choices)if(owned.has(s.id))$('skin-'+s.id).onclick=()=>{selectedSkin=s.id;selectedSkins[id]=s.id;
      try{localStorage.setItem('snickers3-selected-skins-v1',JSON.stringify(selectedSkins));if(id==='snickers')localStorage.setItem('snickers3-selected-skin-v1',s.id);}catch{}
      for(const x of choices)$('skin-'+x.id).setAttribute('aria-pressed',String(x.id===s.id));};
    $('skinBack').onclick=()=>{if(fromPause){mode='playing';pauseGame();}else{mode='menu';hideOverlay();}};
  };
  const pilleDrawCharacter=drawHamster;
  drawHamster=function(p,alpha=1){const id=p?.characterId;if(!id||id==='snickers'||hasenbeinMode&&p===player){pilleDrawCharacter(p,alpha);return;}
    if(mode==='victory'&&player?.bossDance&&p===player){pilleDrawCharacter(p,alpha);return;}
    const c=ctx,bob=p.moving?Math.sin(ambientTime*17)*2:Math.sin(ambientTime*3)*.7;
    c.save();c.translate(p.x,p.y+bob);c.scale(p.face||1,1);c.globalAlpha=p.invuln>0&&Math.floor(ambientTime*14)%2===0?alpha*.65:alpha;
    ellipse(c,0,15,23,7,'#08151c88');
    if(id==='raffzahn'){
      c.strokeStyle='#d69bac';c.lineWidth=5;c.lineCap='round';c.beginPath();c.moveTo(-15,12);c.bezierCurveTo(-49,35,-50,-5,-37,-6);c.stroke();
      ellipse(c,0,3,18,17,'#a89bb1');ellipse(c,-11,-15,10,11,'#bcaabb');ellipse(c,11,-15,10,11,'#bcaabb');
      ellipse(c,-11,-15,6,7,'#e1b3bb');ellipse(c,11,-15,6,7,'#e1b3bb');ellipse(c,0,-3,17,14,'#c7b3c2');
      ellipse(c,2,4,8,5,'#f0d1c8');ellipse(c,4,2,3,2,'#8b5970');
      c.strokeStyle='#f4d9d2';c.lineWidth=1;for(const y of [-1,4]){c.beginPath();c.moveTo(7,y);c.lineTo(25,y-3);c.moveTo(-2,y);c.lineTo(-22,y+3);c.stroke();}
      ellipse(c,-6,-8,3,4,'#1c2930');ellipse(c,9,-8,3,4,'#1c2930');
    }else if(id==='krustenbraten'){
      ellipse(c,0,5,22,19,'#b87f77');path(c,[[-17,-10],[-24,-31],[-6,-17]],'#d2948a');path(c,[[17,-10],[24,-31],[6,-17]],'#d2948a');
      ellipse(c,0,-5,19,17,'#e7a99a');ellipse(c,0,2,11,8,'#f4bdb0');ellipse(c,-4,2,2,2,'#a45b60');ellipse(c,4,2,2,2,'#a45b60');
      ellipse(c,-7,-10,3,3,'#273038');ellipse(c,8,-10,3,3,'#273038');path(c,[[-18,14],[18,14],[16,20],[-16,20]],'#784e4e');
    }else if(id==='slanny'){
      ellipse(c,-10,-31,7,25,'#c9cbaa',-.10);ellipse(c,10,-32,7,27,'#c9cbaa',.11);ellipse(c,-10,-31,3,19,'#e8a2a7',-.10);ellipse(c,10,-32,3,21,'#e8a2a7',.11);
      ellipse(c,0,6,17,18,'#c2c5a1');ellipse(c,0,-5,17,17,'#e0dfbe');ellipse(c,-7,-9,3,4,'#2b3240');ellipse(c,8,-9,3,4,'#2b3240');ellipse(c,0,0,3,2,'#c98487');
      ellipse(c,-7,8,8,6,'#f2e9d2');ellipse(c,8,8,8,6,'#f2e9d2');
    }else if(id==='koettitroeter'){
      ellipse(c,0,8,24,21,'#7d9274');path(c,[[-18,-15],[-30,-25],[-20,-3]],'#a9bd89');path(c,[[18,-15],[30,-25],[20,-3]],'#a9bd89');
      ellipse(c,0,-5,20,18,'#abbf91');ellipse(c,-8,-9,3,4,'#24332d');ellipse(c,9,-9,3,4,'#24332d');ellipse(c,0,4,10,7,'#778d73');
      c.fillStyle='#745367';c.fillRect(-14,-23,29,8);c.fillStyle='#ddc286';c.fillRect(13,-1,24,6);ellipse(c,38,2,9,10,'#eccf89');
    }
    if(p.madelpulator){for(const x of [-7,8])ellipse(c,x,-8,4,3,'#ff3838');}
    c.restore();if(p.shieldReady||p.dashTime>0){c.save();c.strokeStyle=accent();c.lineWidth=3;c.beginPath();c.arc(p.x,p.y,31,0,TAU);c.stroke();c.restore();}
    if(mode!=='victory'||!player?.bossDance)drawSkin(p,alpha);
  };
  const pilleShowOverlay=showOverlay;
  function characterText(value,id){if(!id||id==='snickers')return value;const c=characterRoster[id],plural={raffzahn:'Bisse',krustenbraten:'Schmalzlachen',slanny:'Möhrenbumerangs',koettitroeter:'Schallfronten'}[id];
    const keep=Object.values(weaponBook).map(w=>w.name).filter(name=>name.includes('NUSS'));let content=value;
    keep.forEach((name,i)=>{content=content.replaceAll(name,`¤${i}¤`);});
    content=content.replace(/\bSnickers\b/g,c.name).replace(/\bSNICKERS\b/g,c.name)
      .replace(/NUSS-SCHADEN|NUSSSCHADEN|Nussschaden/g,word=>word===word.toUpperCase()?c.damageLabel.toUpperCase():c.damageLabel)
      .replace(/Nusstreffer|Nuss-Treffer/g,'Treffer')
      .replace(/\beine Nuss\b/gi,'einen Angriff').replace(/\bJede Nuss\b/g,'Jeder Angriff')
      .replace(/\bNüssen\b|\bNüsse\b/g,plural)
      .replace(/\bNuss\b/g,c.attack).replace(/\bNUSS\b/g,c.attack.toUpperCase())
      .replace(/\bNUSS(?=[A-ZÄÖÜ])/g,c.attack.toUpperCase()).replace(/\bNuss(?=[a-zäöü])/g,c.attack)
      .replace(/\bHamster\b|\bHAMSTER\b/g,c.species);
    keep.forEach((name,i)=>{content=content.replaceAll(`¤${i}¤`,name);});return content;
  }
  function characterMarkup(html,id){return id&&id!=='snickers'?html.replace(/>([^<]*)</g,(_,part)=>'>'+characterText(part,id)+'<'):html;}
  showOverlay=function(html){if(['characters','campaignSelect','hallOfFame','hallDetail','endlessSelect','hasenbeinSelect','achievements'].includes(mode))return pilleShowOverlay(html);
    const menuContext=['menu','modeSelect'].includes(mode)||['skins','share'].includes(mode)&&!overlayFromPause;
    const id=hasenbeinMode?'snickers':menuContext?selectedCharacter:player?.characterId||selectedCharacter;pilleShowOverlay(characterMarkup(html,id));};
  const pilleBaseStats=buildStatsMarkup;
  buildStatsMarkup=function(p,live=false){const base=characterRoster[p?.characterId]?.stats||characterRoster.snickers.stats;
    let markup=pilleBaseStats(p,live).replace('Start 1,4','Start '+displayNumber(base.damage)).replace('Basis 264','Basis '+displayNumber(base.speed));
    if(p?.characterId&&p.characterId!=='snickers')markup=markup.replace('THEORETISCHER FRONT-DPS','BASIS-SALVEN-DPS').replace('Alle Frontnüsse treffen; mit Krit, ohne Begleiter/Procs','Rechenwert vor Reichweite, Flächenschaden und Rückweg; ohne Begleiter/Procs');
    return markup;};
  const pilleBuildDetails=buildDetailsMarkup;
  buildDetailsMarkup=function(p,live=false){return characterMarkup(pilleBuildDetails(p,live),p?.characterId||'snickers');};
  const pilleSkillsHud=updateSkills;
  updateSkills=function(){pilleSkillsHud();if(player?.characterId&&player.characterId!=='snickers'){
    const hud=$('skillHud');hud.innerHTML=characterMarkup(hud.innerHTML,player.characterId).replace(/title="([^"]*)"/g,(_,title)=>`title="${characterText(title,player.characterId)}"`);
  }};
  const pilleHud=updateHud;
  updateHud=function(){pilleHud();if(player&&!hasenbeinMode){const name=$('playerNameLabel');if(name)name.textContent=characterRoster[player.characterId||'snickers'].name;}};
  const pilleResume=resumeSavedRun;
  resumeSavedRun=function(key=RUN_SAVE_KEY){pilleResume(key);if(player?.characterId&&characterUnlocked(player.characterId))selectCharacter(player.characterId);updateHud();};
  const pilleStartGame=startGame;
  startGame=function(...args){pilleStartGame(...args);if(mode==='intro'&&player?.characterId&&player.characterId!=='snickers'){
    const c=characterRoster[player.characterId],setting=endlessMode?'ENDLOSMODUS':impossibleMode?'IMPOSSIBLE':hardMode?'HARD MODE':gamePlusLevel?'NEW GAME+':'NORMALE HASENJAGD';
    showOverlay(`<span class="eyebrow">${setting} · ${c.species.toUpperCase()}</span><h2 id="overlayTitle">${c.name} BETRITT DEN GARTEN.</h2><p>${c.name} kämpft mit ${c.attack} statt normaler Nussgeschosse. Die exklusive Startwaffe <strong>${weaponBook[c.starter].name}</strong> ist geladen. Alle bisherigen Skills bleiben wählbar und passen sich im Build an diese Figur an; deine gewählte Hall-of-Fame-Vorlage bleibt erhalten.</p><blockquote class="snickers-quote"><b>${c.name}</b>„Heute schreibe ich meine eigene Gartenlegende!“</blockquote><button class="primary-button" id="introButton">${setting} STARTEN ↗</button>`);
    $('introButton').onclick=beginFirstWave;
  }};

  const pilleHelp=helpMarkup;
  helpMarkup=function(){return pilleHelp().replace('3 Retries je Run;', '3 Retries je Run;').replace('Der Timer beendet den Nachschub.', 'Der Timer beendet den Nachschub. Beim Sieg werden alle übrig gebliebenen Nüsse, Herzen, Power-ups und Munitionskisten eingesammelt.').replace('Skins sind rein optisch.', 'Vor jedem neuen Run wählst du eine von fünf Figuren. Raffzahn beißt im Nahkampf und legt Tunnel-Fallen; Korenlius setzt Schmalzlachen und einen Backofenwagen ein; Slannys Möhrenbumerangs kehren zurück und er pflanzt Rankenfallen; der Köttitröter schickt Schallfronten und eine wandernde Resonanzwand los. Die Startwaffen benutzen E und haben eigene Munition. Jeder Hasenbein-Skin gehört nur zur Figur des besiegten Hall-Builds. Skins sind rein optisch.').replace('Munition wird auf den Abschnittsstart zurückgesetzt,', 'Munitionskisten fallen etwa doppelt so häufig, füllen weiterhin 50 % auf. Munition wird auf den Abschnittsstart zurückgesetzt,');};

  // Stanislav: campaign stages keep their difficulty, but each hero gets a
  // separate visual world. Endless combines the four worlds for every hero.
  const heroStageThemes={
    raffzahn:[['KÄSEKANAL','#33474e','#122932','#c6d7ad'],['NEON-RATTENLABOR','#3d4260','#1b2540','#dfc499'],['ROSTIGER SCHROTTGANG','#665d53','#292f37','#b6d5cf'],['KÄSE-TRESOR','#77704a','#352f2b','#f1d88f'],['OTTAHS VERBOTENE KÄSEKAMMER','#634950','#251f31','#f3d18c']],
    krustenbraten:[['KNUSPERBÄCKEREI','#a77e61','#563d38','#ffdeb2'],['GEHEIME BRATENKÜCHE','#9f655e','#432f3a','#ffd1ac'],['SCHINKENRÄUCHEREI','#5e564d','#282e32','#e5ae83'],['KARAMELL-OFEN','#a66242','#4a2931','#ffd889'],['OTTAHS SAHNE-BACKSTUBE','#ac8176','#533848','#fff0bc']],
    slanny:[['FREIES HASENMOOR','#657d73','#283a46','#bde9a4'],['VERBOTENER MÖHRENPFAD','#6b7382','#34334f','#e6c893'],['GLASWURZEL-HÖHLE','#4c6576','#262d48','#b8e8e1'],['MONDLICHT-LICHTUNG','#687498','#33344e','#e4eab0'],['OTTAHS WURZELKATAKOMBEN','#58606d','#292b3b','#e9b08d']],
    koettitroeter:[['MOOSIGE PROBEBÜHNE','#43615b','#192d33','#d5e3a2'],['MESSING-ORCHESTERWERK','#5b6358','#27343e','#f5dd90'],['FELSEN-RESONANZSAAL','#616773','#323448','#c4daf4'],['MITTERNACHTS-KONZERT','#48536c','#202539','#efd59a'],['OTTAHS LETZTE ZUGABE','#685363','#2c2939','#f4bd92']]
  };
  function getStageName(index=stageVisual){if(endlessMode||hasenbeinMode)return stageNames[index]||stageNames[0];return heroStageThemes[player?.characterId]?.[Math.min(4,index)]?.[0]||stageNames[index]||stageNames[0];}
  const stanislavGround=makeGround;
  makeGround=function(){stanislavGround();if(!player||hasenbeinMode)return;
    if(endlessMode){
      // The old random stage remains visible underneath four mixed districts.
      const tiles=[['raffzahn','#9bc8d5','♧'],['krustenbraten','#f4ad86','✽'],['slanny','#b6d89a','❀'],['koettitroeter','#e7d08d','♫']];
      tiles.forEach(([id,color,mark],i)=>{const x=i%2?W-260:30,y=i<2?112:H-228;g.fillStyle=color+'23';g.fillRect(x,y,225,106);g.strokeStyle=color+'8c';g.lineWidth=4;g.strokeRect(x,y,225,106);g.fillStyle=color;g.textAlign='center';g.font='bold 40px Arial';g.fillText(mark,x+112,y+66);});
      g.fillStyle='#f5e9ccaa';g.font='bold 14px Arial';g.textAlign='center';g.fillText('VIER WELTEN · EINE ENDLOSE JAGD',W/2,103);return;
    }
    const id=player.characterId;if(!heroStageThemes[id])return;
    const [name,top,bottom,accent]=heroStageThemes[id][Math.min(4,stageVisual)];
    const gradient=g.createLinearGradient(0,80,W,H);gradient.addColorStop(0,top);gradient.addColorStop(.55,bottom);gradient.addColorStop(1,top);
    g.fillStyle=gradient;g.fillRect(0,0,W,H);
    if(id==='raffzahn'){
      // Brick tunnels, manholes, drainpipes and cable tracks.
      for(let y=101;y<H-45;y+=41)for(let x=-85+(Math.floor(y/41)%2)*48;x<W;x+=98){g.fillStyle=accent+'0f';g.fillRect(x+2,y+2,93,36);g.strokeStyle=accent+'47';g.lineWidth=1.5;g.strokeRect(x+2,y+2,93,36);}
      for(const x of [86,238,1040,1192]){g.strokeStyle=accent+'75';g.lineWidth=13;g.beginPath();g.moveTo(x,95);g.bezierCurveTo(x+16,245,x-17,510,x,H-59);g.stroke();g.strokeStyle='#112634';g.lineWidth=4;g.stroke();}
      for(const [x,y] of [[210,275],[1060,430],[640,564]]){ellipse(g,x,y,60,28,accent+'18');g.strokeStyle=accent+'85';g.lineWidth=4;g.beginPath();g.ellipse(x,y,58,27,0,0,TAU);g.stroke();for(let k=-2;k<=2;k++){g.beginPath();g.moveTo(x+k*16,y-17);g.lineTo(x+k*16,y+17);g.stroke();}}
      if(stageVisual>=3){g.strokeStyle=accent+'9b';g.lineWidth=14;g.strokeRect(33,149,186,151);g.fillStyle=accent+'38';g.fillRect(69,185,113,77);g.font='bold 42px Arial';g.fillStyle=accent;g.fillText('⌖',123,240);}
    }else if(id==='krustenbraten'){
      // Warm kitchen checkerboard, ovens, baking trays and steam.
      for(let y=94;y<H-43;y+=66)for(let x=0;x<W;x+=66){g.fillStyle=((x/66+Math.floor((y-94)/66))%2)?accent+'17':'#171c233a';g.fillRect(x+2,y+2,62,62);}
      for(const x of [53,1005]){g.fillStyle='#171a2199';g.fillRect(x,172,222,108);g.strokeStyle=accent+'a0';g.lineWidth=5;g.strokeRect(x,172,222,108);for(const dx of [58,159]){ellipse(g,x+dx,207,29,11,accent+'67');ellipse(g,x+dx,207,22,6,'#1a2024');}}
      for(let i=0;i<7;i++){const x=110+i*173;ellipse(g,x,H-84,34,18,accent+'5a');g.strokeStyle=accent+'aa';g.lineWidth=3;g.beginPath();g.moveTo(x-24,H-84);g.quadraticCurveTo(x,H-110,x+24,H-84);g.stroke();}
      if(stageVisual>=3){g.fillStyle=accent+'40';g.fillRect(419,94,440,52);g.font='bold 28px Arial';g.fillStyle=accent;g.fillText('♨ SAHNEKÜCHE ♨',475,130);}
    }else if(id==='slanny'){
      // Open marsh with winding footpaths, roots, carrot patches and moon pools.
      g.fillStyle=accent+'17';g.beginPath();g.moveTo(398,H);g.bezierCurveTo(264,530,928,310,822,94);g.lineTo(985,94);g.bezierCurveTo(1080,340,436,540,568,H);g.fill();
      for(let i=0;i<37;i++){const x=60+sr()*(W-120),y=119+sr()*(H-197);ellipse(g,x,y,21+sr()*27,6+sr()*12,i%4===0?'#95cddd33':accent+'27');g.strokeStyle=accent+'92';g.lineWidth=2;g.beginPath();g.moveTo(x,y);g.quadraticCurveTo(x-12,y-19,x-14,y-22);g.moveTo(x,y);g.quadraticCurveTo(x+12,y-17,x+15,y-20);g.stroke();}
      for(const x of [53,236,1053,1227]){g.strokeStyle=accent+'88';g.lineWidth=13;g.beginPath();g.moveTo(x,678);g.bezierCurveTo(x+88,575,x-66,384,x+20,280);g.stroke();}
      if(stageVisual>=2){for(const [x,y] of [[135,303],[1110,422],[281,565]])path(g,[[x-30,y+29],[x,y-60],[x+43,y+29]],'#a8d6e138');}
    }else{
      // Timber stage boards, angled spotlights, brass horns and speakers.
      for(let y=99;y<H-34;y+=48){g.fillStyle=accent+'12';g.fillRect(0,y,W,43);g.strokeStyle=accent+'6b';g.lineWidth=2;g.beginPath();g.moveTo(0,y);g.lineTo(W,y);g.stroke();for(let x=80+(Math.floor(y/48)%2)*117;x<W;x+=235){g.beginPath();g.moveTo(x,y);g.lineTo(x,y+43);g.stroke();}}
      for(const x of [170,410,870,1110]){path(g,[[x-35,93],[x+35,93],[x+120,H-59],[x-120,H-59]],accent+'15');ellipse(g,x,103,35,16,'#1a263399');ellipse(g,x,103,18,10,accent+'81');}
      for(const x of [33,1138]){g.fillStyle='#172332';g.fillRect(x,193,109,174);g.strokeStyle=accent+'8a';g.lineWidth=4;g.strokeRect(x,193,109,174);ellipse(g,x+54,305,35,35,accent+'72');ellipse(g,x+54,228,17,17,accent+'62');}
      if(stageVisual>=2){g.fillStyle=accent+'ad';g.font='bold 48px Arial';g.fillText('♫',589,402);}
    }
    g.fillStyle=accent+'28';g.fillRect(0,0,W,88);g.fillRect(0,H-52,W,52);
    g.strokeStyle=accent+'ba';g.lineWidth=7;g.beginPath();g.moveTo(28,92);g.lineTo(W-28,92);g.moveTo(28,H-54);g.lineTo(W-28,H-54);g.stroke();
    for(let i=0;i<18;i++){
      const x=55+(i%9)*145,y=i<9?131:H-101;
      g.save();g.translate(x,y);g.strokeStyle=accent;g.fillStyle=accent+'99';g.lineWidth=3;
      if(id==='raffzahn'){
        g.beginPath();g.arc(0,0,23,0,TAU);g.stroke();g.fillRect(-8,-7,16,14);g.beginPath();g.moveTo(-31,5);g.lineTo(-11,5);g.moveTo(11,5);g.lineTo(31,5);g.stroke();
      }else if(id==='krustenbraten'){
        g.beginPath();g.ellipse(0,7,27,12,0,0,TAU);g.fill();g.beginPath();g.moveTo(-18,5);g.lineTo(-13,-15);g.lineTo(12,-15);g.lineTo(18,5);g.stroke();g.fillRect(-7,-11,14,5);
      }else if(id==='slanny'){
        g.beginPath();g.moveTo(-25,14);g.quadraticCurveTo(0,-28,24,15);g.stroke();g.beginPath();g.ellipse(0,7,9,17,0,0,TAU);g.fill();g.beginPath();g.moveTo(-7,-6);g.lineTo(-15,-16);g.moveTo(7,-6);g.lineTo(13,-16);g.stroke();
      }else{
        g.beginPath();g.arc(-8,4,12,0,TAU);g.stroke();g.beginPath();g.moveTo(4,2);g.lineTo(28,-7);g.lineTo(28,10);g.closePath();g.fill();g.font='bold 23px Arial';g.fillText('♪',-15,-17);
      }
      g.restore();
    }
    g.fillStyle=accent+'66';g.font='bold 23px Arial';g.textAlign='center';g.fillText(name,W/2,48);
    // Difficulty violence stays clearly visible over each new world.
    if(hardMode){g.fillStyle=impossibleMode?'#35101d47':'#811b243d';g.fillRect(0,100,W,H-130);
      for(let i=0;i<(impossibleMode?50:37);i++){const x=60+sr()*(W-120),y=122+sr()*(H-190),r=8+sr()*29;ellipse(g,x,y,r,r*.32,i%3?'#6b182b9c':'#b42e3bb0');}
      if(impossibleMode)for(let i=0;i<30;i++){const x=sr()*W,y=130+sr()*(H-210);g.strokeStyle='#171c25c0';g.lineWidth=2;g.beginPath();g.moveTo(x-22,y-11);g.lineTo(x,y);g.lineTo(x+18,y+15);g.stroke();}
    }
    g.fillStyle='#0b182188';g.fillRect(0,0,W,18);g.fillRect(0,H-18,W,18);
  };

  const heroStory={
    raffzahn:{relic:'goldener Käsekompass',dance:'DEN SCHWANZKREISEL',
      intro:['DER KÄSEKOMPASS IST WEG.','Raffzahn kartiert gerade die besten Käsestellen im Kanal. Hasenbein stiehlt seinen goldenen Käsekompass und flüchtet durch den Abfluss. Die Ratte folgt seinen Spuren bis in den Garten.','Mein Kompass zeigt nur noch in Richtung Ärger.'],
      plus:['DER KANAL HAT EIN GEDÄCHTNIS.','Raffzahn hat seinen Kompass zurück. Im NG+ locken Hasenbein und Karnil ihn erneut ins Rattenlabor; dieses Mal wartet hinter jeder Biegung eine Falle.','Diesmal bringe ich eine ganze Karte für euch mit.'],
      plus2:['DIE LETZTE KÄSESPUR.','Ein Gerücht führt Raffzahn in Ottahs geheime Käsekammer. Pafti bewacht die Tür; Raffzahn packt seine alten Tricks und den erkämpften Build ein.','Wer meinen Käse versteckt, unterschätzt meine Nase.'],
      act1:['FALSCHE FÄHRTE IM KANAL.','Hasenbein fällt. Der Käsekompass zeigt plötzlich nach unten: Unter dem Kanal surrt ein Neonlabor. Cyber-Hasenbein hat die Spur absichtlich gelegt.','Ich rieche Ozon. Und sehr schlechte Ausreden.'],
      act2:['DIE SPUR FÜHRT ZUM SCHROTT.','Der Cyberhase ist ausgeschaltet. Sein letztes Signal führt Raffzahn in einen Schrottgang, wo Karnil den gestohlenen Kompass in seinem Felsbau versteckt.','Schrott ist auch nur Käse mit weniger Geschmack.'],
      ottah:['HINTER DEM TRESOR GRUNZT ES.','Raffzahn befreit den Kompass aus Karnils Geröll. Ottah sitzt auf der letzten Schatztruhe; Pafti hat schon seinen Controller gezückt.','Der Käse gehört mir. Die Rechnung euch.'],
      pafti:['PAFTI DRÜCKT DEN RESET-KNOPF.','Der Mann mit dem SNES-Controller als Kopf springt aus einer Kanalluke und heilt Karnil um 25 % seiner maximalen Lebenspunkte. Raffzahn rollt die Karte wieder aus.','Ich habe deine Abkürzung längst markiert, Pafti.'],
      victory:['RAFFZAHNS KÄSEKOMPASS ZEIGT HEIMWÄRTS.','Im befreiten Kanal verteilt Raffzahn Käse an die übrigen Ratten. Hasenbeins Truppe muss künftig den Müll sortieren.','Der Käsekompass ist wieder frei. Raffzahn zeichnet die Fluchtwege der Hasen rot ein.','Auch Ottah und Pafti finden kein Versteck mehr. Raffzahn hängt seinen Kompass über die Käsekammer.'],
      defeat:'Raffzahn verliert die Spur im Abfluss. Sein Käsekompass wartet beim nächsten Versuch.',quotes:['Der Boss hat jetzt Hausverbot im Kanal.','Ein Tunnelnetz genügt, und die Hasen rennen.','Mein Käse riecht weniger streng als dein Plan.'],danceQuotes:['Kompass geborgen, Boss zerlegt! Jetzt kommt der Schwanzkreisel!','Einmal im Kreis, zweimal über deinen Stolz: der Schwanzkreisel!','Die Ratte hat gewonnen. Achtung, jetzt dreht der Schwanzkreisel auf!']},
    krustenbraten:{relic:'geheimes Krustenrezept',dance:'DEN SPECKWACKEL-WALZER',
      intro:['DAS REZEPT IST VERSCHWUNDEN.','Korenlius öffnet seine Backstube und entdeckt einen Hasen mit seinem geheimen Krustenrezept unter dem Arm. Er schnappt sich die Pfanne und stürmt Hasenbein hinterher.','Niemand klaut mein Rezept und kommt ungebraten davon.'],
      plus:['ZWEITE RUNDE, ZWEITER OFEN.','Karnil schickt Hasenbein zurück und fälscht Korenlius’ Rezept. Die Sessel in der Backstube beben; Korenlius nimmt seine echten Notizen und den fertigen Build mit.','Für den Teig brauche ich Wut. Die habe ich.'],
      plus2:['DAS GROSSE SAHNE-DUELL.','Ottah hat die Sahne für Korenlius’ Siegesbraten beschlagnahmt. Korenlius zieht mit Pfanne und Rezept in die letzte Küche; Pafti wartet bereits am Herd.','Ich bin der Koch. Wer hier Boss spielt, wird Beilage.'],
      act1:['EIN HASE IN DER BRATENKÜCHE.','Hasenbein ist geschlagen, doch das Rezept ist weg. Unter dem Backofen zischt ein geheimes Labor; Cyber-Hasenbein kopiert jede Zeile auf seine Metallplatte.','Mein Rezept wird nicht von einer Maschine abgeschrieben.'],
      act2:['DIE RÄUCHEREI BEBT.','Die Rezeptkopien verbrennen. Aus der Räucherei dröhnt Karnil; das Sauferkel hält das Original zwischen seinen Steinplatten fest. Korenlius hebt die Pfanne.','Von Schwein zu Schwein: Gib das Blatt her.'],
      ottah:['EINE SAHNE ZU VIEL.','Karnil verliert das Rezept. Ottah beansprucht trotzdem die Küche und blockiert mit Pafti den letzten Ofen. Korenlius schiebt seinen Backofenwagen an.','Der Herd gehört dem, der kochen kann.'],
      pafti:['PAFTI VERSALZT DEN BRATEN.','Paftis Controller-Kopf leuchtet, und Karnil bekommt 25 % maximales Leben zurück. Korenlius stellt den Ofen auf höchste Stufe.','Ich schicke dir die Rechnung für diese Extra-Portion.'],
      victory:['DIE KRUSTE GEHÖRT KORENLIUS.','Korenlius serviert ein Festessen. Selbst die besiegten Hasen bekommen eine Portion, sofern sie das Rezept in Ruhe lassen.','Im blutigen Garten brennt ein kleiner Ofen. Korenlius rettet sein Rezept und eine warme Mahlzeit.','Ottah und Pafti geben die Küche frei. Korenlius backt in der Ruine das erste unverdorbene Brot.'],
      defeat:'Die Backstube ist leer, aber Korenlius hat seine Pfanne für den nächsten Anlauf aufgehoben.',quotes:['Der nächste Boss kriegt nur noch die kalte Pfanne.','Karnil, das Rezept ist in meiner Handschrift!','Ein bisschen mehr Kruste hätte dir gutgetan.'],danceQuotes:['Pfanne heiß, Boss kalt: jetzt kommt der Speckwackel-Walzer!','Der Braten ist fertig! Platz für meinen Speckwackel-Walzer!','Wenn die Kruste knackt, tanzt das Schwein den Speckwackel-Walzer!']},
    slanny:{relic:'Freiheitsabzeichen',dance:'DEN MÖHREN-MOONWALK',
      intro:['EIN HASE SAGT NEIN.','Hasenbein verlangt, dass Slanny für ihn jagt. Slanny verweigert den Befehl; der General reißt ihm sein Freiheitsabzeichen ab. Slanny verlässt die Hasenarmee und nimmt die Verfolgung auf.','Ich bin ein Hase. Euer Soldat bin ich nicht.'],
      plus:['FREIHEIT BRAUCHT EINEN ZWEITEN SIEG.','Hasenbein meldet sich mit Verstärkung zurück und will Slannys Abzeichen erneut einsammeln. Slanny trägt seinen fertigen Build in den verbotenen Möhrenpfad.','Du kannst mein Abzeichen nehmen. Meinen Willen nicht.'],
      plus2:['DIE LETZTE GRENZE.','Ottah und Pafti versperren die Wurzelkatakomben. Dort warten weitere Hasen, die sich von Hasenbein befreien wollen. Slanny geht voran.','Hinter mir wird nicht mehr marschiert. Neben mir schon.'],
      act1:['DER GENERAL FÄLLT, DIE KETTE BLEIBT.','Hasenbein ist besiegt. Slanny sieht sein Abzeichen auf einem Laborbildschirm: Cyber-Hasenbein hat es als Köder in sein System gesperrt.','Metallohren machen dich nicht zum Anführer.'],
      act2:['DIE WURZELN BRECHEN AUF.','Slanny schaltet den Cyberhasen ab und findet sein Abzeichen. Doch Karnil verschüttet die Lichtung, auf der Hasenbein seine Truppe versammelt hat.','Aus dem Schutt hole ich alle raus.'],
      ottah:['DAS LETZTE TOR.','Nach Karnils Fall wollen Ottah und Pafti den freien Hasen den Ausgang verschließen. Slanny lässt Wurzeln durch die Sperre brechen.','Hier kommt niemand mehr auf eure Liste.'],
      pafti:['PAFTI HÄLT DIE UHR AN.','Ein Mann mit einem SNES-Controller als Kopf heilt Karnil um 25 % seiner maximalen Lebenspunkte. Slanny setzt sein Abzeichen wieder auf.','Mehr Leben macht euren Befehl nicht richtiger.'],
      victory:['SLANNYS HASEN SIND FREI.','Slanny gibt jedem Hasen die Wahl, den Garten zu verlassen oder zu bleiben. Das Freiheitsabzeichen hängt über dem offenen Tor.','Selbst in den Blutlachen steckt ein kleiner freier Pfad. Slanny führt seine ehemaligen Kameraden hindurch.','Ottah und Pafti werden zurückgelassen. Die Ruine gehört nun den Hasen, die nie wieder Befehle von Hasenbein brauchen.'],
      defeat:'Slanny muss sich aus der Jagd zurückziehen. Sein Abzeichen liegt bereit für den nächsten Anlauf.',quotes:['Hasenbein: Ich bin hier der Hase mit dem Plan.','Dieser Möhrenbumerang war keine Einladung.','Ich hoppelte nicht weg. Ich kam zurück.'],danceQuotes:['Abzeichen zurück, Boss weg: Zeit für den Möhren-Moonwalk!','Hasenbein liegt. Ich gleite mit dem Möhren-Moonwalk hinaus!','Ein Hase kann auch frei tanzen: Möhren-Moonwalk!']},
    koettitroeter:{relic:'goldenes Trompetenmundstück',dance:'DIE TRÖTENDREHUNG DES GRAUENS',
      intro:['DAS KONZERT WURDE GESTOHLEN.','Der Köttitröter stimmt seine Trompete. Hasenbein schnappt sich das goldene Mundstück und erklärt den Garten zur Sperrzone für Musik. Der Troll folgt ihm auf die Bühne.','Das war mein Solo. Jetzt wird es laut.'],
      plus:['KEIN STILLER ZWEITER AKT.','Hasenbein kehrt mit Ohrstöpseln und noch mehr Hasen zurück. Der Köttitröter stimmt den alten Build neu und fordert sein Publikum heraus.','Ohrstöpsel helfen nicht gegen Rhythmus.'],
      plus2:['DIE LETZTE ZUGABE.','Ottah beansprucht den Konzertsaal und Pafti will den Strom abschalten. Der Köttitröter legt sein Mundstück ein und hebt die Trompete.','Meine letzte Note gilt euch beiden.'],
      act1:['DAS SOLO IM ORCHESTERWERK.','Hasenbein gibt das Mundstück nicht her: Ein Cyberhase übernimmt im Orchesterwerk die Verstärker und spielt jede Note verkehrt.','Wenn du Töne stiehlst, musst du sie auch treffen.'],
      act2:['KARNILS FALSCHER TAKT.','Der Cyberhase ist ausgeschaltet. Karnil lässt im Felsen-Resonanzsaal Geröll auf die Instrumente fallen. Der Köttitröter bläst einen tiefen Ton dagegen.','Jetzt kommt der Bass, ihr Felsnasen.'],
      ottah:['EIN DUETT OHNE EINLADUNG.','Der Saal ist wieder frei, bis Ottah und Pafti die Bühne stürmen. Der Köttitröter hebt seine Trompete zur letzten Zugabe.','Erst mein Konzert, dann eure Ausreden.'],
      pafti:['PAFTI SPIELT AUF REPEAT.','Sein SNES-Controller-Kopf heilt Karnil um 25 % seines maximalen Lebens. Der Köttitröter stellt seine Resonanzwand auf.','Das war die falsche Taste. Zugabe!'],
      victory:['DER KÖTTITRÖTER SPIELT WIEDER.','Der Garten hört das erste Konzert seit Hasenbeins Jagd. Karnil muss die schwersten Lautsprecher schleppen.','Zwischen blutigen Steinen spielt der Troll eine ruhige Melodie. Kein Hase wagt, das Mundstück noch einmal zu stehlen.','Ottah und Pafti räumen die Bühne. Über der verdorbenen Arena erklingt die lauteste Zugabe des Gartens.'],
      defeat:'Die Trompete ist verstimmt. Der Köttitröter holt Luft für eine neue Runde.',quotes:['Der Boss hat den Takt verloren.','Sogar Karnil hat im richtigen Moment gegrunzt.','Mein Mundstück, mein Konzert, mein Applaus.'],danceQuotes:['Vorhang auf! Jetzt kommt die Trötendrehung des Grauens!','Boss besiegt. Eins, zwei, tröööt: die Trötendrehung des Grauens!','Meine Zugabe heißt Trötendrehung des Grauens!']}
  };
  function heroSceneArt(id,chapter){const c=characterRoster[id],rosterColor=c.color,dark=impossibleMode?'#151924':hardMode?'#301d2c':'#19333b',groundColor=impossibleMode?'#392831':hardMode?'#593539':'#425449';
    const details=id==='raffzahn'?'<path d="M-35 13q-93-16-62 38" fill="none" stroke="#dab3c9" stroke-width="11"/><path d="M-31-30l-14-37 27 18m47 18 14-37-27 18" fill="#c5aec3"/>':id==='krustenbraten'?'<path d="M-35-29l-18-26 3 40m83-14 18-26-3 40" fill="#f0afaa"/><ellipse cy="4" rx="26" ry="17" fill="#f5c7b8"/><circle cx="-9" cy="3" r="4" fill="#ab7275"/><circle cx="9" cy="3" r="4" fill="#ab7275"/>':id==='slanny'?'<path d="M-24-26l-16-85q-14-14-13 7l6 81m54-2 17-86q13-14 13 8l-6 81" fill="#cfd2ac" stroke="#f1aba9" stroke-width="7"/>':'<path d="M-28-34l-31-28 8 43m79-15 31-28-8 43" fill="#b5c891"/><path d="M30 16l84-15v27l-84-4" fill="#eed991" stroke="#b79062" stroke-width="4"/>';
    return `<div class="character-scene character-scene-${id}" role="group" aria-label="${c.name}: ${chapter}"><svg viewBox="0 0 640 228" role="img" aria-label="${c.name} begegnet Hasenbein auf dem Weg zu ${escapeHTML(getStageName(stageVisual))}"><rect width="640" height="228" rx="13" fill="${dark}"/><circle cx="355" cy="65" r="45" fill="${rosterColor}" opacity=".24"/><path d="M0 186Q130 156 265 180T640 173V228H0Z" fill="${groundColor}"/><path d="M41 180q27-91 97-95m396 101q-44-102-119-112" fill="none" stroke="${rosterColor}" stroke-width="13" opacity=".30"/><g class="character-scene-hero" transform="translate(184 135)"><ellipse cy="14" rx="45" ry="40" fill="${rosterColor}"/>${details}<ellipse cy="-20" rx="37" ry="34" fill="${rosterColor}"/><ellipse cx="-14" cy="-24" rx="4" ry="5" fill="#29313a"/><ellipse cx="15" cy="-24" rx="4" ry="5" fill="#29313a"/><ellipse cy="-7" rx="8" ry="5" fill="#f1d7b9"/><path d="M-24 46l-13 27m58-27 12 27" stroke="${rosterColor}" stroke-width="11" stroke-linecap="round"/></g><g transform="translate(475 137)" fill="#a9ad96"><ellipse cy="4" rx="32" ry="37"/><path d="M-22-21l-12-54 15 33m38 19 13-52-2 52"/><ellipse cy="-16" rx="25" ry="25"/></g><g class="character-scene-relic"><circle cx="341" cy="98" r="24" fill="#f6d17d" stroke="#fff1bf" stroke-width="3"/><text x="341" y="107" text-anchor="middle" font-size="22" fill="#483637">${id==='raffzahn'?'⌖':id==='krustenbraten'?'▤':id==='slanny'?'✧':'♫'}</text></g><text x="320" y="215" text-anchor="middle" font-size="11" fill="#f4e8d2" font-family="Arial">${escapeHTML(chapter)}</text></svg></div>`;
  }
  const stanislavSceneBase=heroSceneArt;
  heroSceneArt=function(id,chapter,part='intro'){
    const figures={
      act1:['Cyber-Hasenbein','<ellipse cy="6" rx="34" ry="38" fill="#467e9b"/><path d="M-21-15l-11-60 20 39m24 21 18-62-3 60" fill="#76c9d1" stroke="#274c67" stroke-width="4"/><ellipse cy="-17" rx="27" ry="24" fill="#71bec9"/><circle cx="-12" cy="-19" r="5" fill="#f48393"/><circle cx="13" cy="-19" r="5" fill="#f48393"/><path d="M-26 20h52m-43-48h34" stroke="#d5fcff" stroke-width="3"/>'],
      act2:['Karnil','<ellipse cy="13" rx="48" ry="35" fill="#876c6d"/><path d="M-41-19l-14-43 35 31m43-1 34-30-12 43" fill="#b68e80"/><ellipse cy="-17" rx="42" ry="33" fill="#b7897c"/><ellipse cy="-1" rx="24" ry="14" fill="#d7a18f"/><circle cx="-9" cy="-2" r="4" fill="#664c52"/><circle cx="10" cy="-2" r="4" fill="#664c52"/><path d="M-40 1l-17 12 8-27m89 15 17 12-8-27" fill="#f6dfba"/>'],
      ottah:['Ottah und Pafti','<ellipse cy="17" rx="44" ry="34" fill="#816667"/><path d="M-38-16l-13-38 32 27m41 0 31-27-11 38" fill="#b18b7d"/><ellipse cy="-19" rx="39" ry="31" fill="#aa8175"/><ellipse cy="-2" rx="25" ry="15" fill="#e0aa94"/><path d="M-34 7l-12 16 3-27m76 11 12 16-3-27" fill="#f8dfbf"/><rect x="33" y="-71" width="45" height="29" rx="8" fill="#d8d0c3" stroke="#393547" stroke-width="4"/><path d="M41-57h11m-6-6v12" stroke="#333549" stroke-width="3"/><circle cx="66" cy="-58" r="3" fill="#bf6875"/>'],
      pafti:['Pafti','<path d="M-29 7h58l16 63h-92Z" fill="#655168"/><rect x="-40" y="-58" width="80" height="42" rx="12" fill="#e0d8c9" stroke="#52455d" stroke-width="5"/><path d="M-25-37h22m-11-11v22" stroke="#474252" stroke-width="5"/><circle cx="18" cy="-39" r="5" fill="#c96b8f"/><circle cx="29" cy="-29" r="5" fill="#947eb5"/><path d="M-30 9l-35 40m95-40 35 40" stroke="#857189" stroke-width="9"/>'],
      victory:['dem zurückeroberten Schatz','<circle cy="-16" r="43" fill="#f5dd8a" opacity=".26"/><path d="M-52 42l52-79 53 79Z" fill="#bfa571" stroke="#fbdf9e" stroke-width="5"/><circle cy="-2" r="19" fill="#f7de93"/><path d="M-9-2h18m-9-9v18" stroke="#745c48" stroke-width="4"/>'],
      endless:['vier vermischten Welten','<path d="M-52-54h49v49h-49zm55 0h49v49H3zm-55 55h49v49h-49zM3 1h49v49H3z" fill="#d6bb9a" stroke="#fff0cd" stroke-width="3"/><text x="-26" y="-20" text-anchor="middle" font-size="23" fill="#38434b">⌖</text><text x="28" y="-20" text-anchor="middle" font-size="23" fill="#38434b">♨</text><text x="-26" y="34" text-anchor="middle" font-size="23" fill="#38434b">✧</text><text x="28" y="34" text-anchor="middle" font-size="23" fill="#38434b">♫</text>']
    };
    const [label,body]=figures[part]||figures[part==='plus2'?'ottah':'act1'];
    // The original rabbit illustration belongs to the two Hasenbein opening
    // scenes; every later chapter has the actor and antagonist of that chapter.
    if(part==='intro'||part==='plus'||part==='defeat')return stanislavSceneBase(id,chapter);
    const art=stanislavSceneBase(id,chapter);
    return art.replace(/<g transform="translate\(475 137\)" fill="#a9ad96">.*?<\/g>/,
      `<g class="character-scene-antagonist" transform="translate(475 137)">${body}</g>`)
      .replace(`${characterRoster[id].name} begegnet Hasenbein auf dem Weg zu`,`${characterRoster[id].name} begegnet ${label} auf dem Weg zu`);
  };
  function storyAccent(){return impossibleMode?'IMPOSSIBLE · VERDORBENE WELT':hardMode?'HARD MODE · BLUTIGE JAGD':gamePlusLevel===2?'NEW GAME+2':gamePlusLevel===1?'NEW GAME+':'ANGENEHME HASENJAGD';}
  function characterScene(id,part,buttonId,label){const [title,copy,quote]=heroStory[id][part],rank=storyAccent();
    return `<span class="eyebrow">${rank} · ${characterRoster[id].name}</span><h2 id="overlayTitle">${title}</h2>${heroSceneArt(id,title,part)}<p>${copy}${impossibleMode?' Die Arena ist verdorben und die Gegner sind unerbittlich.':hardMode?' Blutige Spuren führen durch die neue Arena.':''}</p><blockquote class="snickers-quote"><b>${characterRoster[id].name}</b>„${quote}“</blockquote><button class="primary-button" id="${buttonId}">${label} ↗</button>`;
  }
  function replaceHeroInterlude(part,buttonId,label){const id=player?.characterId;if(!heroStory[id]||mode!=='interlude')return;
    const action=$(buttonId).onclick;showOverlay(characterScene(id,part,buttonId,label));$(buttonId).onclick=action;
  }
  const stanislavIntro=startGame;
  startGame=function(...args){stanislavIntro(...args);const id=player?.characterId;if(mode!=='intro'||!heroStory[id])return;
    const part=gamePlusLevel===2?'plus2':gamePlusLevel===1?'plus':'intro';
    showOverlay(endlessMode?`<span class="eyebrow">ENDLOSMODUS · ${characterRoster[id].name}</span><h2 id="overlayTitle">DIE VIER WELTEN MISCHEN SICH.</h2>${heroSceneArt(id,'ALLES DURCHEINANDER','endless')}<p>${characterRoster[id].name} kämpft diesmal durch Kanäle, Backstuben, Hasenmoore und Konzertbühnen zugleich. ${endlessFromHall?'Dein ausgewählter Hall-Build bleibt erhalten; sein Niveau verstärkt die Gegner.':'Dieser Run beginnt frisch.'} Jede fünfte Welle wartet ein Boss.</p><blockquote class="snickers-quote"><b>${characterRoster[id].name}</b>„${heroStory[id].intro[2]}“</blockquote><button class="primary-button" id="introButton">ENDLOS STARTEN ↗</button>`:characterScene(id,part,'introButton',gamePlusLevel===2?'RUNDE DREI BEGINNEN':gamePlusLevel===1?'RUNDE ZWEI BEGINNEN':'JAGD BEGINNEN'));
    $('introButton').onclick=beginFirstWave;
  };
  const stanislavActOne=showGeneralInterlude;
  showGeneralInterlude=function(){stanislavActOne();replaceHeroInterlude('act1','actTwoButton','ZUM NÄCHSTEN ABSCHNITT');};
  const stanislavActTwo=showCyberInterlude;
  showCyberInterlude=function(){stanislavActTwo();replaceHeroInterlude('act2','actThreeButton','ZUM NÄCHSTEN ABSCHNITT');};
  const stanislavOttah=showOttahInterlude;
  showOttahInterlude=function(){stanislavOttah();replaceHeroInterlude('ottah','ottahStart','ZUM LETZTEN BOSS');};
  ...
  bossQuoteMarkup=function(){const id=player?.characterId,story=heroStory[id];if(!story)return stanislavBossQuote();return `<blockquote class="snickers-quote"><b>${characterRoster[id].name}</b>„${story.quotes[Math.floor(Math.random()*story.quotes.length)]}“</blockquote>`;};
  const stanislavVictory=beginVictory;
  beginVictory=function(next){stanislavVictory(next);const id=player?.characterId,story=heroStory[id];if(mode!=='victory'||!story||!player.bossDance)return;
    const nextQuote=(player.lastBossQuote??0)%story.danceQuotes.length;
    player.bossDance.quote=story.danceQuotes[nextQuote];player.bossDance.dance=story.dance;
    syncBossCelebration();saveRunNow();
  };
  const stanislavFinish=finish;
  finish=function(won){stanislavFinish(won);const id=player?.characterId,story=heroStory[id];if(!story||endlessMode||mode!=='won'&&mode!=='lost')return;
    const next=$('retryButton').onclick,menu=$('endMenuButton').onclick;
    const tier=clamp(gamePlusLevel,0,2),mood=impossibleMode?'In der verdorbenen Welt':hardMode?'Zwischen Blutlachen':'Im Garten';
    const headline=won?story.victory[0]:characterRoster[id].name+' MUSS SICH ZURÜCKZIEHEN.';
    const conclusion=won?`${mood} endet ${characterRoster[id].name}s ${tier?'erneute ':'erste '}Jagd. ${story.victory[tier+1]}`:story.defeat;
    showOverlay(`<span class="eyebrow">${won?'ABSPANN · SIEG':'ABSPANN · NIEDERLAGE'} · ${storyAccent()}</span><h2 id="overlayTitle">${headline}</h2>${heroSceneArt(id,won?story.relic.toUpperCase():'BIS ZUM NÄCHSTEN VERSUCH',won?'victory':'defeat')}<p>${conclusion}</p><blockquote class="snickers-quote"><b>${characterRoster[id].name}</b>„${won?story.quotes[tier%story.quotes.length]:story.intro[2]}“</blockquote><div class="stats"><div><strong>${score.toLocaleString('de-DE')}</strong><span>PUNKTE</span></div><div><strong>${kills}</strong><span>GEGNER BESIEGT</span></div><div><strong>${formatTime(runTime)}</strong><span>ÜBERLEBT</span></div></div><div class="overlay-actions"><button id="retryButton" class="primary-button">${won?gamePlusLevel===0?'NEW GAME+ STARTEN ↗':gamePlusLevel===1?'NEW GAME+2 STARTEN ↗':'KOMPLETT NEUER RUN ↗':'WEITER ↗'}</button><button id="endMenuButton" class="secondary-button">Hauptmenü</button></div>`);
    $('retryButton').onclick=next;$('endMenuButton').onclick=menu;
  };

  skillBook.brunoOperation={icon:'✂',title:'DIE SESSELOPERATION',short:'Bruno läuft frei',requires:'bruno',requiresFlag:'bruno',desc:'Verbessert BRUNO STERNI (DIE SESSELSAU): entfernt seinen Sessel. Bruno läuft mit 205 Tempo über die Arena, feuert alle 0,68 statt 0,85 s und verursacht 2,5 statt 2 × √Angriffsschaden. Die fünf vorhandenen Bruno-Upgrades wirken weiter.',apply:p=>{p.brunoOperation=true;p.brunoX=p.x+90;p.brunoY=p.y-35;p.brunoCd=Math.min(p.brunoCd||.85,.35);}};
  purpleRewards.add('brunoOperation');upgradePools.forEach(pool=>pool.push('brunoOperation'));
  Object.assign(ngPlusSkillBook,{
    ngGardenEcho:{icon:'≋',title:'ECHO DER FÜNF WELTEN',short:'Welten-Echo',ngplus:true,desc:'NG+: Jede siebte normale Salve erzeugt höchstens alle 0,5 s zwei Echos in Zielrichtung. Jedes Echo trifft für 32 % des normalen Angriffsschadens. Echos lösen keine weiteren Echos aus.',apply:p=>p.ngGardenEcho=true},
  ...
  blueRewards.add('ngGardenEcho');blueRewards.add('ngTravelerCache');purpleRewards.add('ng2IronEncore');purpleRewards.add('ng2BattleRhythm');
  const stanislavFire=fire;
  fire=function(){const result=stanislavFire();if(!result||!player?.ngGardenEcho)return result;
    player.echoCd=Math.max(0,player.echoCd||0);if(player.shotCount%7===0&&player.echoCd<=0){player.echoCd=.5;for(const off of [-.20,.20])contentProjectile(player,player.angle+off,player.damage*.32,{speed:590,life:1.28,r:5,auto:true,characterAttack:player.characterId==='snickers'?null:player.characterId});}
    return result;
  };
  const stanislavExpansion=updateExpansion;
  updateExpansion=function(dt){stanislavExpansion(dt);if(mode!=='playing'||!player)return;
    player.echoCd=Math.max(0,(player.echoCd||0)-dt);
    if(player.ng2IronEncore){player.ironEncoreCd=Math.max(0,(player.ironEncoreCd||0)-dt);if(player.ironEncoreCd===0&&enemyBullets.some(b=>b.life>0&&dist(b,player)<110)){eraseNearbyShots(player,110,2);player.ironEncoreCd=11;}}
  };
  achievements.push(
    {id:'stan_rat_run',title:'KÄSEKOMPASS ZURÜCK',desc:'Schließe eine Normal-Kampagne mit Ratte Raffzahn ab.',goal:1,kind:'gag'},
    {id:'stan_rat_whistle',title:'UNTERGRUNDARBEIT',desc:'Setze Raffzahns Tunnelnetz 20-mal ein (über mehrere Runs).',goal:20,kind:'progress'},
    {id:'stan_pig_run',title:'KRUSTE GEGEN KARNIL',desc:'Schließe eine Hard-Kampagne mit Korenlius Krustenbraten ab.',goal:1,kind:'gag'},
    {id:'stan_pig_shield',title:'OFEN STATT PANIK',desc:'Fange als Korenlius insgesamt 25 gegnerische Geschosse mit dem Backofenwagen ab.',goal:25,kind:'progress'},
    {id:'stan_rabbit_run',title:'HASEN OHNE GENERAL',desc:'Schließe eine Impossible-Kampagne mit Slags Slanny ab.',goal:1,kind:'gag'},
    {id:'stan_rabbit_boss',title:'WURZELN IM CHEFBÜRO',desc:'Triff mit Slannys Wurzelfalle 12-mal einen Boss (über mehrere Runs).',goal:12,kind:'progress'},
    {id:'stan_troll_run',title:'LETZTE TROLLZUGABE',desc:'Schließe eine Kampagne mit dem Köttitröter ab.',goal:1,kind:'gag'},
    {id:'stan_troll_shield',title:'BLECHBLÄSER-BLOCKADE',desc:'Fange als Köttitröter mit der Resonanzwand insgesamt 30 gegnerische Geschosse ab.',goal:30,kind:'progress'},
    {id:'stan_all_starters',title:'VIER TÖNE, VIER WEGE',desc:'Benutze in beliebigen Runs alle vier exklusiven Startwaffen der neuen Figuren.',goal:4,kind:'collection'},
    {id:'stan_four_heroes',title:'DIE STANISLAV-TRUPPE',desc:'Schließe eine Kampagne mit jeder der vier neuen Figuren ab.',goal:4,kind:'collection'}
  );
  const stanislavConsumeWeapon=consumeWeapon;
  consumeWeapon=function(id){const used=stanislavConsumeWeapon(id);if(used&&player?.characterId!=='snickers'&&characterRoster[player.characterId]?.starter===id){recordDistinct('stan_all_starters',id);if(id==='ratWhistle')addAchievementProgress('stan_rat_whistle');}return used;};
  function countHeroVictory(id,difficulty){if(!heroStory[id])return;
    if(id==='raffzahn'&&difficulty==='normal')addAchievementProgress('stan_rat_run');
    if(id==='krustenbraten'&&difficulty==='hard')addAchievementProgress('stan_pig_run');
    if(id==='slanny'&&difficulty==='impossible')addAchievementProgress('stan_rabbit_run');
    if(id==='koettitroeter')addAchievementProgress('stan_troll_run');
    recordDistinct('stan_four_heroes',id);
  }
  const stanislavPersist=persistProgressionWin;
  persistProgressionWin=function(){stanislavPersist();countHeroVictory(player?.characterId,impossibleMode?'impossible':hardMode?'hard':'normal');};
  // Campaign wins stored by the previous release count retroactively. Weapon
  // usage was not archived, so usage achievements only count observed shots.
  for(const entry of hallOfFame)countHeroVictory(entry.build?.characterId,hallDifficulty(entry));

  $('startButton').onclick=()=>startGame(0);$('menuSettingsButton').onclick=showSettings;$('ngPlusButton').onclick=()=>startGame(1);$('ngPlus2Button').onclick=()=>startGame(2);$('hallOfFameButton').onclick=showHallOfFame;$('endlessButton').onclick=showEndlessSelect;$('hasenbeinButton').onclick=showHasenbeinSelect;$('helpButton').onclick=showHelp;$('achievementButton').onclick=showAchievements;$('powerupButton').onclick=showPowerups;$('pauseButton').onclick=pauseGame;$('soundButton').onclick=toggleSound;$('settingsButton').onclick=showSettings;
  updateNGPlusMenu();
  ...
  document.addEventListener('keydown',e=>{
    ...
    const gameKeys=['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space','KeyE','KeyQ','KeyR','KeyP','Escape',...(hasenbeinMode?['KeyF','KeyG']:[])];
    ...
    if(hasenbeinMode&&e.code==='KeyF')hasenbeinGuard();if(hasenbeinMode&&e.code==='KeyG')hasenbeinCall();
  });
  ...
  $('touchCycle').addEventListener('pointerdown',e=>{e.preventDefault();inputMode='touch';pointer.active=false;cycleWeapon(hasenbeinMode?-1:1);});
  $('touchShock').addEventListener('pointerdown',e=>{e.preventDefault();inputMode='touch';pointer.active=false;if(hasenbeinMode)hasenbeinEarthquake();});
  $('touchGuard').addEventListener('pointerdown',e=>{e.preventDefault();inputMode='touch';pointer.active=false;if(hasenbeinMode)hasenbeinGuard();});
  $('touchCall').addEventListener('pointerdown',e=>{e.preventDefault();inputMode='touch';pointer.active=false;if(hasenbeinMode)hasenbeinCall();});
  ...
  applySettings(false);
  initContentPatch();
  $('characterButton').onclick=showCharacters;
