...
(() => {
  ...
  let gamePlusLevel=0, newGamePlus=false, newGamePlus2=false, ngPlusUnlocked=false, ngPlusBuild=null, ngPlus2Unlocked=false, ngPlus2Build=null, ngPlusEverUnlocked=false, hallOfFame=[];
  try{
  ...
  function persistProgressionWin(){
    ...
    if(gamePlusLevel===0){
      ...
      ngPlus2Unlocked=true;ngPlus2Build=build;ngPlusEverUnlocked=true;
      try{localStorage.setItem('snickers3-ngplus2-unlocked-v1','1');localStorage.setItem('snickers3-ngplus2-build-v1',JSON.stringify(build));localStorage.setItem('snickers3-ngplus-ever-v1','1');}catch{}
    }else{
      const entry={completedAt:new Date().toISOString(),score,kills,time:Math.floor(runTime),maxHp:player.maxHp,retriesLeft,skills:Object.keys(player.skills||{}),specials:Object.keys(player.specials||{}),weapons:[...(player.weapons||[])],scorePerks:{...(player.scorePerks||{})}};
  ...
  function updateNGPlusMenu(){
    const b1=$('ngPlusButton'),b2=$('ngPlus2Button'),hall=$('hallOfFameButton');
  ...
  function startGame(level=0) {
    ...
    gamePlusLevel=Number(level)||0;newGamePlus=gamePlusLevel>0;newGamePlus2=gamePlusLevel===2;
  ...
  const weaponBook={
    ...
    ossiWall:{name:'OSSI MAUER',icon:'▥',desc:'Riegelt in normalen Wellen die Hälfte der Gegner kurz ab. Im Bosskampf blockt sie ungefähr die Hälfte der Boss-Schüsse.',max:3}
  };
  ...
  function useWeapon(){
    ...
    if(id==='nutBomb'){
      ...
      if(boss&&!boss.dead){
        player.ossiWallTime=Math.max(player.ossiWallTime||0,5.5);ossiWalls=[{x:player.x,y:player.y-78,life:5.5,maxLife:5.5,boss:true}];
        floater(player.x,player.y-54,'OSSI MAUER · 50 % SCHUSS-BLOCK',accent());tone(105,.42,'square',.07,55);
      }else{
  ...
  function spawnEnemy(type) {
    ...
    const enemyHpMult=gamePlusLevel===2?2:1,enemySpeedMult=gamePlusLevel===2?1.32:gamePlusLevel===1?1.1:1;
    const e={ x, y, type, hp:stats[0]*enemyHpMult, maxHp:stats[0]*enemyHpMult, speed:stats[1]*enemySpeedMult, r:stats[2], points:stats[3], phase:rnd(0,TAU), hit:0, fireCd:rnd(1.8,3), slow:0, chargeCd:rnd(1.6,2.8), windup:0, charge:0, aim:0, burstLeft:0, burstCd:0, dead:false, mortarCd:rnd(2.5,4), summonCd:rnd(3,5), summoned:false, mineCd:rnd(2.5,4), howlCd:rnd(2.8,4.5),teleportCd:rnd(2.4,4),rocketCd:rnd(2,3.4),shockCd:rnd(2.5,4) };
  ...
  const scoreSkillBook={
    ...
    dodge:{icon:'◈',title:'DODGE-CHANCE',desc:'+1 % Chance, einen gegnerischen Treffer komplett auszuweichen. Jede weitere Stufe gibt erneut +1 %.',apply:p=>p.dodgeChance=Math.min(.25,(p.dodgeChance||0)+.01)}
  };
  ...
  function bossUpgradeScreen(kind){
    ...
    showOverlay(`<span class="eyebrow">${label}</span><h2 id="overlayTitle">BEUTE AUS DEM BOSS.</h2><p>Wähle eines von drei Boss-Upgrades. Diese Belohnungen sind bewusst stärker als normale Wave-Upgrades, aber keine Legendary-3%-Skills. Nach dem Boss wurden deine Spezialwaffen wie nach einer Welle teilweise nachgeladen; die Nussbombe ist voll.</p><div class="upgrades special-upgrades">${choices.map((c,i)=>`<button class="upgrade special-upgrade" id="specialUpgrade${i}" data-skill="${c.id}"><small class="upgrade-kind special-kind">BOSS-UPGRADE</small><span class="upgrade-icon">${c.icon}</span><strong>${c.title}</strong><span>${c.desc}</span><em>SPEZIAL WÄHLEN ↗</em></button>`).join('')}</div>`);
    choices.forEach((c,i)=>{ $(`specialUpgrade${i}`).onclick=()=>{
      if(mode!=='bossUpgrade')return;c.apply(player);player.specials??={};player.specials[c.id]=(player.specials[c.id]||0)+1;player.weaponUses.nutBomb=maxWeaponAmmo('nutBomb');updateWeaponHud();updateSkills();tone(620,.25,'triangle',.08,1240);
  ...
  function upgradeScreen() {
    ...
    choices.forEach((c,i) => { $(`upgrade${i}`).onclick = () => {
      ...
      c.apply(player);if(!c.weapon)player.skills[c.id]=(player.skills[c.id]||0)+1;
  ...
  function finish(won) {
    ...
    if (won) { score += 5000 + player.hp * 250 + Math.max(0, Math.round((620 - runTime) * 15)); addAchievementProgress('wave_runner',9);if(lastDefeatedBoss==='karnil')addAchievementProgress('housepig');if(retriesLeft>0)addAchievementProgress('nutty_survivor');if(newGamePlus)addAchievementProgress('ngplus_clear');persistProgressionWin(); }
  ...
  function updateHud() {
    ...
    $('waveLabel').textContent = boss ? `${gamePlusLevel===2?'NG+2 · ':newGamePlus?'NG+ · ':''}${boss.karnil?'BOSS 03 / 03':boss.cyber?'BOSS 02 / 03':'BOSS 01 / 03'}` : `${gamePlusLevel===2?'NG+2 · ':newGamePlus?'NG+ · ':''}WELLE ${String(wave+1).padStart(2,'0')} / 09`;
  ...
  function hurtPlayer(damage = 1, source = null) {
    ...
    if(gamePlusLevel===1)damage*=2;else if(gamePlusLevel===2)damage*=4;
    player.waveHits++;
  ...
  function enemyShot(x,y,a,speed=174,cyber=false) { if(boss&&!boss.dead&&(player.ossiWallTime||0)>0&&Math.random()<.5){particles.push({type:'spark',x,y,life:.18,maxLife:.18,r:5,color:'#d5c7a0'});return;}if(enemyBullets.length<260)enemyBullets.push({x,y,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,a,r:7,life:7,cyber}); }
  ...
  function render(dt){
    ...
    for(const w of ossiWalls){
      const fade=clamp(w.life/w.maxLife,0,1);ctx.save();ctx.globalAlpha=.45+.55*fade;ctx.translate(w.x,w.y);ctx.fillStyle='#958a70';ctx.strokeStyle='#3f3a31';ctx.lineWidth=3;ctx.fillRect(-38,-18,76,36);ctx.strokeRect(-38,-18,76,36);ctx.strokeStyle='#c8bda4';ctx.lineWidth=2;for(let yy=-12;yy<=12;yy+=12){ctx.beginPath();ctx.moveTo(-34,yy);ctx.lineTo(34,yy);ctx.stroke();}for(let xx=-22;xx<=22;xx+=22){ctx.beginPath();ctx.moveTo(xx,-16);ctx.lineTo(xx,16);ctx.stroke();}ctx.fillStyle='#efe4c7';ctx.font='bold 9px Arial';ctx.textAlign='center';ctx.fillText('OSSI MAUER',0,3);ctx.restore();
    }
